...
public class CockpitA4MLaser extends CockpitGunner
{
    ...
    protected boolean doFocusEnter()
    {
        ...
        {
            ((SkyhawkA4M)aircraft()).bLASER = true;
    ...
    {
        super.doFocusLeave();
        ((SkyhawkA4M)aircraft()).bLASER = false;
    }
    ...
    {
        if(!((SkyhawkA4M)aircraft()).bNvision)
        {
        ...
        ((SkyhawkA4M)aircraft()).pos.getAbs(point3d, orient2);
    ...
    public void laserTrack(Orient orient)
    {
        ...
        if(!((SkyhawkA4M)aircraft()).hold)
        {
            LaserHook[1] = new HookNamed(super.mesh, "_Laser1");
            ((SkyhawkA4M)aircraft()).pos.getRender(_tmpLoc);
            LaserLoc1.set(0.0D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
            LaserHook[1].computePos(this, _tmpLoc, LaserLoc1);
            ...
            if(Landscape.rayHitHQ(LaserP1, LaserP2, LaserPL))
            {
                ...
                ((SkyhawkA4M)aircraft()).spot.set(LaserP2);
        ...
        if(((SkyhawkA4M)aircraft()).hold)
        {
            ...
            for(int j = 0; j < i; j++)
            {
                ...
                if(((actor instanceof ArtilleryGeneric) || (actor instanceof CarGeneric) || (actor instanceof TankGeneric) || (actor instanceof BigshipGeneric) || (actor instanceof ShipGeneric)) && !(actor instanceof StationaryGeneric) && actor != World.getPlayerAircraft() && actor.getArmy() != World.getPlayerArmy())
                {
                    Point3d pointOrtho = new Point3d();
                    pointOrtho.set(actor.pos.getAbsPoint());
                   // SkyhawkA4M _tmp1 = (SkyhawkA4M)aircraft();
                    if(((Tuple3d) (pointOrtho)).x > ((SkyhawkA4M)aircraft()).spot.x - 8D && ((Tuple3d) (pointOrtho)).x < ((SkyhawkA4M)aircraft()).spot.x + 8D && ((Tuple3d) (pointOrtho)).y < ((SkyhawkA4M)aircraft()).spot.y + 8D && ((Tuple3d) (pointOrtho)).y > ((SkyhawkA4M)aircraft()).spot.y - 8D)
                    {
    ...
    public void laser(Orient orient)
    {
        Orient orient2 = new Orient();
        Point3d point3d = new Point3d();
        ((Actor) ((SkyhawkA4M)aircraft())).pos.getAbs(point3d, orient2);
        float roll = orient2.getRoll();
        float fn = orient2.getPitch();
        float antiroll = 360F - roll;
        if(antiroll > 180F)
        ...
        dY = Math.sin(f) * d;
        dX = Math.cos(f) * d;
        float aa = ((SkyhawkA4M)aircraft()).azimult;
        float ta = ((SkyhawkA4M)aircraft()).tangate;
        if(f1 > 0.0F && f1 <= 90F)
        ...
        if(!((SkyhawkA4M)aircraft()).hold)
        {
            ((SkyhawkA4M)aircraft()).spot.set(spot1);
            v = 0.0F;
            h = 0.0F;
            v1 = 0.0F;
            h1 = 0.0F;
            ((SkyhawkA4M)aircraft()).azimult = 0.0F;
            ((SkyhawkA4M)aircraft()).tangate = 0.0F;
        }
        if(((SkyhawkA4M)aircraft()).hold)
        {
            ((SkyhawkA4M)aircraft()).spot.y += -ta;
            ((SkyhawkA4M)aircraft()).spot.x += aa;
            autotrack.clear();
            List list = Engine.targets();
            int i = list.size();
            for(int j = 0; j < i; j++)
            {
                ...
                if(((actor instanceof ArtilleryGeneric) || (actor instanceof CarGeneric) || (actor instanceof TankGeneric) || (actor instanceof BigshipGeneric) || (actor instanceof ShipGeneric)) && !(actor instanceof StationaryGeneric) && actor != World.getPlayerAircraft() && actor.getArmy() != World.getPlayerArmy())
                {
                    Point3d pointOrtho = new Point3d();
                    pointOrtho.set(actor.pos.getAbsPoint());
                    if(((Tuple3d) (pointOrtho)).x > ((Tuple3d) (((SkyhawkA4M)aircraft()).spot)).x - 10D)
                    {
                        if(((Tuple3d) (pointOrtho)).x < ((Tuple3d) (((SkyhawkA4M)aircraft()).spot)).x + 10D)
                        {
                            if(((Tuple3d) (pointOrtho)).y < ((Tuple3d) (((SkyhawkA4M)aircraft()).spot)).y + 10D)
                            {
                                if(((Tuple3d) (pointOrtho)).y > ((Tuple3d) (((SkyhawkA4M)aircraft()).spot)).y - 10D)
                                {
                                    autotrack.add(pointOrtho);
                                    ((SkyhawkA4M)aircraft()).spot.set(pointOrtho);
                                }
                            }
                        }
                    }
    ...
    public CockpitA4MLaser()
    {
        super("3DO/Cockpit/CockpitSkyhawkA4MLaser/A4MLaser.him", "he111_gunner");
    ...
    static 
    {
        Property.set(com.maddox.il2.objects.air.CockpitA4MLaser.class, "weaponControlNum", 10);
        Property.set(com.maddox.il2.objects.air.CockpitA4MLaser.class, "astatePilotIndx", 0);
    }
}