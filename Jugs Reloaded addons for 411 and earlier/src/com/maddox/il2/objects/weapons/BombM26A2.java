...
public class BombM26A2 extends Bomb
{
    ...
    {
        super.start();
		if (CommonUtils.is412orLater())
			setStartDelayedExplosion(true);
        if(CommonUtils.is411orLater() && fuze == null)
            t1 = Time.current() + 1000L;
        else
        if(delayExplosion == 0.0F)
            t1 = Time.current() + 200L;
        else
    ...
    {
        ExplodeCharge(pos.getAbsPoint());
        Actor actor = null;
        if(Actor.isValid(getOwner()))
            actor = getOwner();
        Orient orient = new Orient();
        Vector3d vector3d = new Vector3d();
        Point3d point3d = new Point3d(pos.getCurrentPoint());
        ...
        for(int i = 0; i < 20; i++)
        {
            ...
            ((Bomb) (bombm41a1)).pos.setUpdateEnable(true);
            ((Bomb) (bombm41a1)).pos.setAbs(point3d, orient);
            ((Bomb) (bombm41a1)).pos.reset();
