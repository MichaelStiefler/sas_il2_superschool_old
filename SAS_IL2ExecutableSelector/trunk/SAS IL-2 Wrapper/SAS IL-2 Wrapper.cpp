...
UINT SetForegroundWindowInternalThread(LPVOID pParam)
{
    ...
    if(SwitchToThisWindow != NULL) {
        ...
        MySwitchToThisWindow(hWndIl2Main, FALSE);
        bIl2MainActivated = TRUE;
    } else {
        bIl2MainActivated = SetForegroundWindowInternal(hWndIl2Main);
    }
...
{
    TRACE(L"IL-2 3D Window found (handle = 0x%08X)\r\n", hWndIl2Main);
    TRACE(L"SetForegroundWindowInternal(0x%08X)\r\n", hWndIl2Main);
    bIl2MainActivated = TRUE;
    CloseHandle(CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)SetForegroundWindowInternalThread, hWndIl2Main, 0, NULL));
}

//************************************
// Method:    CheckIl2MainWindow
// FullName:  CheckIl2MainWindow
// Access:    public 
// Returns:   void
// Qualifier:
//************************************
void CheckIl2MainWindow()
{
    if(!bIl2MainActivated) {
        HWND hWndIl2Main = FindWindow(L"MaddoxRtsWndClassW", NULL);

        if(hWndIl2Main != NULL) {
            ActivateIl2MainWindow(hWndIl2Main);
        }
    }
}
