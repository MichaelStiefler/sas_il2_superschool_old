...
public abstract class Beaufort extends Scheme2 implements TypeBomber {
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		...
		if (s.startsWith("xx")) {
			...
			if (s.startsWith("xxcontrols")) {
				...
				case 1: // '\001'
					if (getEnergyPastArmor(1.5F, shot) > 0.0F) {
						...
						if (World.Rnd().nextFloat() < 0.15F) {
							...
							mydebuggunnery("*** Engine1 Mix Controls Out..");
						}
					}
					break;
				...
				case 2: // '\002'
					...
					if (World.Rnd().nextFloat() < 0.15F) {
						...
						mydebuggunnery("*** Engine2 Mix Controls Out..");
					}
					break;
			...
			if (s.startsWith("xxengine")) {
				...
				if (s.startsWith("xxengine2"))
					j = 1;
				mydebuggunnery("Engine Module[" + j + "]: Hit..");
				if (getEnergyPastArmor(World.Rnd().nextFloat(0.2F, 0.55F), shot) > 0.0F) {
					if (World.Rnd().nextFloat() < shot.power / 280000F) {
						mydebuggunnery("Engine Module: Engine Crank Case Hit - Engine Stucks..");
						FM.AS.setEngineStuck(shot.initiator, j);
					}
					if (World.Rnd().nextFloat() < shot.power / 100000F) {
						mydebuggunnery("Engine Module: Engine Crank Case Hit - Engine Damaged..");
						FM.AS.hitEngine(shot.initiator, j, 2);
					}
				}
				if (getEnergyPastArmor(0.85F, shot) > 0.0F && World.Rnd().nextFloat() < FM.EI.engines[j].getCylindersRatio() * 0.75F) {
					FM.EI.engines[j].setCyliderKnockOut(shot.initiator, World.Rnd().nextInt(1, (int) (shot.power / 25200F)));
					mydebuggunnery("Engine Module: Cylinders Hit, " + FM.EI.engines[j].getCylindersOperable() + "/" + FM.EI.engines[j].getCylinders() + " Left..");
					if (World.Rnd().nextFloat() < shot.power / 80000F) {
						FM.AS.hitEngine(shot.initiator, j, 2);
						mydebuggunnery("Engine Module: Cylinders Hit - Engine Fires..");
					}
				}
			...
			if (s.startsWith("xxspar")) {
				...
				if (s.startsWith("xxspark") && chunkDamageVisible("Keel1") > 1 && getEnergyPastArmor(9.6F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F) {
					mydebuggunnery("*** Keel1 Spars Damaged..");
			...
			if (s.startsWith("xxstruts")) {
				if (s.startsWith("xxstruts1") && chunkDamageVisible("Engine1") > 1 && getEnergyPastArmor(19.5F * World.Rnd().nextFloat(1.0F, 8F), shot) > 0.0F) {
					mydebuggunnery("*** Engine1 Spars Damaged..");
				...
				if (s.startsWith("xxstruts2") && chunkDamageVisible("Engine2") > 1 && getEnergyPastArmor(19.5F * World.Rnd().nextFloat(1.0F, 8F), shot) > 0.0F) {
					mydebuggunnery("*** Engine2 Spars Damaged..");
				...
				if (getEnergyPastArmor(0.9F, shot) > 0.0F && World.Rnd().nextFloat() < 11.25F) {
					if (shot.power < 12000F) {
						if (FM.AS.astateTankStates[i1] == 0) {
							mydebuggunnery("Fuel Tank (" + i1 + "): Pierced..");
						...
						if (shot.powerType == 3 && World.Rnd().nextFloat() < 0.15F) {
							...
							mydebuggunnery("Fuel Tank (" + i1 + "): Hit..");
						}
					} else {
		...
		if (s.startsWith("xcf")) {
			...
			FM.EI.engines[0].setReadyness(shot.initiator, FM.EI.engines[0].getReadyness() - World.Rnd().nextFloat(0.0F, shot.power / 45000F));
			mydebuggunnery("*** Engine1 Hit - Readyness Reduced to " + FM.EI.engines[0].getReadyness() + "..");
		} else if (s.startsWith("xengine2")) {
			if (chunkDamageVisible("Engine2") < 2)
				hitChunk("Engine2", shot);
			FM.EI.engines[1].setReadyness(shot.initiator, FM.EI.engines[1].getReadyness() - World.Rnd().nextFloat(0.0F, shot.power / 45000F));
			mydebuggunnery("*** Engine2 Hit - Readyness Reduced to " + FM.EI.engines[1].getReadyness() + "..");
	...
	// {
	// super.update(f);
	// if(!isChunkAnyDamageVisible("Nose_D") &&
	// !hierMesh().isChunkVisible("Nose_CAP"))
	// {
	// mydebuggunnery("Make Nose CAP visible !!!!!!!!!!!!");
