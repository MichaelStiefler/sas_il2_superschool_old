...
public class CockpitCantZ506B_Bombardier extends CockpitPilot {
	...
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			...
			float f = ((CantZ506B) aircraft()).fSightCurForwardAngle;
			float f1 = 0.0F;
			if (bEntered) {
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (!aircraft().thisWeaponsName.startsWith("1x")) {
			...
			if (f2 > 360F)
				f2 -= 360F;
			f2 *= 0.00872664F;
			float f3 = ((CantZ506B) aircraft()).fSightSetForwardAngle - (float) Math.toRadians(f2);
			...
			mesh.chunkSetAngles("Cylinder", 0.0F, ((CantZ506B) aircraft()).fSightCurSideslip, 0.0F);
		}
	}
	...
	static {
		Property.set(CockpitCantZ506B_Bombardier.class, "astatePilotIndx", 0);
	}
}
