...
public class CockpitR_5_SKIS extends CockpitPilot {
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			if (fm != null) {
				...
				if (((R_5_SKIS) aircraft()).bChangedPit) {
					reflectPlaneToModel();
					((R_5_SKIS) aircraft()).bChangedPit = false;
				}
				setTmp = setOld;
				setOld = setNew;
				setNew = setTmp;
	...
	public void reflectWorldToInstruments(float f) {
		...
		mesh.chunkSetAngles("zSpeed1a", 0.0F, floatindex(cvt(Pitot.Indicator((float) fm.Loc.z, fm.getSpeedKMH()), 0.0F, 550F, 0.0F, 11F), speedometerScale), 0.0F);
		...
		if ((fm.AS.astateCockpitState & 8) == 0 && (fm.AS.astateCockpitState & 0x20) == 0) {
			...
			mesh.chunkSetAngles("zTCil1a", 0.0F, cvt(fm.EI.engines[0].tWaterOut, 0.0F, 350F, 0.0F, -75F), 0.0F);
		}
	...
	protected boolean doFocusEnter() {
		if (super.doFocusEnter()) {
			...
			if (hasTubeSight) {
				...
				point3d.set(0.18000000715255737D, 0.0D, 0.0D);
	...
	private final Point3d refSightAimPoint = new Point3d(-1.1000000000000001D, -0.001D, 0.873D);
