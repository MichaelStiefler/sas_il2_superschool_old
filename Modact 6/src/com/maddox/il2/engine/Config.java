...
public class Config {
	...
	public void load() throws GLContextException, ProviderException {
		...
		if (isUSE_RENDER()) {
			...
			windowUse3RendersUI = ini.get("window", "Use3RendersUI", windowUse3RendersUI);
	...
	public void save() {
		if (isUSE_RENDER()) {
			...
			ini.setValue("window", "Use3RendersUI", windowUse3RendersUI ? "1" : "0");
	...
	private void loadNet() {
		...
		NetChannel.checkTimeSpeedDifferense = ini.get("NET", "checkTimeSpeedDifferense",
				(float)NetChannel.checkTimeSpeedDifferense, 0.01F, 1000F);
		NetChannel.checkTimeSpeedInterval = ini
				.get("NET", "checkTimeSpeedInterval", NetChannel.checkTimeSpeedInterval, 1, 1000);
	...
	public GLContext createGlContext(GLContext glcontext, boolean flag, boolean flag1, int i, int j, int k, int l, int i1)
	...
	public GLContext createGlContext(String s) throws GLContextException {
		windowTitle = s;
		return createGlContext(null, windowChangeScreenRes, windowFullScreen, windowWidth, windowHeight, windowColourBits,
				windowDepthBits, windowStencilBits);
	}

	public static void typeCurrentScreenMode() {
		ScreenMode screenmode = ScreenMode.current();
		System.err.println("Current screen mode: " + screenmode.width() + "x" + screenmode.height() + "x"
				+ screenmode.colourBits());
	}
	...
	public static void typeScreenModes() {
		...
		for (int i = 0; i < ascreenmode.length; i++) {
			if (i % 4 == 0)
				System.err.println("");
			System.err.print("\t" + i + " " + ascreenmode[i].width() + "x" + ascreenmode[i].height() + "x"
					+ ascreenmode[i].colourBits());
		}
	...
	private static final String _VERSION = "4.13RC04";
	private static final int _TRACK_SINGLE_VERSION = 36;
	public static final int TRACK_NET_VERSION = 108;
	public static final boolean TRACK_CHECK = false;
	public static final String VERSION = "4.13RC04m";
	public static final String NET_VERSION = "FB_PF_v_4.13RC04m";
	public static final int TRACK_SINGLE_VERSION = 136;
