...
public abstract class PaintScheme {
	...
	public String typedName(Aircraft aircraft) {
		...
		return typedName(aircraft.getClass(), regiment, squadron.indexInRegiment(), wing.indexInSquadron(),
				wing.aircIndex(aircraft));
	}
	...
	public void prepare(Aircraft aircraft, boolean flag) {
		...
		Squadron squadron = wing.squadron();
		Regiment regiment = wing.regiment();
		prepare(aircraft.getClass(), aircraft.hierMesh(), regiment, squadron.indexInRegiment(), wing.indexInSquadron(),
				wing.aircIndex(aircraft), flag);
	}
	...
	public void prepareNum(Aircraft aircraft, int i, boolean flag) {
		...
		Squadron squadron = wing.squadron();
		Regiment regiment = wing.regiment();
		prepareNum(aircraft.getClass(), aircraft.hierMesh(), regiment, squadron.indexInRegiment(), wing.indexInSquadron(), i,
				flag);
	}
	...
	public void prepareNum(Class class1, HierMesh hiermesh, Regiment regiment, int i, int j, int k, boolean flag) {
		if (flag) {
			...
			if (regiment.country() == countryUSA || regiment.country() == countryBritain && "rn".equals(regiment.branch())
					|| regiment.country() == countryJapan || regiment.country() == countryUSABlue)
	...
	public static void init() {
		psGermanFighterString = (new String[][] {
				new String[] { I18N.color("White") + " ", I18N.color("Red") + " ", I18N.color("Gold") + " ",
						I18N.color("Green") + " " },
				new String[] { I18N.color("White") + " ", I18N.color("Black") + " ", I18N.color("Brown") + " ",
						I18N.color("Blue") + " " } });
		psRussianBomberString = (new String[] { I18N.color("Red"), I18N.color("Yellow"), I18N.color("Aqua"),
				I18N.color("Green") });
		psFinnishFighterString = (new String[][] {
				new String[] { I18N.color("Red") + " ", I18N.color("White") + " ", I18N.color("Yellow") + " ",
						I18N.color("Blue") + " " },
				new String[] { I18N.color("Red") + " ", I18N.color("White") + " ", I18N.color("Yellow") + " ",
						I18N.color("Blue") + " " } });
	}
	...
	// +++ TODO: 4.12 changed code +++
	protected void changeMat(Class class1, HierMesh hiermesh, String s, String s1, String s2, String s3, float f, float f1,
	...
	protected void changeMat(HierMesh hiermesh, String s, String s1, String s2, String s3, float f, float f1, float f2,
	...
	public static Mat makeMat(Class class1, String s, String s1, String s2, String s3, float f, float f1, float f2, float f3,
			float f4, float f5) {
		if (!Config.isUSE_RENDER())
			return null;
		if (class1 != null)
	...
	private static void customizeMat(SectFile sectfile, Class class1, String s, String s1, boolean flag) {
		...
		} else if (class1.isAssignableFrom(DXXI_SARJA3_EARLY.class) || class1.isAssignableFrom(DXXI_SARJA3_LATE.class)
				|| class1.isAssignableFrom(DXXI_SARJA4.class)) {
		...
		} else if (class1.isAssignableFrom(P_36A3.class) || class1.isAssignableFrom(P_36A4.class)
				|| class1.isAssignableFrom(H_75A3.class) || class1.isAssignableFrom(H_75A4.class)) {
		...
		} else if ((class1.isAssignableFrom(P_40E_NEW.class) || class1.isAssignableFrom(P_40EM105_NEW.class))
				&& s1.indexOf("FINACID") != -1) {
	...
	private String getPlaneSpecificMaterialID(Class class1) {
		if (class1.isAssignableFrom(IL_2I.class) || class1.isAssignableFrom(IL_2MLate.class)
				|| class1.isAssignableFrom(IL_2Type3.class) || class1.isAssignableFrom(IL_2Type3M.class))
			return "IL-2";
		if (class1.isAssignableFrom(MIG_3EARLY.class) || class1.isAssignableFrom(MIG_3AM38.class)
				|| class1.isAssignableFrom(MIG_3POKRYSHKIN.class) || class1.isAssignableFrom(MIG_3SHVAK.class)
				|| class1.isAssignableFrom(MIG_3UB.class) || class1.isAssignableFrom(MIG_3UD.class))
			return "MIG-3";
		if (class1.isAssignableFrom(U_2VS.class))
			return "U-2";
		if (class1.isAssignableFrom(R_10.class))
	...
	private String[] getCustomMarkings(Class class1, String s, String s1, String s2) {
		...
		if (s.equals("whitestar1")) {
			...
			if (i > 0) {
				...
				if (i > 0x12857b4 && i < 0x1287ce4 && s2.equals("Overlay7")
						&& (World.cur().camouflage == 2 || World.cur().camouflage == 5)) {
			...
			if (s.equals("redstar0") || s.equals("redstar1") || s.equals("redstar2") || s.equals("redstar3")) {
				...
				if (j > 0) {
					...
					// +++ TODO: 4.12 changed code +++
					if (j < 19391000 && !(SB_2M100A.class).isAssignableFrom(class1)
							&& !(SB_2M103.class).isAssignableFrom(class1) && !(IL_4_DB3B.class).isAssignableFrom(class1)
							&& !(IL_4_DB3F.class).isAssignableFrom(class1) && !(IL_4_DB3M.class).isAssignableFrom(class1)
							&& !(IL_4_DB3T.class).isAssignableFrom(class1) && !(IL_4_IL4.class).isAssignableFrom(class1))
					// --- TODO: 4.12 changed code ---
					{
			...
			} else if (BF_109G6AS.class.isAssignableFrom(class1) || BF_109G6Late.class.isAssignableFrom(class1)
					|| BF_109G10.class.isAssignableFrom(class1) || FW_190F8.class.isAssignableFrom(class1)
					|| FW_190A8.class.isAssignableFrom(class1) || FW_190A6.class.isAssignableFrom(class1)) {
				if (s.equals("balken0"))
	...
	public static final char psGermanBomberLetter[][] = { { 'H', 'K', 'L', 'B' }, { 'M', 'N', 'P', 'C' },
			{ 'R', 'S', 'T', 'D' }, { 'U', 'V', 'W', 'E' }, { 'X', 'Y', 'Z', 'F' } };
	public static final float psGermanBomberColor[][] = { { 0.945F, 0.929F, 0.886F }, { 0.584F, 0.122F, 0.122F },
			{ 0.89F, 0.729F, 0.0F }, { 0.247F, 0.806F, 0.0F } };
	...
	public static final float psRussianBomberColor[][] = { { 0.718F, 0.176F, 0.102F }, { 1.0F, 0.843F, 0.314F },
			{ 0.247F, 0.733F, 0.937F }, { 0.263F, 0.447F, 0.184F } };
	public static String psFinnishFighterString[][];
	public static final float psFinnishFighterColor[][] = { { 0.584F, 0.122F, 0.122F }, { 0.945F, 0.929F, 0.886F },
			{ 0.89F, 0.729F, 0.0F }, { 0.1F, 0.235F, 0.695F } };
	public static final String psFinnishFighterPrefix[][] = { { "German/02", "German/", "German/03", "German/10" },
