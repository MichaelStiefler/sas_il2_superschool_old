...
public class F_18D extends F_18 implements TypeGuidedMissileCarrier, TypeCountermeasure, TypeThreatDetector, TypeGSuit, TypeAcePlane, TypeFuelDump, TypeStormovikArmored, TypeDockable {
	...
	public void update(float f) {
		...
		if(FM instanceof Maneuver)
			...
			{
				if(((FlightModelMain) (FM)).CT.getRefuel() < 0.90F)
		...
		if(this.backfire)
			backFire();
		if((!(FM instanceof RealFlightModel) || !((RealFlightModel)FM).isRealMode()) && (FM instanceof Maneuver) && ((Maneuver)FM).get_task() == 7 && !((FlightModelMain) (FM)).AP.way.isLanding()) //TODO straft
		{
			...
			else if(pilot != null && isAlive(pilot.target_ground))
			{
	...
	public void typeDockableAttemptAttach()
	{
		if(((FlightModelMain) (FM)).AS.isMaster() && !typeDockableIsDocked())
		{
			...
			if(aircraft instanceof TypeTankerDrogue && ((FlightModelMain) (FM)).CT.getRefuel() > 0.95F)
