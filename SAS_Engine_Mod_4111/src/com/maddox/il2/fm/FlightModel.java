...
public class FlightModel extends FlightModelMain
{
	...
	//TODO: Replace shakeMe by new parametrized version from 4.11.1
	//--------------------------------------------------------------------------
//	public void shakeMe(float f, float f1, float f2)
//	{
//		producedAMM.x += f;
//		producedAMM.y += f1;
//		producedAMM.z += f2;
//	}
	...
	private void updateTurret(Turret turret1, int i, float f)
	{
		...
		if (W.lengthSquared() > 0.25D)
			f2 *= 1.0F - (float) Math.sqrt(W.length() - 0.5D);
		if (getOverload() > 0.5F)
			f2 *= Aircraft.cvt(getOverload(), 0.0F, 5F, 1.0F, 0.0F);
		else if (getOverload() < -0.25F)
			f2 *= Aircraft.cvt(getOverload(), -1F, 0.0F, 0.0F, 1.0F);
		if (turret1.target != null && (turret1.target instanceof Aircraft)
				&& ((Aircraft) turret1.target).FM.isTakenMortalDamage())
			turret1.target = null;
		...
		else
		{
			...
			f1 = (float) Ve.length();
			float f3 = 10F * (float) Math.sin((float) (Time.current() & 65535L) * 0.003F);
		...
		case 0: // '\0'
			...
			if (Time.current() > turret1.timeNext && World.Sun().ToSun.z < -0.22F)
			{
		...
		case 1: // '\001'
			...
			if ((float) (World.Rnd().nextInt() & 0xff) >= 32F * (f2 + 1.0F) && f1 >= 148F + 27F * f2)
				break;
			if (f1 < 450F + 66.6F * f2)
			{
				...
				case 0: // '\0'
					if (((Tuple3d) (Vt)).x < -0.95999997854232788D)
					{
					...
					if (((Tuple3d) (Vt)).x < -0.33000001311302185D)
					{
						turret1.tMode = 3;
				...
				case 1: // '\001'
				case 2: // '\002'
					if (((Tuple3d) (Vt)).x < -0.9100000262260437D)
					{
		...
		case 3: // '\003'
			...
			turret1.tuLim[0] = tu[0] * World.Rnd().nextFloat(0.85F + 0.05F * f2, 1.15F - 0.05F * f2)
					+ World.Rnd().nextFloat(-tAcc2 + tAcc3 * f2, tAcc2 - tAcc3 * f2);
			turret1.tuLim[1] = tu[1] * World.Rnd().nextFloat(0.85F + 0.05F * f2, 1.15F - 0.05F * f2)
					+ World.Rnd().nextFloat(-tAcc2 + tAcc3 * f2, tAcc2 - tAcc3 * f2);
			if (Time.current() > turret1.timeNext)
				turret1.tMode = 1;
			break;
		...
		case 2: // '\002'
			...
			if(this.actor != World.getPlayerAircraft() && !((Aircraft)this.actor).isNetPlayer())
			{
				turret1.tuLim[0] = tu[0] * World.Rnd().nextFloat(1.82F + 0.5F * f2, 1.82F - 0.5F * f2) + World.Rnd().nextFloat(-5F + 1.666F * f2, 5F - 1.666F * f2);
				turret1.tuLim[1] = tu[1] * World.Rnd().nextFloat(1.82F + 0.5F * f2, 1.82F - 0.5F * f2) + World.Rnd().nextFloat(-5F + 1.666F * f2, 5F - 1.666F * f2);
				if(Time.current() > turret1.timeNext)
				{
					turret1.tMode = 1;
					...
					{
						turret1.tMode = 0;
						turret1.timeNext = Time.current() + World.Rnd().nextLong(100L, (long)((f2 + 1.0F) * 700F));
					}
				}
			} else
			{
	...
	private float setGunner()
	{
		...
		java.lang.String s1 = this.actor.name();
		int i = s1.length();
	...
	private boolean isNightTargetVisible(Actor actor, float f)
	{
		...
		float f6 = cvt(f1, 0.095F, 1.0F, 1E-005F, tAcc1) * f5 * f4;
