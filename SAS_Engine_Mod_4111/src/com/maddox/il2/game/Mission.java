...
public class Mission implements Destroy
{
	...
	public Aircraft loadAir(SectFile sectfile, String string,
			String string_66_, String string_67_,
		...
		aircraft.setName(string_67_);
		int i_68_ = 0;
		if (bool) {
			i_68_ = ((Integer) actors.get(curActor)).intValue();
			if (i_68_ == 0)
				aircraft.load(sectfile, string_66_, i, null, 0);
			else
				aircraft.load(sectfile, string_66_, i, net.masterChannel(),
						i_68_);
		} else
			aircraft.load(sectfile, string_66_, i, null, 0);
	...
	public static void preparePlayerNumberOn(SectFile sectfile) {
		...
		if (!"".equals(string)) {
			String string_69_ = sectfile.get("MAIN", "playerNum", "");
			sectfile.set(string, "numberOn" + string_69_,
	...
	private void prepareTakeoff(SectFile sectfile, boolean bool) {
		if (bool) {
			...
			for (int i_71_ = 0; i_71_ < i_70_; i_71_++)
				prepareWingTakeoff(sectfile, sectfile.var(i, i_71_));
		} else {
	...
	private void prepareWingTakeoff(SectFile sectfile, String string) {
		String string_72_ = string + "_Way";
		int i = sectfile.sectionIndex(string_72_);
		if (i >= 0) {
			int i_73_ = sectfile.vars(i);
			if (i_73_ != 0) {
				ArrayList arraylist = new ArrayList(i_73_);
				for (int i_74_ = 0; i_74_ < i_73_; i_74_++)
					arraylist.add(sectfile.line(i, i_74_));
				String string_75_ = (String) arraylist.get(0);
				if (string_75_.startsWith("TAKEOFF")) {
					StringBuffer stringbuffer = new StringBuffer("NORMFLY");
					NumberTokenizer numbertokenizer
					= new NumberTokenizer(string_75_);
					numbertokenizer.next((String) null);
					double d = numbertokenizer.next(1000.0);
					double d_76_ = numbertokenizer.next(1000.0);
					WingTakeoffPos wingtakeoffpos
					= new WingTakeoffPos(d, d_76_);
					if (mapWingTakeoff == null) {
						mapWingTakeoff = new HashMap();
						mapWingTakeoff.put(wingtakeoffpos, wingtakeoffpos);
					} else {
						for (;;) {
							WingTakeoffPos wingtakeoffpos_77_
							= ((WingTakeoffPos)
									mapWingTakeoff.get(wingtakeoffpos));
							if (wingtakeoffpos_77_ == null) {
					...
					d_76_ = (double) wingtakeoffpos.y;
					stringbuffer.append(" ");
					stringbuffer.append(d);
					stringbuffer.append(" ");
					stringbuffer.append(d_76_);
					do {
						if (i_73_ > 1) {
							String string_78_ = (String) arraylist.get(1);
							if (!string_78_.startsWith("TAKEOFF")
									&& !string_78_.startsWith("LANDING")) {
								numbertokenizer
								= new NumberTokenizer(string_78_);
								numbertokenizer.next((String) null);
								numbertokenizer.next((String) null);
								numbertokenizer.next((String) null);
								stringbuffer.append(" ");
					...
					for (int i_79_ = 0; i_79_ < i_73_; i_79_++)
						sectfile.lineAdd(i, (String) arraylist.get(i_79_));
				}
			}
		}
	}
	...
	private void loadChiefs(SectFile sectfile) throws Exception {
		...
		if (i >= 0) {
			...
			int i_80_ = sectfile.vars(i);
			for (int i_81_ = 0; i_81_ < i_80_; i_81_++) {
				LOADING_STEP((float) (60
						+ Math.round((float) i_81_
								/ (float) i_80_ * 20.0F)),
								"task.Load_tanks");
				NumberTokenizer numbertokenizer
				= new NumberTokenizer(sectfile.line(i, i_81_));
				String string = numbertokenizer.next();
				String string_82_ = numbertokenizer.next();
				int i_83_ = numbertokenizer.next(-1);
				if (i_83_ < 0)
					System.out.println("Mission: Wrong chief's army [" + i_83_
							+ "]");
				else {
					...
					if (Chief.new_SKILL_IDX < 0 || Chief.new_SKILL_IDX > 3)
						...
						else if (chiefsIni.sectionIndex(string_82_) < 0)
							System.out.println("Mission: Wrong chief's type ["
									+ string_82_ + "]");
						else {
							int i_84_ = string_82_.indexOf('.');
							if (i_84_ <= 0)
								System.out.println
								...
								String string_85_
								= string_82_.substring(0, i_84_);
								String string_86_
								= string_82_.substring(i_84_ + 1);
								String string_87_
								= chiefsIni.get(string_85_, string_86_);
								if (string_87_ == null)
									System.out.println
									("Mission: Wrong chief's type ["
											+ string_82_ + "]");
									...
									numbertokenizer
									= new NumberTokenizer(string_87_);
									string_87_ = numbertokenizer.nextToken();
									numbertokenizer.nextToken();
									String string_88_ = null;
									if (numbertokenizer.hasMoreTokens())
										string_88_
										= numbertokenizer.nextToken();
									Class var_class
									= ObjIO.classForName(string_87_);
									if (var_class == null)
										System.out.println
										...
										try {
											...
											constructor = var_class.getConstructor(aclass);
										} catch (Exception exception) {
											System.out.println
										...
										try {
											...
											objects[0] = string;
											objects[1] = new Integer(i_83_);
											objects[2] = chiefsIni;
											objects[3] = string_82_;
											objects[4] = sectfile;
										...
										if (string_88_ != null)
											((Actor) object).icon
											= IconDraw.get(string_88_);
										if (i_89_ != curActor && net != null
												&& net.isMirror()) {
											for (int i_90_ = i_89_;
													i_90_ < curActor; i_90_++) {
												Actor actor
												= ((Actor)
														actors.get(i_90_));
												if (actor.net == null
	...
	private Actor loadStationaryActor(String string, String string_91_, int i,
			double d, double d_92_, float f,
			float f_93_, String string_94_,
			String string_95_, String string_96_) {
		Class var_class;
		try {
			var_class = ObjIO.classForName(string_91_);
		} catch (Exception exception) {
			System.out
			.println("Mission: class '" + string_91_ + "' not found");
			return null;
		}
		ActorSpawn actorspawn
		= (ActorSpawn) Spawn.get(var_class.getName(), false);
		if (actorspawn == null) {
			System.out.println("Mission: ActorSpawn for '" + string_91_
					+ "' not found");
			return null;
		}
		spawnArg.clear();
		...
		spawnArg.country = string_94_;
		Chief.new_DELAY_WAKEUP = 0.0F;
		ArtilleryGeneric.new_RADIUS_HIDE = 0.0F;
		if (string_94_ != null) {
			try {
				Chief.new_DELAY_WAKEUP = (float) Integer.parseInt(string_94_);
		...
		if (string_95_ != null) {
			try {
				int i_97_ = Integer.parseInt(string_95_);
				Chief.new_SKILL_IDX = i_97_;
				spawnArg.skill = i_97_;
			} catch (Exception exception) {
				/* empty */
			}
			if (Chief.new_SKILL_IDX < 0 || Chief.new_SKILL_IDX > 3) {
		...
		if (string_96_ != null) {
			try {
				Chief.new_SLOWFIRE_K = Float.parseFloat(string_96_);
			} catch (Exception exception) {
				/* empty */
			}
	...
	private void loadStationary(SectFile sectfile) {
		...
		if (i >= 0) {
			...
			for (int i_99_ = 0; i_99_ < i_98_; i_99_++) {
				LOADING_STEP((float) (80 + Math.round((float) i_99_
						/ (float) i_98_ * 5.0F)),
	...
	private void loadRocketry(SectFile sectfile) {
		...
		if (i >= 0) {
			int i_102_ = sectfile.vars(i);
			for (int i_103_ = 0; i_103_ < i_102_; i_103_++) {
				NumberTokenizer numbertokenizer
				= new NumberTokenizer(sectfile.line(i, i_103_));
				if (numbertokenizer.hasMoreTokens()) {
					String string = numbertokenizer.next("");
					if (numbertokenizer.hasMoreTokens()) {
						String string_104_ = numbertokenizer.next("");
						if (numbertokenizer.hasMoreTokens()) {
							int i_105_ = numbertokenizer.next(1, 1, 2);
							double d = numbertokenizer.next(0.0);
							if (numbertokenizer.hasMoreTokens()) {
								double d_106_ = numbertokenizer.next(0.0);
								if (numbertokenizer.hasMoreTokens()) {
									float f = numbertokenizer.next(0.0F);
									if (numbertokenizer.hasMoreTokens()) {
										float f_107_
										= numbertokenizer.next(0.0F);
										int i_108_ = numbertokenizer.next(1);
										float f_109_
										...
										try {
											...
											= (RocketryGeneric.New
													(string, string_104_,
															netchannel, i_110_, i_105_,
															d, d_106_, f, f_107_,
															i_108_, f_109_, point2d));
										} catch (Exception exception) {
											System.out.println
	...
	private void loadBornPlaces(SectFile sectfile) {
		...
		if (i >= 0) {
			int i_112_ = sectfile.vars(i);
			if (i_112_ != 0 && World.cur().airdrome != null
					&& World.cur().airdrome.stay != null) {
				World.cur().bornPlaces = new ArrayList(i_112_);
				for (int i_113_ = 0; i_113_ < i_112_; i_113_++) {
					NumberTokenizer numbertokenizer
					= new NumberTokenizer(sectfile.line(i, i_113_));
					int i_114_
					= numbertokenizer.next(0, 0, Army.amountNet() - 1);
					float f = (float) numbertokenizer.next(1000, 500, 10000);
					double d = (double) (f * f);
					float f_115_ = (float) numbertokenizer.next(0);
					float f_116_ = (float) numbertokenizer.next(0);
					...
					int i_135_ = 0;
					try {
						i_117_ = numbertokenizer.next(1000, 0, 10000);
						i_135_++;
						i_118_ = numbertokenizer.next(200, 0, 500);
						i_135_++;
						i_119_ = numbertokenizer.next(0, 0, 360);
						i_135_++;
						i_120_ = numbertokenizer.next(0, 0, 99999);
						i_135_++;
						i_121_ = numbertokenizer.next(0, 0, 99999);
						i_135_++;
						i_122_ = numbertokenizer.next(5000, 0, 99999);
						i_135_++;
						i_123_ = numbertokenizer.next(50, 1, 99999);
						i_135_++;
						...
						d_131_ = numbertokenizer.next(3.8, 0.0, 10.0);
						if (numbertokenizer.next(0, 0, 1) == 1) {
							bool_128_ = true;
							i_135_++;
						}
					...
					if (!isDogfight())
						...
						for (int i_137_ = 0; i_137_ < point_stays.length;
								i_137_++) {
							if (point_stays[i_137_] != null) {
								Point_Stay point_stay
								= (point_stays[i_137_]
										[point_stays[i_137_].length - 1]);
								if ((double) (((point_stay.x - f_115_)
										* (point_stay.x - f_115_))
										+ ((point_stay.y - f_116_)
												* (point_stay.y - f_116_)))
												<= d) {
					...
					if (bool_136_) {
						BornPlace bornplace
						= new BornPlace((double) f_115_, (double) f_116_,
								i_114_, f);
						bornplace.zutiRadarHeight_MIN = i_121_;
						bornplace.zutiRadarHeight_MAX = i_122_;
						bornplace.zutiRadarRange = i_123_;
						bornplace.zutiSpawnHeight = i_117_;
						bornplace.zutiSpawnSpeed = i_118_;
						bornplace.zutiSpawnOrient = i_119_;
						bornplace.zutiMaxBasePilots = i_120_;
						bornplace.zutiAirspawnIfCarrierFull = bool_124_;
						bornplace.zutiAirspawnOnly = bool_125_;
						bornplace.zutiDisableSpawning = bool_129_;
						bornplace.zutiEnableFriction = bool_130_;
						bornplace.zutiFriction = d_131_;
						bornplace.zutiEnablePlaneLimits = bool_126_;
						bornplace.zutiDecreasingNumberOfPlanes = bool_127_;
						bornplace.zutiIncludeStaticPlanes = bool_128_;
						bornplace.zutiBpIndex = i_113_;
						bornplace.zutiStaticPositionOnly = bool_132_;
						bornplace.spawnFromStationatyPlanes = bool_133_;
						bornplace.restoreOriginalSpawnPoint = bool_134_;
						bornplace.bParachute = bool;
						World.cur().bornPlaces.add(bornplace);
						if (actors != null) {
							int i_138_ = actors.size();
							for (int i_139_ = 0; i_139_ < i_138_; i_139_++) {
								Actor actor = (Actor) actors.get(i_139_);
								if (Actor.isValid(actor) && actor.pos != null
										&& ZutiSupportMethods
										.isStaticActor(actor)) {
									Point3d point3d = actor.pos.getAbsPoint();
									double d_140_
									= (((point3d.x - (double) f_115_)
											* (point3d.x - (double) f_115_))
											+ ((point3d.y - (double) f_116_)
													* (point3d.y
															- (double) f_116_)));
									if (d_140_ <= d)
						...
						int i_141_
						= sectfile.sectionIndex("BornPlace" + i_113_);
						if (i_141_ >= 0) {
							int i_142_ = sectfile.vars(i_141_);
							for (int i_143_ = 0; i_143_ < i_142_; i_143_++) {
								String string = sectfile.line(i_141_, i_143_);
								...
								while (stringtokenizer.hasMoreTokens()) {
									...
									case 1:
										...
										break;
									default:
										string_144_ += " " + stringtokenizer
										.nextToken();
									}
									i_145_++;
								}
								zutiaircraft.setLoadedWeapons(string_144_,
										false);
								String string_146_ = zutiaircraft.getAcName();
								if (string_146_ != null) {
									string_146_ = string_146_.intern();
									Class var_class
									= ((Class)
											Property.value(string_146_,
													"airClass", null));
									if (var_class != null) {
		...
		if (Main.cur().netServerParams.isSingle()
			...
			if (i >= 0) {
				...
				for (int i_148_ = 0; i_148_ < i_147_; i_148_++)
					Target.create(sectfile.line(i, i_148_));
			}
		}
	}
	...
	private void checkBridgesAndHouses(SectFile sectfile) {
		...
		for (int i_157_ = 0; i_157_ < i; i_157_++) {
			String string = sectfile.sectionName(i_157_);
			if (string.endsWith("_Way")) {
				int i_158_ = sectfile.vars(i_157_);
				for (int i_159_ = 0; i_159_ < i_158_; i_159_++) {
					String string_160_ = sectfile.var(i_157_, i_159_);
					if (string_160_.equals("GATTACK")) {
						SharedTokenizer.set(sectfile.value(i_157_, i_159_));
						...
						= SharedTokenizer.next((String) null);
						if (string_161_ != null
								&& string_161_.startsWith("Bridge")) {
							LongBridge longbridge
							= ((LongBridge)
									Actor.getByName(" " + string_161_));
				...
				int i_162_ = sectfile.vars(i_157_);
				for (int i_163_ = 0; i_163_ < i_162_; i_163_++) {
					SharedTokenizer.set(sectfile.value(i_157_, i_163_));
					SharedTokenizer.next((String) null);
					int i_164_ = (int) SharedTokenizer.next(1.0);
					if (i_164_ < 0) {
						i_164_ = -i_164_ - 1;
						LongBridge longbridge = LongBridge.getByIdx(i_164_);
						if (longbridge != null && !longbridge.isAlive())
							longbridge.BeLive();
					}
				}
		...
		int i_165_ = sectfile.sectionIndex("Target");
		if (i_165_ >= 0) {
			int i_166_ = sectfile.vars(i_165_);
			for (int i_167_ = 0; i_167_ < i_166_; i_167_++) {
				SharedTokenizer.set(sectfile.line(i_165_, i_167_));
				int i_168_ = SharedTokenizer.next(0, 0, 7);
				if (i_168_ == 1 || i_168_ == 2 || i_168_ == 6 || i_168_ == 7) {
					SharedTokenizer.next(0);
					SharedTokenizer.next(0);
					SharedTokenizer.next(0);
					SharedTokenizer.next(0);
					int i_169_ = SharedTokenizer.next(0);
					int i_170_ = SharedTokenizer.next(0);
					int i_171_ = SharedTokenizer.next(1000, 50, 3000);
					...
					case 1:
					case 6:
						World.cur().statics.restoreAllHouses((float) i_169_,
								(float) i_170_,
								(float) i_171_);
						break;
					case 2:
	...
	public static void doMissionStarting() {
		...
		for (int i_173_ = 0; i_173_ < i; i_173_++) {
			Actor actor = (Actor) arraylist.get(i_173_);
			if (Actor.isValid(actor)) {
				try {
	...
	private void sendCmd(int i) {
		if (net.isMirrored()) {
			try {
				...
				int i_177_ = list.size();
				for (int i_178_ = 0; i_178_ < i_177_; i_178_++) {
					NetChannel netchannel = (NetChannel) list.get(i_178_);
	...
	private void doReplicateNotMissionActors(boolean bool) {
		if (net.isMirrored()) {
			...
			for (int i_179_ = 0; i_179_ < i; i_179_++) {
				NetChannel netchannel = (NetChannel) list.get(i_179_);
	...
	private void doSendMission(NetChannel netchannel, int i) {
		try {
			NetMsgGuaranted netmsgguaranted = new NetMsgGuaranted();
			netmsgguaranted.writeByte(i);
			int i_180_ = sectFile.sections();
			for (int i_181_ = 0; i_181_ < i_180_; i_181_++) {
				String string = sectFile.sectionName(i_181_);
				if (!string.startsWith("$$$")) {
					...
					int i_182_ = sectFile.vars(i_181_);
					for (int i_183_ = 0; i_183_ < i_182_; i_183_++) {
						...
						netmsgguaranted.writeInt(i_181_);
						netmsgguaranted.write255(sectFile.var(i_181_, i_183_));
						netmsgguaranted.write255(sectFile.value(i_181_,
								i_183_));
					}
				}
			}
	...
	private void netInput(NetMsgInput netmsginput) throws IOException {
		...
		if (net instanceof Master
			...
			bool = true;
		boolean bool_184_ = netmsginput.channel() instanceof NetChannelStream;
		...
		case 0:
			...
			if (bool) {
				if (bool_184_) {
					NetMsgGuaranted netmsgguaranted_185_
					= new NetMsgGuaranted();
					netmsgguaranted_185_.writeByte(13);
					netmsgguaranted_185_.writeLong(Time.current());
					net.postTo(netmsginput.channel(), netmsgguaranted_185_);
				}
				netmsgguaranted.writeByte(0);
				...
				if (!bool_184_ && isExistFile(string)) {
		...
		case 3:
			if (bool) {
				int i_186_ = actors.size();
				int i_187_ = 0;
				while (i_187_ < i_186_) {
					netmsgguaranted = new NetMsgGuaranted();
					netmsgguaranted.writeByte(3);
					int i_188_ = 64;
					while (i_188_-- > 0 && i_187_ < i_186_) {
						Actor actor = (Actor) actors.get(i_187_++);
						if (Actor.isValid(actor))
		...
		case 4:
			if (bool) {
				...
				for (int i_189_ = 0; i_189_ < actors.size(); i_189_++) {
					Actor actor = (Actor) actors.get(i_189_);
					if (Actor.isValid(actor)) {
						try {
								...
								|| (!(netobj instanceof NetControl)
										&& (!(netobj instanceof NetUser)
												...
												&& (!(netobj instanceof GameTrack)
														|| !netobj.isMirror())) {
							Object object_190_ = netobj.superObj();
							if (!(object_190_ instanceof Destroy)
									|| !((Destroy) object_190_).isDestroyed()) {
	...
	private int[] getSwTbl(String string, long l) {
		int i = (int) l;
		int i_191_ = Finger.Int(string);
		if (i < 0)
			i = -i;
		if (i_191_ < 0)
			i_191_ = -i_191_;
		int i_192_ = (i_191_ + i / 7) % 16 + 15;
		int i_193_ = (i_191_ + i / 21) % Finger.kTable.length;
		if (i_192_ < 0)
			i_192_ = -i_192_ % 16;
		if (i_192_ < 10)
			i_192_ = 10;
		if (i_193_ < 0)
			i_193_ = -i_193_ % Finger.kTable.length;
		int[] is = new int[i_192_];
		for (int i_194_ = 0; i_194_ < i_192_; i_194_++)
			is[i_194_]
					= Finger.kTable[(i_193_ + i_194_) % Finger.kTable.length];
		return is;
	}
	...
	private String generateHayrakeCode(Point3d point3d) {
		double d = point3d.x;
		double d_195_ = point3d.y;
		long l = (long) (d + d_195_);
		Random random = new Random(l);
		byte[] is = new byte[12];
		for (int i = 0; i < is.length; i++) {
			boolean bool = false;
			while (!bool) {
				byte i_196_ = (byte) (random.nextInt(26) + 65);
				if (i_196_ != 74 && i_196_ != 81 && i_196_ != 89
						&& i_196_ <= 90) {
					for (int i_197_ = 0;
							i_197_ < is.length && i_196_ != is[i_197_];
							i_197_++) {
						if (i_197_ == is.length - 1) {
							bool = true;
							is[i] = i_196_;
						}
					}
	...
	private void populateRunwayLights() {
		...
		for (int i = 0; i < arraylist.size(); i++) {
			if (arraylist.get(i) instanceof AirportGround) {
				for (int i_198_ = 0; i_198_ < actors.size(); i_198_++) {
					if (actors.get(i_198_) instanceof SmokeGeneric
							&& (actors.get(i_198_) instanceof Smoke.Smoke15
									|| actors.get(i_198_) instanceof Smoke.Smoke14
									|| actors.get(i_198_) instanceof Smoke.Smoke13
									|| actors.get(i_198_) instanceof Smoke.Smoke12)) {
						AirportGround airportground
						= (AirportGround) arraylist.get(i);
						Actor actor = (Actor) actors.get(i_198_);
						double d = (airportground.pos.getAbsPoint().x
								- actor.pos.getAbsPoint().x);
						double d_199_ = (airportground.pos.getAbsPoint().y
								- actor.pos.getAbsPoint().y);
						if (Math.abs(d) < 2000.0 && Math.abs(d_199_) < 2000.0
								&& (actor.getArmy() == 1
								|| actor.getArmy() == 2)) {
	...
	private void populateBeacons() {
		ArrayList arraylist = new ArrayList();
		ArrayList arraylist_200_ = new ArrayList();
		for (int i = 0; i < actors.size(); i++) {
			if (actors.get(i) instanceof TypeHasBeacon) {
				...
				Point3d point3d = ((Actor) actors.get(i)).pos.getAbsPoint();
				arraylist_200_.add(new Object[] { actors.get(i), point3d });
		...
		if (arraylist.size() != 0) {
			...
			for (int i = 0; i < arraylist_200_.size(); i++) {
				Object[] objects = (Object[]) arraylist_200_.get(i);
				Actor actor = (Actor) objects[0];
		...
		if (World.cur().diffCur.RealisticNavigationInstruments) {
			...
			while (iterator.hasNext() && i < 10) {
				...
				if (actor.getArmy() == World.getPlayerArmy()) {
					...
					String string_202_ = Property.stringValue(actor.getClass(),
							"i18nName", "");
						...
						try {
							...
							= (actor.getClass().toString().substring
									(actor.getClass().toString().indexOf("$")
											+ 1));
							string_202_ = I18N.technic(string_203_);
						} catch (Exception exception) {
							/* empty */
						}
					}
					...
					if (beaconsRed.contains(actor))
						i_204_ = beaconsRed.indexOf(actor);
					else if (beaconsBlue.contains(actor))
						i_204_ = beaconsBlue.indexOf(actor);
					if (string.equals("NDB"))
						Main3D.cur3D().ordersTree.addShipIDs(i, i_204_, actor,
								string_202_, "");
					else {
	...
	private void sortBeaconsList(List list) {
		...
		do {
			for (int i = 0; i < list.size() - 1; i++) {
				...
				Object[] objects_206_ = (Object[]) list.get(i + 1);
				if ((objects[0] instanceof TypeHasHayRake
						&& !(objects_206_[0] instanceof TypeHasHayRake))
						|| (objects[0] instanceof BigshipGeneric
								&& !(objects_206_[0] instanceof BigshipGeneric))) {
					Object[] objects_207_ = objects;
					list.set(i, objects_206_);
					list.set(i + 1, objects_207_);
					bool = true;
				}
			}
		} while (bool);
	...
	private void zutiLoadBornPlaceCountries(BornPlace bornplace,
		...
		if (bornplace != null) {
			...
			int i_208_ = sectfile.sectionIndex("BornPlaceCountries" + i);
			if (i_208_ >= 0) {
				...
				= ResourceBundle.getBundle("i18n/country",
						RTSConf.cur.locale,
						LDRres.loader());
				int i_209_ = sectfile.vars(i_208_);
				for (int i_210_ = 0; i_210_ < i_209_; i_210_++) {
					try {
						String string = sectfile.var(i_208_, i_210_);
						String string_211_ = resourcebundle.getString(string);
						if (!bornplace.zutiHomeBaseCountries
								.contains(string_211_))
							bornplace.zutiHomeBaseCountries.add(string_211_);
					} catch (Exception exception) {
						/* empty */
					}
				}
	...
	private void zutiLoadScouts_Red(SectFile sectfile) {
		...
		if (i > -1) {
			...
			int i_212_ = sectfile.vars(i);
			for (int i_213_ = 0; i_213_ < i_212_; i_213_++) {
				String string = sectfile.line(i, i_213_);
				StringTokenizer stringtokenizer = new StringTokenizer(string);
				String string_214_ = null;
				if (stringtokenizer.hasMoreTokens())
					string_214_ = stringtokenizer.nextToken();
				if (string_214_ != null) {
					string_214_ = string_214_.intern();
					Main.cur().mission.ScoutsRed.add(string_214_);
				}
			}
		}
	}
	...
	private void zutiLoadScouts_Blue(SectFile sectfile) {
		...
		if (i > -1) {
			...
			int i_215_ = sectfile.vars(i);
			for (int i_216_ = 0; i_216_ < i_215_; i_216_++) {
				String string = sectfile.line(i, i_216_);
				StringTokenizer stringtokenizer = new StringTokenizer(string);
				String string_217_ = null;
				if (stringtokenizer.hasMoreTokens())
					string_217_ = stringtokenizer.nextToken();
				if (string_217_ != null) {
					string_217_ = string_217_.intern();
					Main.cur().mission.ScoutsBlue.add(string_217_);
				}
			}
		}
	}
	...
	public static int getMissionDate(boolean bool) {
		...
		int i_218_ = curMonth();
		int i_219_ = curDay();
		int i_220_ = i * 10000 + i_218_ * 100 + i_219_;
		if (Main.cur().mission == null) {
			SectFile sectfile = Main.cur().currentMissionFile;
			if (sectfile == null)
				return i_220_;
			String string = sectfile.get("MAIN", "MAP");
			int i_221_ = World.land().config.getDefaultMonth("maps/" + string);
			i = sectfile.get("SEASON", "Year", 1940, 1930, 1960);
			i_218_ = sectfile.get("SEASON", "Month", i_221_, 1, 12);
			i_219_ = sectfile.get("SEASON", "Day", 15, 1, 31);
			i_220_ = i * 10000 + i_218_ * 100 + i_219_;
			int i_222_ = 19400000 + i_221_ * 100 + 15;
			if (bool && i_220_ == i_222_)
				i_220_ = 0;
		} else if (bool) {
			SectFile sectfile = Main.cur().currentMissionFile;
			if (sectfile == null)
				return 0;
			String string = sectfile.get("MAIN", "MAP");
			int i_223_ = World.land().config.getDefaultMonth("maps/" + string);
			int i_224_ = 19400000 + i_223_ * 100 + 15;
			if (i_220_ == i_224_)
				i_220_ = 0;
		}
		return i_220_;
