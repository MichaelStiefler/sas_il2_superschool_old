...
public class Maneuver extends AIFlightModel
{
    ...
    public void update(float f)
    {
        ...
        if(showFM)
            OutCT(20);
        super.update(f);
        float rolllimitbyspeedhight = 60F;
        ...
        if(CT.bHasAirBrakeControl &&
           ((actor instanceof TypeFastJet) || (actor instanceof TypeSupersonic) || EI.engines[0].getType() == 2))
        {
        ...
        case 77: // 'M' TURN_HARD
            ...
            case 0: // '\0'
                ...
                rolllimitbyspeedhight = 36F + (getSpeed() / Vmin - 1F) * 20F + (Alt - 100F) / 10F;
                if(rolllimitbyspeedhight > 72F)
                    rolllimitbyspeedhight = 72F;
                if(World.Rnd().nextInt(0, 100) > 50)
                    bankAngle = rolllimitbyspeedhight;
                else
                    bankAngle = 360F - rolllimitbyspeedhight;
                submaneuver++;
                break;
        ...
        case 4: // '\004' DEFENCE
            ...
            CT.RudderControl = 0.0F;
            if((((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && getSpeedKMH() < 300F)
              || ((!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && getSpeedKMH() < 220F))
            {
                push(3);
                pop();
            }
        ...
        case 2: // '\002' PULL_UP
            ...
            // DBW AI MOD - add Jet
            if(Vwld.z > ((EI.engines[0].getType() == 2)? 2.5D : 1.0D))
            {
            ...
            if((EI.engines[0].getType() != 2 && mn_time > 25F) ||
               (EI.engines[0].getType() == 2 && mn_time > 60F))
            {
                push(49);
                pop();
            }
        ...
        case 3: // '\003' LEVEL_PLANE
            ...
            if((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet))
                dA = (getSpeedKMH() - 250F - Or.getTangage() * 10F - getVertSpeed() * 5F) * 0.004F;
            else
        ...
        case 11: // '\013' DIVING_0_RPM
            ...
            if(Math.abs(Or.getKren()) < 90F)
            {
                CT.AileronControl = -0.04F * Or.getKren();
                if(Vwld.z > 0.0D ||
                   (((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && getSpeedKMH() < 330F) ||
                   ((!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && getSpeedKMH() < 270F))
                    dA = -0.04F;
                else
                    dA = 0.04F;
        ...
        case 16: // '\020' LOOP
            if(first)
            {
                ...
                AP.setStabAll(false);
                setSpeedMode(6);
                //TODO: DBW AI MOD - too big Vmax like TypeFastJet or TypeSuperSonic
                if(getSpeed() < Math.min(0.75F * Vmax , 220F))
                {
                    push();
                    push(22);
                    pop();
            ...
            case 2: // '\002'
                if((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet))
                    CT.RudderControl = -0.1F * getAOS() * (getSpeed() > 500F ? 1.0F : 0.0F);
                else
        ...
        case 18: // '\022' HALF_LOOP_UP
            if(first)
            {
                if(!isCapableOfACM())
                    pop();
                if((((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && getSpeed() < 250F)
                   || (!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && getSpeed() < Vmax * 0.75F)
                {
                    push();
                    push(22);
                    pop();
        ...
        case 15: // '\017' MIL_TURN
            if(first && getSpeed() < 0.35F * (Vmin +
               ((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)? 300F : Vmax)))
            {
                pop();
            } else
            {
        ...
        case 21: // '\025' WAYPOINT
            ...
            if((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet))
            {
                if(getAltitude() < AP.way.curr().z() - 80F && !AP.way.isLanding())
                    CT.ElevatorControl += 0.015F;
                if(getAltitude() < AP.way.curr().z() - 50F && Or.getTangage() < -2F && !AP.way.isLanding())
                    CT.ElevatorControl += 0.020F;
                if(getAltitude() < AP.way.curr().z() - 30F && getVertSpeed() < -1F && !AP.way.isLanding())
                    CT.ElevatorControl += 0.015F;
                if(getAltitude() < AP.way.curr().z() - 10F && Or.getTangage() < -5F && AP.way.isLanding() && AP.way.Cur() < 3)
                    CT.ElevatorControl += 0.020F;
                if(getAltitude() < AP.way.curr().z() - 10F && getVertSpeed() < -12F && AP.way.isLanding() && AP.way.Cur() < 4)
                    CT.ElevatorControl += 0.020F;
                if(getAltitude() < AP.way.curr().z() - 100F && AP.way.isLanding() && AP.way.Cur() < 2)
                    CT.ElevatorControl += 0.020F;
                if(getVertSpeed() < 0 && (float)Alt / -getVertSpeed() < 60F && getAOA() < 15F && !(AP.way.isLanding() && AP.way.Cur() > 2))
                    CT.ElevatorControl += 0.040F;
            }
            else if(EI.engines[0].getType() == 2)
            {
                if(getAltitude() < AP.way.curr().z() - 80F && !AP.way.isLanding())
                    CT.ElevatorControl += 0.015F;
                if(getAltitude() < AP.way.curr().z() - 50F && Or.getTangage() < -2F && !AP.way.isLanding())
                ...
                if(getAltitude() < AP.way.curr().z() - 30F && getVertSpeed() < -1F && !AP.way.isLanding())
                    CT.ElevatorControl += 0.015F;
                if(getAltitude() < AP.way.curr().z() - 10F && Or.getTangage() < -4F && AP.way.isLanding() && AP.way.Cur() < 3)
                    CT.ElevatorControl += 0.020F;
                if(getAltitude() < AP.way.curr().z() - 10F && getVertSpeed() < -10F && AP.way.isLanding() && AP.way.Cur() < 4)
                    CT.ElevatorControl += 0.020F;
                if(getAltitude() < AP.way.curr().z() - 60F && AP.way.isLanding() && AP.way.Cur() < 2)
                    CT.ElevatorControl += 0.020F;
                if(getVertSpeed() < 0 && (float)Alt / -getVertSpeed() < 60F && getAOA() < 15F && !(AP.way.isLanding() && AP.way.Cur() > 2))
                    CT.ElevatorControl += 0.040F;
            }
            ...
            if(CT.ElevatorControl < -1.0F)
                CT.ElevatorControl = -1.0F;
            if(CT.ElevatorControl > 0.02F && Or.getTangage() - oldTangage < -0.06F && Math.abs(Or.getRoll() - 360F) < 10F)
                if(CT.bHasElevatorTrim && CT.getTrimElevatorControl() < 0.55F)
                    CT.setTrimElevatorControl(CT.getTrimElevatorControl() + 0.05F);
            if(CT.ElevatorControl < -0.02F && Or.getTangage() - oldTangage > 0.06F && Math.abs(Or.getRoll() - 360F) < 10F)
                if(CT.bHasElevatorTrim && CT.getTrimElevatorControl() > -0.55F)
                    CT.setTrimElevatorControl(CT.getTrimElevatorControl() - 0.05F);
            oldTangage = Or.getTangage();
            if(mn_time > 300F)
                pop();
            ...
            if(((Aircraft)actor).aircIndex() == 0 && !isReadyToReturn())
            {
                ...
                if(World.getPlayerAircraft() != null)
                {
                    ...
                    if(((Aircraft)actor).getRegiment() == World.getPlayerAircraft().getRegiment())
                    {
                        ...
                        //TODO: DBW AI MOD TypeSupersonic, TypeFastJet
                        if(Speak5minutes == 0 && (
                           ((actor instanceof TypeFastJet) && 33000F < f17 && f17 < 45000F) ||
                           (!(actor instanceof TypeFastJet) && 22000F < f17 && f17 < 30000F)))
                        {
                            Voice.speak5minutes((Aircraft)actor);
                            Speak5minutes = 1;
                        }
                        if(Speak1minute == 0 && (
                           ((actor instanceof TypeFastJet) && f17 < 15000F) ||
                           (!(actor instanceof TypeFastJet) && f17 < 10000F)))
                        {
        ...
        case 99: // 'c' SEEK_AND_DESTROY
            ...
            } else
            {
                ...
                case 1: // '\001'
                    ...
                    //DBW AI MOD: original "if(submanDelay > 5000)" = probably it's a bug and fix
                    if(submanDelay > 500)
                        submanDelay = 500;
                    submaneuver++;
                    break;
        ...
        case 25: // '\031' LANDING
            ...
            if(AP.way.isLanding())
            {
                ...
                if(flag3)
                {
                    ...
                    tmpV3f.sub(Po, Pc);
                    tmpV3f.normalize();
                    if(tmpV3f.dot(Ve) < 0.0D && f29 > 1000F && !TaxiMode && !Gears.onGround() && !AP.way.isLandingOnShip())
                    {
                        AP.way.first();
                        push(10);
                        pop();
                    ...
                    tmpV3f.sub(Loc);
                    float f39;
                  /*  if(actor instanceof TypeFastJet)
                         f39 = 0.0005F * (7500F - f29);
                    else */
                         f39 = 0.0005F * (3000F - f29);
                    if(f39 > 1.0F)
                        f39 = 1.0F;
                    if(f39 < 0.1F)
                        f39 = 0.1F;
                    ...
                    if(Or.getTangage() > gp + 4.0F)
                        ...
                        of50 = f50;
                    tmpOr.increment(0.0F, 4F + f50, (float)(tmpV3f.y * 0.5D)); // AAAAA
                    Or.interpolate(tmpOr, 0.5F * f);
                    callSuperUpdate = false;
                    W.set(0.0D, 0.0D, 0.0D);
                    float newelev = 0.05F + 0.3F * f50;
                    gp = Gears.Pitch;
                    if(gp > 180F)
                        gp -= 360F;
                    if(gp > 7.0F)
                    ...
                    // stock : CT.RudderControl = (float)(-tmpV3f.y * 0.02D);
                    CT.RudderControl = (float)(tmpV3f.y * 0.016D);
          ...
          /*  if(!Gears.onGround() && !flag3)
            ...
            oldTangage = Or.getTangage();
            if(Alt > ((EI.engines[0].getType() == 2)? 160F : 60F))
            {
                if((Alt < ((EI.engines[0].getType() == 2)? 320F : 160F)) ||
            ...
            } else
            {
                ...
                if(Vrel.length() < Math.min(18F, (double)(VmaxFLAPS * ((actor instanceof TypeFastJet)? 0.18F : 0.25F))) && wayCurPos == null && !AP.way.isLandingOnShip())
                {
                    TaxiMode = true;
                    AP.way.setCur(0);
                    return;
                ...
                if(f24 < 3F)
                    f24 = 3F;
                if((Alt < 7F && Or.getTangage() < f24 || AP.way.isLandingOnShip() && Or.getTangage() < f24) && !Gears.onGround())
                {
                    CT.ElevatorControl += (f24 - Or.getTangage()) * 0.333F * f;
                    if(CT.ElevatorControl > 1.0F)
                        CT.ElevatorControl = 1.0F;
                    if(CT.ElevatorControl < -0.2F && (actor instanceof TypeFastJet))
                        CT.ElevatorControl = -0.2F;
                    else if(CT.ElevatorControl < 0.04F)
                        CT.ElevatorControl = 0.04F;
                }
                if((Alt < 7F && Or.getTangage() > f24 + 0.8F) && !Gears.onGround())
                {
                    CT.ElevatorControl += (f24 - Or.getTangage()) * 0.2F * f;
                    if(CT.ElevatorControl > 1.0F)
                        CT.ElevatorControl = 1.0F;
                    if(CT.ElevatorControl < -0.2F && (actor instanceof TypeFastJet))
                        CT.ElevatorControl = -0.2F;
                ...
                if(Gears.onGround())
                {
                    ...
                    if(!TaxiMode)
                    {
                        ...
                        if(((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && !AP.way.isLandingOnShip())
                            if(!((Aircraft)actor).getHasHydraulicPressureSystem() ||
                                (((Aircraft)actor).getHasHydraulicPressureSystem() && ((Aircraft)actor).getHydraulicPressure()))
                                CT.AirBrakeControl = 1.0F;
                    } else
        ...
        case 26: // '\032' TAKEOFF
            ...
            if(Actor.isAlive(AP.way.takeoffAirport) && (AP.way.takeoffAirport instanceof AirportCarrier))
            {
                ...
                {
                    f8 -= ((AirportCarrier)AP.way.takeoffAirport).height();
                ...
                } else
                {
                    if(!bAlreadyCheckedStage7)
                    {
                        if(AP.way.takeoffAirport instanceof AirportCarrier)
                        {
                            ...
                            bHasCatapult = Gears.setCatapultOffset(bigshipgeneric, new SectFile("com/maddox/il2/objects/Catapults.ini"));
                    ...
                    } else
                    {
                        ...
                        if(Actor.isAlive(AP.way.takeoffAirport) && flag6 &&  AP.way.takeoffAirport.takeoffRequest > 0)
                            flag2 = false;
                        if(flag2)
                        {
                            canTakeoff = true;
            ...
            if(EI.engines[0].getStage() == 6 && CT.getPower() > f18)
            {
                ...
                if(Or.getTangage() < f24)
                    dA = -0.7F * (Or.getTangage() - f24) + f27 * (float)((Tuple3d) (getW())).y + 0.5F * (float)((Tuple3d) (getAW())).y;
                else
                    dA = -0.1F * (Or.getTangage() - f24) + f27 * (float)((Tuple3d) (getW())).y + 0.5F * (float)((Tuple3d) (getAW())).y;
            ...
            if(EI.engines[0].getStage() == 6)
            {
                ...
                if(d1 > -1D && d1 < 1.0D)
                {
                    if(flag5 && Actor.isAlive(AP.way.takeoffAirport) && CT.getPower() > 0.3F)
                    {
            ...
            CT.AileronControl = -0.05F * Or.getKren() + 0.3F * (float)((Tuple3d) (getW())).y;
            if(f8 > 5F && !Gears.isUnderDeck())
                CT.GearControl = 0.0F;
            float f28 = 1.0F;
            if(hasBombs() || !flag5)
                f28 *= 1.7F;
            if(f8 > 120F * f28 || getSpeed() > Vmin * 1.8F * f28 || f8 > 80F * f28 && getSpeed() > Vmin * 1.6F * f28 || f8 > 40F * f28 && getSpeed() > Vmin * 1.3F * f28 && mn_time > 60F + (float)((Aircraft)actor).aircIndex() * 3F)
            {
                CT.FlapsControl = 0.0F;
                CT.GearControl = 0.0F;
                rwLoc = null;
        ...
        case 28: // '\034' WAVEOUT
            ...
            rolllimitbyspeedhight = 24F + (getSpeed() / Vmin - 1F) * 16F + (Alt - 60F) / 20F;
            if(rolllimitbyspeedhight > 60F)
                rolllimitbyspeedhight = 60F;
            CT.AileronControl = -0.04F * (Or.getKren() - direction);
            if(Math.abs(Or.getRoll() - 360F) > rolllimitbyspeedhight)
                CT.AileronControl *= 0.8F;
            switch(submaneuver)
            {
            case 0: // '\0'
        ...
        case 29: // '\035' SINUS
            ...
            CT.FlapsControl = 0.2F;
            //TODO: Blown Flaps
            if((((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && (double)getSpeed() < 140D) ||
               ((!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && (double)getSpeed() < 120D))
            {
                CT.FlapsControl = 0.33F;
                if(CT.bHasBlownFlaps)
                	CT.BlownFlapsControl = 1.0F;
            }
            if((((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && (double)getSpeed() < 94D) ||
               ((!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && (double)getSpeed() < 80D))
            {
                CT.FlapsControl = 1.0F;
                if(CT.bHasBlownFlaps)
                	CT.BlownFlapsControl = 1.0F;
        ...
        case 37: // '%' CARREL_POKRYSHKIN
            if(first)
            {
                if(!isCapableOfACM())
                    pop();
                if((((actor instanceof TypeSupersonic) || (actor instanceof TypeFastJet)) && getSpeed() < 150F) ||
                  ((!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet)) && getSpeed() < Vmax * 0.5F))
                {
                    pop();
                    break;
                }
        ...
        case 90: // 'Z' LOOKDOWN_LEFT
            ...
            if(Math.abs(Or.getRoll() - 360F) > rolllimitbyspeedhight) // Stock=60F
            {
                CT.AileronControl *= 0.8F;
            } else
            {
    ...
    {
    	//TODO: DBW AI Mod Edits
        if(!(actor instanceof TypeSupersonic) && !(actor instanceof TypeFastJet) && !(actor instanceof TypeDiveBomber))
            bombsOutCounter = 129;
        else
            bombsOutCounter = 0;
        bombsOut = false;
        ...
        if((actor instanceof TypeSupersonic) && (actor instanceof TypeFastJet))
            ...
            smConstSpeed = 120F;
        float f1 = ((Tuple3d) (Vwld)).z <= 0.0D ? 4F : 3F;
        boolean flag = false;
        boolean flag1 = false;
        if(hasBombs())
        {
        ...
        if(!(actor instanceof TypeStormovik) && !(actor instanceof TypeFighter) && !(actor instanceof TypeDiveBomber))
        {
            set_maneuver(0);
            return;
        }
        ...
        if(flag1)
            if(CT.Weapons[3][0] instanceof BombGunTorp45_36AV_A)
                Ve.z = (((Tuple3d) (Loc)).z - 100D) + World.Rnd().nextDouble() * 50D;
            else
                Ve.z = (((Tuple3d) (Loc)).z - 200D) + World.Rnd().nextDouble() * 50D;
        float f2 = (float)((Tuple3d) (Loc)).z - (float)((Tuple3d) (Ve)).z;
        if(f2 < 0.0F)
            f2 = 0.0F;
        ...
        } else
        {
            ...
            if(flag)
                f4 = (f3 + 5F) * 0.33333F;
            Vtarg.x = (float)((Tuple3d) (tmpV3d)).x * f4 * (float)Skill;
            Vtarg.y = (float)((Tuple3d) (tmpV3d)).y * f4 * (float)Skill;
            Vtarg.z = (float)((Tuple3d) (tmpV3d)).z * f4 * (float)Skill;
            Ve.add(Vtarg);
            Ve.sub(Loc);
            if(flag)
                addWindCorrection();
            ...
            } else
            {
                ...
                if(Math.abs(((Tuple3d) (Ve)).y) > 0.10000000149011612D)
                    CT.AileronControl = -(float)Math.atan2(((Tuple3d) (Ve)).y, ((Tuple3d) (Ve)).z) - 0.016F * Or.getKren();
                else
                    CT.AileronControl = -(float)Math.atan2(((Tuple3d) (Ve)).y, ((Tuple3d) (Ve)).x) - 0.016F * Or.getKren();
                if(Math.abs(((Tuple3d) (Ve)).y) > 0.0010000000474974513D)
                    CT.RudderControl = -98F * (float)Math.atan2(((Tuple3d) (Ve)).y, ((Tuple3d) (Ve)).x);
                else
                    CT.RudderControl = 0.0F;
                if((double)CT.RudderControl * ((Tuple3d) (W)).z > 0.0D)
                    W.z = 0.0D;
                else
                ...
                if(getOverload() > maxG || AOA > f1 || CT.ElevatorControl > f7)
                    CT.ElevatorControl -= 0.2F * f;
                else
                    CT.ElevatorControl += 0.2F * f;
            }
        ...
        } else
        {
          ...
          //TODO: DBW AI Mod Edits
            ...
            if(f1 < 0.0F)
                f1 = 0.0F;
            Ve.z += 250D;
            float f2 = (float)Math.sqrt(((Tuple3d) (Ve)).x * ((Tuple3d) (Ve)).x + ((Tuple3d) (Ve)).y * ((Tuple3d) (Ve)).y) + RandomVal * (float)(2.75D - (double)Skill);
            ...
            Vf.set(Ve);
            CT.BayDoorControl = 1.0F;
            float f3 = (float)((Tuple3d) (Vwld)).z * 0.1019F;
            f3 += (float)Math.sqrt(f3 * f3 + 2.0F * f1 * 0.1019F);
            float f4 = (float)Math.sqrt(((Tuple3d) (Vwld)).x * ((Tuple3d) (Vwld)).x + ((Tuple3d) (Vwld)).y * ((Tuple3d) (Vwld)).y);
            float f5 = f4 * f3 + 10F;
            actor.getSpeed(tmpV3d);
            tmpV3d.scale((double)f3 * 0.45000000000000001D * (double)Skill);
    ...
    private void groundAttackDiveBomber(Actor actor, float f)
    {
        ...
        } else
        {
            ...
            if(f3 > 1000F || submaneuver == 3 && sub_Man_Count > 100)
            {
                ...
                case 1: // '\001'
                    f4 = (f3 - man1Dist) / (float)Vwld.length() + 4F + Alt / 48F;
                    // fall through
            ...
            case 2: // '\002'
                ...
                CT.AileronControl = (float)((double)(-0.04F * f7) - 0.5D * ((Tuple3d) (getW())).x);
                if(getOverload() < 4F)
            ...
            case 3: // '\003'
                ...
                if(sub_Man_Count > 80)
                {
                    float f8 = (float)((Tuple3d) (Vwld)).z * 0.1019F;
                    f8 = f8 + (float)Math.sqrt(f8 * f8 + 2.0F * f5 * 0.1019F) + 0.00035F * f5;
                    float f10 = (float)Math.sqrt(((Tuple3d) (Vwld)).x * ((Tuple3d) (Vwld)).x + ((Tuple3d) (Vwld)).y * ((Tuple3d) (Vwld)).y);
                    float f11 = f10 * f8;
    ...
    private void farTurnToDirection(float f)
    {
        ...
        float f7;
        if((actor instanceof TypeSupersonic) && (actor instanceof TypeFastJet))
            f7 = (getSpeed() / 300F) * 45F;
        else
            f7 = (getSpeed() / Vmax) * 45F;
        if(f7 > 85F)
            f7 = 85F;
        float f8 = (float)Ve.x;
        if(f8 < 0.0F)
    ...
    protected void doCheckGround(float f)
    {
        ...
        if(-Vwld.z * ((actor instanceof TypeFastJet) ? 2.2D : 1.5D) > (double)Alt
           || Alt < ((actor instanceof TypeFastJet) ? 25F : 10F))
        {
    ...
    protected void setSpeedControl(float f)
    {
        ...
        case 3: // '\003'
            ...
            if(getSpeed() > 0)
                if(AP.getWayPointDistance() / getSpeed() * getVertSpeed() < getAltitude() - AP.way.curr().z() - 30F && getVertSpeed() < 0F)
                    f1 -= AP.getWayPointDistance() / getSpeed() * getVertSpeed() / 100F;
            if(f1 > 0.93F)
                f1 = 0.93F;
            if(f1 < 0.35F && !AP.way.isLanding())
                f1 = 0.35F;
            if(f1 > 0.7F && AP.way.isLanding() && AP.way.Cur() > 1)
                f1 = 0.7F;
            if(f1 > 0.6F && AP.way.isLanding() && AP.way.Cur() > 2)
                f1 = 0.6F;
            if(f1 > 0.5F && AP.way.isLanding() && AP.way.Cur() > 4)
                f1 = 0.5F;
            if(AP.way.curr().z() - getAltitude() > 0)
                if(getSpeed() > AP.way.curr().Speed * 1.3F && AP.way.isLanding() && AP.way.Cur() > 1)
                    f1 = 0.0F;
            break;
