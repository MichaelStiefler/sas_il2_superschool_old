...
//TODO: This class is stuffed, needs fixing. Currently causing freezing :S
package com.maddox.il2.fm;
        ...
        case 10:
            ...
            float f24 = engineMoment;
                ...
                if(isPropAngleDeviceOperational())
                {
                    if(w < f24)
                    {
                        f24 = Math.min(1.0F, 0.01F * (f24 - w) - 0.012F * aw);
                        propTarget -= f24 * getPropAngleDeviceSpeed() * f;
                    } else
                    {
                        f24 = Math.min(1.0F, 0.01F * (w - f24) + 0.012F * aw);
                        propTarget += f24 * getPropAngleDeviceSpeed() * f;
                    }
                    if(stage == 6 && propTarget < propPhiW - 0.12F)
                    {
            ...
            computePropForces(w, 0.0F, 0.0F, propAoA0, 0.0F);
            float f25 = w * _1_wMax;
            float f26 = f25 * pressureExtBar;
            float f27 = f25 * f25;
            float f28 = 1.0F - 0.006F * (Atmosphere.temperature((float)((Tuple3d) (((FlightModelMain) (reference)).Loc)).z) - 290F);
            float f291 = 1.0F - 0.0011F * reference.getSpeed();
            propForce = thrustMax * f26 * f27 * f28 * f291 * getStageMultiplier();
            float f301 = engineMoment - propMoment;
            aw = (f301 / (propI + engineI)) * 1.0F;
            if(aw > 0.0F)
                aw *= engineAcceleration;
            w += aw * f;
            if(w < -wMaxAllowed)
...
label0:
		...
		case 10: // '\010'
           ...
           do
           {
               float f8 = 0.5F * (f5 + f6);
               if (flag)
                   computePropForces (wWEP * propReductor, f, 0.0F, f8, f1);
               else
                   computePropForces (wMax * propReductor, f, 0.0F, f8, f1);
               ...
               {
                   f9 = (float) Math.toDegrees (Math.atan (f / (wMax * propReductor * propr)) + (double) f8);
                   return propForce;
               }
           } while(true);
        case 2: // '\002'
