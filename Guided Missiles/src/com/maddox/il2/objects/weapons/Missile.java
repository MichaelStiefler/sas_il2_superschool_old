...
public class Missile extends Rocket {
		...
		if (this.getFailState() == FAIL_TYPE_IVAN) {
			if (this.ivanTimeLeft < Time.tickLenFs()) {
				if (this.nextRandomFloat() < 0.5F) {
					...
					this.deltaTangage = this.nextRandomFloat(-turnStepMax, turnStepMax);
				} else {
					if (this.nextRandomFloat() < 0.5F) {
					...
					this.deltaAzimuth = this.nextRandomFloat(-turnStepMax, turnStepMax);
				}
				this.ivanTimeLeft = this.nextRandomFloat(IVAN_TIME_MIN, IVAN_TIME_MAX);
			} else {
	...
	private void doStart(float f) {
		...
		Missile.activeMissiles.add(this.myActiveMissile);
	}
	...
	private void getMissileProperties() {
		...
		if (this.nextRandomFloat(0, 100.0F) < failureRate) {
			this.setFailState();
			float randFail = this.nextRandomFloat();
	...
	private float nextRandomFloat() {
		return World.Rnd().nextFloat();
	}

	private float nextRandomFloat(float fMin, float fMax) {
		return World.Rnd().nextFloat(fMin, fMax);
	}

	private int nextRandomInt(int iMin, int iMax) {
		return World.Rnd().nextInt(iMin, iMax);
	}
	...
	private void setFailState() {
		if (this.failState == FAIL_TYPE_NONE) {
			this.failState = this.nextRandomInt(1, FAIL_TYPE_NUMBER);
		}
	}
