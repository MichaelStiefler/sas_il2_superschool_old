...
public class Main3D extends Main {
        ...
        private void drawPointer(int i, double d, int j, int k) {
            ...
            if (d2 >= 0.0D) {
                if (d2 <= d1) {
                    ...
                    d4 = k;
                    d3 = Math.tan(1.5707963267948966D - d2) * k;
                } else {
                    d3 = -j;
                    d4 = -Math.tan(d2) * j;
                }
            ...
            d3 += j;
            d4 += k;
            HUD.addPointer((float) d3, (float) d4, Army.color(i), (float) (((1.0D - (d / Main3D.this.iconDrawMax)) * 0.80000000000000004D) + 0.20000000000000001D), (float) d2);
        }
            ...
            switch (Main3D.this.iconTypes()) {
                ...
                case 2: // '\002'
                    if (d <= dotrange.id()) {
                        ...
                        if (World.cur().isArcade() && !Mission.isNet()) {
                            Aircraft aircraft = (Aircraft) actor;
                            if (aircraft.FM instanceof Maneuver) {
                                Maneuver maneuver = (Maneuver) aircraft.FM;
                                int i3 = (int) (aircraft.FM.CT.PowerControl * 100F);
                                int j3 = (int) (aircraft.FM.getOverload() * 10F);
                                float f2 = j3 / 10F;
                                s1 = s1 + " T:" + i3 + "% G:" + f2 + " " + ManString.tname(maneuver.get_task()) + "" + ManString.name(maneuver.get_maneuver());
                            }
                        }
                    }
    ...
    public class RenderMap2D extends Render {
        ...
        public RenderMap2D(Renders renders, float f) {
            ...
            this.setClearColor(GWindowLookAndFeel.getUIBackgroundColor());
        }
    }
    ...
    public Main3D() {
        ...
        this.tmpOr = new Orient(0.0F, 0.0F, 0.0F);
        this.taxingHelperMesh = null;
        this.viewMirror = 2;
        this.iconTypes = 3;
    ...
    private void drawTaxingPath() {
        ...
        if (this.taxingHelperMesh == null) {
            this.taxingHelperMesh = new Mesh("3DO/Effects/TaxingHelper/mono.sim");
        }
        for (int i = 0; i < (Aircraft.playerTaxingWay.size() - 1); i++) {
            ...
            this.tmpVector.sub(point3d1, point3d);
            float f = (float) this.tmpVector.length();
            this.taxingHelperMesh.setScaleXYZ(f, 1.0F, 1.0F);
            this.tmpVector.normalize();
            this.tmpOr.setAT0(this.tmpVector);
            this.taxingHelperMesh.setPos(point3d, this.tmpOr);
            int j = this.taxingHelperMesh.materialFind("Helper");
            if (j != -1) {
                Mat mat = this.taxingHelperMesh.material(j);
                mat.setLayer(0);
                mat.set((byte) 13, f * 0.1F);
                mat.set((byte) 11, Aircraft.taxHlpX);
            }
            this.taxingHelperMesh.render();
        }
    ...
    public void setViewInside() {
        ...
        if (World.Sun().needsCubeUpdate((float) this.camera3D.pos.getAbsPoint().z)) {
    ...
    public void setView(Actor actor, boolean flag) {
        if ((this.viewActor() != actor) || flag || this.bViewFly || this.bViewEnemy) {
            boolean flag1 = this.viewActor() != actor;
            this.endViewFly();
            this.endViewEnemy();
            this.endViewInside();
            Selector.resetGame();
            ...
            this.camera3D.pos.resetAsBase();
            if (flag1 && World.Sun().needsCubeUpdate((float) this.camera3D.pos.getAbsPoint().z)) {
                World.land().cubeFullUpdate();
            }
        }
    }
    ...
    public boolean beginApp(GLContext glcontext, int i) {
        Reflection.setBoolean(World.cur(), "bHakenAllowed", Config.cur.ini.get("game", "HakenAllowed", 1) != 0);
        ...
        SFS.mount("sas_basic_60.sfs");
        try {
            SFS.mount("sas_basic_60_light.sfs");
    ...
    public String startPlayRecordedMission() {
        ...
        } else if (!this.keyRecord.startPlay(this.playRecordedSect, this.playRecorderIndx, 12, this.playRecordedMissionCallback)) {
    ...
    public boolean saveRecordedMission(String s) {
        ...
        try {
            ...
            sectfile.lineAdd(i, "137", "");
            long l = Finger.incLong(this.mission.finger(), 137);
            long l1 = World.cur().diffCur.get();
            sectfile.lineAdd(i, "" + l1, "");
            l = Finger.incLong(this.mission.finger(), l1);
    ...
    public boolean saveRecordedStates2(InOutStreams inoutstreams) {
        try {
            ...
            if (Actor.isValid(this.cockpitCur)) {
                float f = this.cockpitCur.getReticleBrightness();
                byte byte0 = 9;
                int j = (int) (f / 0.2F);
            ...
            if (Actor.isValid(this.cockpitCur)) {
                ...
                printwriter.println((int) this.cockpitCur.aircraft().headingBug);
            }
            printwriter.flush();
            printwriter.close();
            return true;
    ...
    public void loadRecordedStates2() {
        try {
            ...
            if (Actor.isValid(this.cockpitCur)) {
                this.cockpitCur.doToggleAim(false);
                boolean flag = (i & 4) != 0;
                this.cockpitCur.doToggleAim(flag);
            }
            FlightModel flightmodel = World.getPlayerFM();
            if (flightmodel != null) {
            ...
            if (Actor.isValid(this.cockpitCur)) {
                ...
                if ((i & 0x200) != 0) {
                    this.cockpitCur.setReticleBrightness(0.0F);
                } else if ((i & 0x400) != 0) {
                    this.cockpitCur.setReticleBrightness(0.2F);
                } else if ((i & 0x800) != 0) {
                    this.cockpitCur.setReticleBrightness(0.4F);
                } else if ((i & 0x1000) != 0) {
                    this.cockpitCur.setReticleBrightness(0.6F);
                } else if ((i & 0x2000) != 0) {
                    this.cockpitCur.setReticleBrightness(0.8F);
                } else if ((i & 0x4000) != 0) {
                    this.cockpitCur.setReticleBrightness(1.0F);
                } else if ((i & 0x8000) != 0) {
                    this.cockpitCur.setReticleBrightness(1.2F);
                } else if ((i & 0x10000) != 0) {
                    this.cockpitCur.setReticleBrightness(1.4F);
                }
                if ((i & 0x20000) != 0) {
                    int j = Integer.parseInt(bufferedreader.readLine());
                    this.cockpitCur.aircraft().headingBug = j;
                }
            }
            inputstream.close();
        } catch (Exception exception) {
    ...
    public void reloadClearColor() {
        this.render3D0.setClearColor(GWindowLookAndFeel.getUIBackgroundColor());
        this.renderMap2D.setClearColor(GWindowLookAndFeel.getUIBackgroundColor());
    }
    ...
    protected void drawFarActors() {
        ...
        if (this.iconFarPadlockItem.str != null) {
            if (!this.iconFarPadlockItem.bGround) {
                ...
                float f1 = 16F;
                this.line3XYZ[0] = (float) (this.iconFarPadlockItem.x - (f1 * 0.86599999999999999D));
                this.line3XYZ[1] = (float) (this.iconFarPadlockItem.y - (f1 * 0.5D));
                this.line3XYZ[2] = this.iconFarPadlockItem.z;
                this.line3XYZ[3] = (float) (this.iconFarPadlockItem.x + (f1 * 0.86599999999999999D));
                ...
                double d = ((-this._lineO.getKren()) * 3.1415926535897931D) / 180D;
    ...
    private void drawRandomSplashScreen() {
        if (SPLASH_TEST) {
            this.drawSplashScreenTest();
            return;
        }
        float f = (float) RendersMain.getViewPortWidth() / (float) RendersMain.getViewPortHeight();
        String splashscreen_section = "normal";
        if (f > 1.5F) {
            splashscreen_section = "wide";
        }

        SectFile sectFileSplashScreens = new SectFile("GUI/splashscreen/splashscreens.ini");
        int splashScreenSectionIndex = sectFileSplashScreens.sectionIndex(splashscreen_section);
        ArrayList splashScreenFileNameArray = null;
        int numVars = 0;
        if (splashScreenSectionIndex >= 0) {
            numVars = sectFileSplashScreens.vars(splashScreenSectionIndex);
            splashScreenFileNameArray = new ArrayList(numVars);
            for (int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVars; splashScreenFileNameIndex++) {
                String splashScreenFileName = sectFileSplashScreens.line(splashScreenSectionIndex, splashScreenFileNameIndex).trim();
                if (splashScreenFileName.length() < 4) {
                    continue;
                }
                if (!splashScreenFileName.endsWith(".jpg") && !splashScreenFileName.endsWith(".jpeg")) {
                    continue;
                }
                splashScreenFileNameArray.add(splashScreenFileName);
            }
        }

        if (splashScreenFileNameArray == null) {
            return;
        }
        numVars = splashScreenFileNameArray.size();
        if (numVars < 1) {
            return;
        }

        RangeRandom rangerandom = new RangeRandom(System.currentTimeMillis());
        String splashScreenFileName = "GUI/splashscreen/" + splashScreenFileNameArray.get(rangerandom.nextInt(0, numVars - 1));
        this.drawSplashScreenFromJpg(splashScreenFileName);
    }

    private void drawSplashScreenTest() {
        SectFile sectFileSplashScreens = new SectFile("GUI/splashscreen/splashscreens.ini");
        ArrayList splashScreenFileNameArray = null;
        int splashScreenSectionIndexWide = sectFileSplashScreens.sectionIndex("wide");
        int splashScreenSectionIndexNormal = sectFileSplashScreens.sectionIndex("normal");
        int numVarsWide = sectFileSplashScreens.vars(splashScreenSectionIndexWide);
        int numVarsNormal = sectFileSplashScreens.vars(splashScreenSectionIndexNormal);
        splashScreenFileNameArray = new ArrayList(numVarsWide + numVarsNormal);
        for (int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVarsWide; splashScreenFileNameIndex++) {
            String splashScreenFileName = sectFileSplashScreens.line(splashScreenSectionIndexWide, splashScreenFileNameIndex).trim();
            if (splashScreenFileName.length() < 4) {
                continue;
            }
            if (!splashScreenFileName.endsWith(".jpg") && !splashScreenFileName.endsWith(".jpeg")) {
        ...
        for (int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVarsNormal; splashScreenFileNameIndex++) {
            String splashScreenFileName = sectFileSplashScreens.line(splashScreenSectionIndexNormal, splashScreenFileNameIndex).trim();
            if (splashScreenFileName.length() < 4) {
                continue;
            }
            if (!splashScreenFileName.endsWith(".jpg") && !splashScreenFileName.endsWith(".jpeg")) {
        ...
        int numVars = splashScreenFileNameArray.size();
        if (numVars < 1) {
            return;
        }
        ...
        for (int i = 0; i < numVars; i++) {
            try {
                String splashScreenFileName = "GUI/splashscreen/" + splashScreenFileNameArray.get(i);
                this.drawSplashScreenFromJpg(splashScreenFileName);
                Thread.sleep(100);
        ...
        try {
            this.drawSplashScreenFromJpg("GUI/menu/wide_background_de.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/wide_background_en.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/wide_background_jp.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/wide_background_ru.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/background_de.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/background_en.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/background_jp.jpg");
            Thread.sleep(100);
            this.drawSplashScreenFromJpg("GUI/menu/background_ru.jpg");
            Thread.sleep(100);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    ...
    private void drawSplashScreenFromJpg(String jpgImageFileName) {
        SFSInputStream jpgFile;
        int jpgFileLength;
        byte jpgFileContents[] = null;
        this.debugSplashScreen("Splash screen jpg file: " + jpgImageFileName);
        try {
            ...
            System.out.println("Error reading splash screen file " + jpgImageFileName);
            e.printStackTrace();
        }

        String splashPath = "PaintSchemes/Temp";
        String splashPathWide = "PaintSchemes/Temp";
        float f = (float) RendersMain.getViewPortWidth() / (float) RendersMain.getViewPortHeight();
        if (f > 1.5F) {
            if (splashPathWide.indexOf("/") != -1) {
                String fileNameWidePart1 = splashPathWide.substring(0, splashPathWide.indexOf("/"));
                String fileNameWidePart2 = splashPathWide.substring(splashPathWide.indexOf("/") + 1);
                splashPathWide = fileNameWidePart1 + "/wide_" + fileNameWidePart2;
            }
        }
        this.debugSplashScreen("Creating splash screen TGA file: " + splashPathWide + "/splash.tga");
        int createSplashResult = createSplash(jpgFileContents, splashPathWide + "/splash.tga");
        ...
        if (createSplashResult == 0) {
            try {
                ...
                pw.println("[Layer0]");
                pw.println("  TextureName splash.tga");
            ...
            ConsoleGL0.exclusiveDraw(splashPath + "/splash.mat");
        }
    }
    ...
    private void debugSplashScreen(String theLine) {
        if (DEBUG_SPLASH) {
            System.out.println(theLine);
        }
    }

    private static final boolean SPLASH_TEST            = false;
    private static final boolean DEBUG_SPLASH           = true;
    ...
    // TODO: SAS.dll Helper Library
    public static final native int createSplash(byte[] jpgFileContents, String tgaFile);
