...
public class CockpitTaube_PIL extends CockpitPilot {
    ...
    class Interpolater extends InterpolateRef {
        ...
        public boolean tick() {
            if (CockpitTaube_PIL.this.fm != null) {
                if (CockpitTaube_PIL.this.bNeedSetUp) {
                    CockpitTaube_PIL.this.reflectPlaneMats();
                    CockpitTaube_PIL.this.bNeedSetUp = false;
                }
                CockpitTaube_PIL.this.setTmp = CockpitTaube_PIL.this.setOld;
                CockpitTaube_PIL.this.setOld = CockpitTaube_PIL.this.setNew;
                CockpitTaube_PIL.this.setNew = CockpitTaube_PIL.this.setTmp;
                CockpitTaube_PIL.this.setNew.throttle = ((10F * CockpitTaube_PIL.this.setOld.throttle) + CockpitTaube_PIL.this.fm.CT.PowerControl) / 11F;
                CockpitTaube_PIL.this.setNew.prop = ((10F * CockpitTaube_PIL.this.setOld.prop) + CockpitTaube_PIL.this.fm.EI.engines[0].getControlProp()) / 11F;
                CockpitTaube_PIL.this.setNew.altimeter = CockpitTaube_PIL.this.fm.getAltitude();
                if (Math.abs(CockpitTaube_PIL.this.fm.Or.getKren()) < 30F) {
                    CockpitTaube_PIL.this.setNew.azimuth = ((35F * CockpitTaube_PIL.this.setOld.azimuth) + -CockpitTaube_PIL.this.fm.Or.getYaw()) / 36F;
                }
                if ((CockpitTaube_PIL.this.setOld.azimuth > 270F) && (CockpitTaube_PIL.this.setNew.azimuth < 90F)) {
                    CockpitTaube_PIL.this.setOld.azimuth -= 360F;
                }
                if ((CockpitTaube_PIL.this.setOld.azimuth < 90F) && (CockpitTaube_PIL.this.setNew.azimuth > 270F)) {
                    CockpitTaube_PIL.this.setOld.azimuth += 360F;
                }
                CockpitTaube_PIL.this.setNew.waypointAzimuth = ((10F * CockpitTaube_PIL.this.setOld.waypointAzimuth) + (CockpitTaube_PIL.this.waypointAzimuth() - CockpitTaube_PIL.this.setOld.azimuth) + World.Rnd().nextFloat(-10F, 10F)) / 11F;
                CockpitTaube_PIL.this.setNew.vspeed = ((199F * CockpitTaube_PIL.this.setOld.vspeed) + CockpitTaube_PIL.this.fm.getVertSpeed()) / 200F;
                boolean flag = false;
                if (CockpitTaube_PIL.this.setNew.gCrankAngle < (CockpitTaube_PIL.this.fm.CT.getGear() - 0.005F)) {
                    if (Math.abs(CockpitTaube_PIL.this.setNew.gCrankAngle - CockpitTaube_PIL.this.fm.CT.getGear()) < 0.33F) {
                        CockpitTaube_PIL.this.setNew.gCrankAngle += 0.0025F;
                        flag = true;
                    } else {
                        CockpitTaube_PIL.this.setNew.gCrankAngle = CockpitTaube_PIL.this.fm.CT.getGear();
                        CockpitTaube_PIL.this.setOld.gCrankAngle = CockpitTaube_PIL.this.fm.CT.getGear();
                    }
                }
                if (CockpitTaube_PIL.this.setNew.gCrankAngle > (CockpitTaube_PIL.this.fm.CT.getGear() + 0.005F)) {
                    if (Math.abs(CockpitTaube_PIL.this.setNew.gCrankAngle - CockpitTaube_PIL.this.fm.CT.getGear()) < 0.33F) {
                ...
                if (flag != this.sfxPlaying) {
                    if (flag) {
                        CockpitTaube_PIL.this.sfxStart(16);
                    } else {
                        CockpitTaube_PIL.this.sfxStop(16);
                    }
    ...
    public void reflectWorldToInstruments(float f) {
        ...
        this.mesh.chunkSetAngles("zGas1a", 0.0F, this.floatindex(this.cvt(this.fm.M.fuel, 0.0F, 190F, 0.0F, 14F), CockpitTaube_PIL.fuelQuantityScale), 0.0F);
        this.mesh.chunkSetAngles("zSpeed1a", 0.0F, this.floatindex(this.cvt(Pitot.Indicator((float) this.fm.Loc.z, this.fm.getSpeedKMH()), 0.0F, 550F, 0.0F, 11F), CockpitTaube_PIL.speedometerScale), 0.0F);
        ...
        if (((this.fm.AS.astateCockpitState & 8) == 0) && ((this.fm.AS.astateCockpitState & 0x20) == 0)) {
            this.mesh.chunkSetAngles("zRPS1a", 0.0F, this.floatindex(this.cvt(this.fm.EI.engines[0].getRPM(), 0.0F, 2400F, 0.0F, 13F), CockpitTaube_PIL.engineRPMScale), 0.0F);
