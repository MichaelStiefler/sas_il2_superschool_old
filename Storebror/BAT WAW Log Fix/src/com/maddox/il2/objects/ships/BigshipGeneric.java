    ...
    implements MsgCollisionRequestListener, MsgCollisionListener, MsgExplosionListener, MsgShotListener, Predator, ActorAlign, ShipHunterInterface, VisibilityLong
{
        ...
        implements ActorSpawn
    {
        ...
        {
//            System.out.println("BigshipGeneric SPAWN getS(" + sectfile.fileName() + ", " + s + ", " + s1 + ")");
...
//                Class class1;
        ...
        {
//            System.out.println("BigshipGeneric.SPAWN(" + class1.getName() + ")");
            try
            {
...
//                Object obj3 = null;
  ...
  public void requestTowAircraft(Aircraft aircraft)
  {
    ...
    Point3d[] p3d_arr = this.prop.propAirport.towPRel;
    Point3d p3d1 = new Point3d();
    Point3d p3d2 = new Point3d();
    Point3d p3d3 = new Point3d();
    Point3d p3d4 = new Point3d();
    Loc loc1 = new Loc();
    Loc loc2 = new Loc();
    for (int i = 0; i < p3d_arr.length / 2; i++)
    {
      this.pos.getCurrent(loc1);
      p3d3.set(p3d_arr[(i + i)]);
      loc1.transform(p3d3);
      p3d4.set(p3d_arr[(i + i + 1)]);
      loc1.transform(p3d4);
      aircraft.pos.getCurrent(loc2);
      loc1.set(0.0D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
      hookNamed.computePos(aircraft, loc2, loc1);
      p3d1.set(loc1.getPoint());
      aircraft.pos.getPrev(loc2);
      loc1.set(0.0D, 0.0D, 0.0D, 0.0F, 0.0F, 0.0F);
      hookNamed.computePos(aircraft, loc2, loc1);
      p3d2.set(loc1.getPoint());
      if (p3d2.z < p3d3.z + 0.5D * (p3d4.z - p3d3.z) + 0.2D)
      {
        Line2d l2d1 = new Line2d(new Point2d(p3d3.x, p3d3.y), new Point2d(p3d4.x, p3d4.y));
        Line2d l2d2 = new Line2d(new Point2d(p3d1.x, p3d1.y), new Point2d(p3d2.x, p3d2.y));
        try
        {
          Point2d p2d = l2d1.crossPRE(l2d2);
          double d1 = Math.min(p3d3.x, p3d4.x);
          double d2 = Math.max(p3d3.x, p3d4.x);
          double d3 = Math.min(p3d3.y, p3d4.y);
          double d4 = Math.max(p3d3.y, p3d4.y);
          if ((p2d.x <= d1) || (p2d.x >= d2) || (p2d.y <= d3) || (p2d.y < d4))
          {
            d1 = Math.min(p3d1.x, p3d2.x);
            d2 = Math.max(p3d1.x, p3d2.x);
            d3 = Math.min(p3d1.y, p3d2.y);
            d4 = Math.max(p3d1.y, p3d2.y);
            if ((p2d.x > d1) && (p2d.x < d2) && (p2d.y > d3) && (p2d.y < d4))
            {
              this.towPortNum = i;
