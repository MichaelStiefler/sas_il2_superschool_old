...
public abstract class HE_177_TD extends Scheme2 implements TypeBomber, TypeTransport {
	...
	public static void moveGear(HierMesh hiermesh, float f, float f1, float f2) {
		...
		if (f > 0.8F) {
			f3 = Aircraft.cvt(f, 0.8F, 1.0F, -90F, 0.0F);
		}
		if (f1 > 0.8F) {
			f4 = Aircraft.cvt(f1, 0.8F, 1.0F, -90F, 0.0F);
		}
	...
	public void rareAction(float f, boolean flag) {
		...
		if (flag) {
			if (this.FM.AS.astateEngineStates[0] > 3 && World.Rnd().nextFloat() < 0.09F) {
				this.FM.AS.hitTank(this, 1, 1);
			}
			if (this.FM.AS.astateEngineStates[0] > 3 && World.Rnd().nextFloat() < 0.09F) {
				this.FM.AS.hitTank(this, 6, 1);
			}
			if (this.FM.AS.astateEngineStates[1] > 3 && World.Rnd().nextFloat() < 0.09F) {
				this.FM.AS.hitTank(this, 2, 1);
			}
			if (this.FM.AS.astateEngineStates[1] > 3 && World.Rnd().nextFloat() < 0.09F) {
				this.FM.AS.hitTank(this, 7, 1);
			}
...
//			if (this.FM.AS.astateTankStates[4] > 5 && World.Rnd().nextFloat() < 0.02F) {
//				this.nextDMGLevel(this.FM.AS.astateEffectChunks[4] + "0", 0, this);
//			}
//			if (this.FM.AS.astateTankStates[5] > 5 && World.Rnd().nextFloat() < 0.02F) {
//				this.nextDMGLevel(this.FM.AS.astateEffectChunks[5] + "0", 0, this);
//			}
//			if (this.FM.AS.astateTankStates[6] > 5 && World.Rnd().nextFloat() < 0.02F) {
//				this.nextDMGLevel(this.FM.AS.astateEffectChunks[6] + "0", 0, this);
//			}
//			if (this.FM.AS.astateTankStates[7] > 5 && World.Rnd().nextFloat() < 0.02F) {
//				this.nextDMGLevel(this.FM.AS.astateEffectChunks[7] + "0", 0, this);
//			}
		}
		for (int i = 1; i < 7; i++)
			if (i < 3 || i == 4) {
				if (this.FM.getAltitude() < 3000F) {
					...
					this.hierMesh().chunkVisible("HMask" + i + "_D0",
							this.hierMesh().isChunkVisible("Pilot" + i + "_FAK"));
				}
			} else if (this.FM.getAltitude() < 3000F) {
				this.hierMesh().chunkVisible("HMask" + i + "_D0", false);
			} else {
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			...
			if (s.startsWith("xxspar")) {
				if ((s.endsWith("cf1") || s.endsWith("cf2")) && World.Rnd().nextFloat() < 0.1F
						&& this.chunkDamageVisible("CF") > 2
				...
				if ((s.endsWith("ta1") || s.endsWith("ta2")) && World.Rnd().nextFloat() < 0.1F
				...
				if ((s.endsWith("li1") || s.endsWith("li2"))
						&& World.Rnd().nextFloat() < 1.0D - 0.92D * Math.abs(Aircraft.v1.x)
				...
				if ((s.endsWith("lm1") || s.endsWith("lm2"))
						&& World.Rnd().nextFloat() < 1.0D - 0.86D * Math.abs(Aircraft.v1.x)
				...
				if ((s.endsWith("lo1") || s.endsWith("lo2"))
						&& World.Rnd().nextFloat() < 1.0D - 0.79D * Math.abs(Aircraft.v1.x)
				...
				if (s.endsWith("e1") && (point3d.y > 2.79D || point3d.y < 2.32D)
						&& this.getEnergyPastArmor(18F, shot) > 0.0F) {
					this.debuggunnery("*** Engine1 Suspension Broken in Half..");
					this.nextDMGLevels(3, 2, "Engine1_D0", shot.initiator);
				}
				if (s.endsWith("e2") && (point3d.y < -2.79D || point3d.y > -2.32D)
						&& this.getEnergyPastArmor(18F, shot) > 0.0F) {
					this.debuggunnery("*** Engine2 Suspension Broken in Half..");
					this.nextDMGLevels(3, 2, "Engine2_D0", shot.initiator);
				}
			}
			if (s.startsWith("xxbomb") && World.Rnd().nextFloat() < 0.01F && this.FM.CT.Weapons[3] != null
					&& this.FM.CT.Weapons[3][0].haveBullets()) {
				this.debuggunnery("*** Bomb Payload Detonates..");
				if ((this.iRust == 0 || this.iRust == 1 && (s.startsWith("xxbomb2") || s.startsWith("xxbomb3"))
						|| this.iRust == 2 && s.startsWith("xxbomb3")) && this.getEnergyPastArmor(3F, shot) > 0.0F) {
			...
			if (s.startsWith("xxengine")) {
				...
				if (s.endsWith("base")) {
					...
					if (this.getEnergyPastArmor(1.45F, shot) > 0.0F
							&& World.Rnd().nextFloat() < this.FM.EI.engines[k].getCylindersRatio() * 0.5F) {
						this.FM.EI.engines[k].setCyliderKnockOut(shot.initiator,
								World.Rnd().nextInt(1, (int) (shot.power / 4800F)));
						this.debuggunnery("*** Engine" + (k + 1) + " Cylinders Hit, "
								+ this.FM.EI.engines[k].getCylindersOperable() + "/"
								+ this.FM.EI.engines[k].getCylinders() + " Left..");
				...
				} else if (s.endsWith("sup") && this.getEnergyPastArmor(0.05F, shot) > 0.0F
			...
			if (s.startsWith("xxtank")) {
				...
				if (this.getEnergyPastArmor(1.5F, shot) > 0.0F)
					if (shot.power < 14100F) {
						...
						if (shot.powerType == 3 && this.FM.AS.astateTankStates[i1] > 2
		...
		if (s.startsWith("xcf") && this.chunkDamageVisible("CF") < 3) {
	...
	public void CalculateRevi(float f, float f1) {
		...
		if (f4 > -90F && f4 < 90F) {
			d4 = d2 + f3 + f2;
		} else if (Math.abs(d2) < Math.abs(f3 + f2)) {
			d4 = d2 + f3 + f2;
		} else {
			d4 = Math.abs(d2) - (f3 + f2);
		}
		d5 = Math.sqrt(d4 * d4 + d1 * d1);
		d6 = Math.sqrt(d5 * d5 + d3 * d3);
		this.HeadTangage = Geom.RAD2DEG((float) Math
				.atan((f3 + f2) * (float) Math.cos(Geom.DEG2RAD(f4)) * (float) Math.sin(Geom.DEG2RAD(f6)) / d6));
		this.HeadTangage1 = f6 - this.HeadTangage;
		this.HeadYaw1 = Geom.RAD2DEG((float) Math.atan((f3 + f2) * (float) Math.sin(Geom.DEG2RAD(f4)) / d));
		this.HeadYaw = Geom
				.RAD2DEG((float) Math.atan((f3 + f2) * (float) Math.sin(Geom.DEG2RAD(f4)) / this.B1_Distance));
		if (f5 < 0.0F) {
	...
	public void typeBomberUpdate(float f) {
		...
		if (this.fSightCurReadyness < 1.0F) {
			...
			this.calibDistance = this.fSightCurSpeed / 3.6F * AircraftLH
			...
			if (this.fSightCurDistance < this.calibDistance
					+ this.fSightCurSpeed / 3.6F * Math.sqrt(this.fSightCurAltitude * (2F / 9.81F))) {
				this.bSightBombDump = true;
			}
			if (this.bSightBombDump)
				if (this.FM.isTick(3, 0)) {
					if (this.FM.CT.Weapons[3] != null
							&& this.FM.CT.Weapons[3][this.FM.CT.Weapons[3].length
									- 1] != null
							&& this.FM.CT.Weapons[3][this.FM.CT.Weapons[3].length
									- 1].haveBullets()) {
