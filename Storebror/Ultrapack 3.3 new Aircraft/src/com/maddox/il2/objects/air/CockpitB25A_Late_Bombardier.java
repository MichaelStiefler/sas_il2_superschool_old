...
public class CockpitB25A_Late_Bombardier extends CockpitPilot
{
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            ...
            setOld = setNew;
            setNew = setTmp;
            float f = ((B25A_Late)aircraft()).fSightCurForwardAngle;
            float f1 = ((B25A_Late)aircraft()).fSightCurSideslip;
            CockpitB25A_Late_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
            mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitB25A_Late_Bombardier.calibrAngle + f);
            if(bEntered)
            {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitB25A_Late_Bombardier.calibrAngle + f, 0.0F);
            }
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        if(bEntered)
        {
            this.mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((B25A_Late)aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
            boolean flag = ((B25A_Late)aircraft()).fSightCurReadyness > 0.93F;
