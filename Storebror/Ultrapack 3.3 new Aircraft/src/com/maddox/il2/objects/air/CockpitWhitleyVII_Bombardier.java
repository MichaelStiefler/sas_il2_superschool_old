...
public class CockpitWhitleyVII_Bombardier extends CockpitPilot
{
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            float f = ((WhitleyMkVII)aircraft()).fSightCurForwardAngle;
            float f1 = ((WhitleyMkVII)aircraft()).fSightCurSideslip;
            CockpitWhitleyVII_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
            mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitWhitleyVII_Bombardier.calibrAngle + f);
            if(bEntered)
            {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitWhitleyVII_Bombardier.calibrAngle + f, 0.0F);
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        if(bEntered)
        {
            mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((WhitleyMkVII)aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
            boolean flag = ((WhitleyMkVII)aircraft()).fSightCurReadyness > 0.93F;
