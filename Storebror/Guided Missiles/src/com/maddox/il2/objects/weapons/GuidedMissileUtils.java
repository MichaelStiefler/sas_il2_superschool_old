							...
							if (!this.multiTrackingCapable) {
								if (theActiveMissile.theOwner == actor) {
									if (theActiveMissile.theVictim != null) {
										...
										else return theActiveMissile.theVictim;
									}
								}
							}
						}
...
//								theSelectedActorOffset.set(fGC, 0, 0);
	...
	public float Pk(Actor actorFrom, Actor actorTo) {
		...
		fPkRet = 100.0F;
		if ((distanceToTarget > this.fPkMaxDist) || (distanceToTarget < this.fPkMinDist) || (angleToTarget > this.fPkMaxAngle) || (angleFromTarget > this.fPkMaxAngleAft) || (gForce > fMaxLaunchLoad)) return 0.0F;
