                ...
                if (viewAircraft.FM.crew > 0) {
                    if (!viewAircraft.FM.AS.isPilotDead(0)) {
                        ...
                        if (p3dCam.x == 0D) {
                            ...
                            if (p3dCam.y < 0D) angleAzimuth = -180F + (float) Math.toDegrees(Math.atan(p3dCam.y / p3dCam.x));
...
//				String hudTrainingCaption;
				...
				if (k != 0) {
					...
					if (k < 0 && mouseSpeedFactor > 0.1F)
						mouseSpeedFactor /= mouseSpeedFactorMultiplier;
//					float dispMouseSpeedFactor = (int)(mouseSpeedFactor * 100F) / 100F;
//					hudTrainingCaption = "Mouse Factor = " + dispMouseSpeedFactor;
				}
//				} else {
//					hudTrainingCaption = "";
					// hudTrainingCaption = "mouseMove " + i + ", " + j + ", " + k;
					// if (RTSConf.cur.keyboard.isPressed(vkShift))
					// hudTrainingCaption += ", Shift";
					// if (RTSConf.cur.keyboard.isPressed(vkCtrl))
					// hudTrainingCaption += ", Ctrl";
					// if (RTSConf.cur.keyboard.isPressed(vkAlt))
					// hudTrainingCaption += ", Alt";
//				}
//				HUD.training(hudTrainingCaption);

				if (RTSConf.cur.keyboard.isPressed(vkShift)) {
					pCamOffset.set(0.0F, (float)-i * mouseSpeedFactor * 0.1F, (float)j * mouseSpeedFactor * 0.1F);
					// pCam.add(0.0F, (float)i * mouseSpeedFactor, (float)j * mouseSpeedFactor);
					oView.transform(pCamOffset);
					pCam.add(pCamOffset);
				} else if (RTSConf.cur.keyboard.isPressed(vkCtrl)) {
					oCam.set(oCam.azimut(), oCam.tangage(), oCam.kren() + (float)i * koofKren);
					oCam.wrap360();
					...
					oCam.set(oCam.azimut() + (float)i * koofAzimut, oCam.tangage() + (float)j * koofTangage, oCam.kren());
					oCam.wrap360();
				}
			} else {
				// TODO: --- Camera Mod 4.12 ---
    ...
    public void movePilotsHead(Aircraft aircraft, float f, float f1) {
        if (Config.isUSE_RENDER() && (aircraft.netUser() == null || this.headTp < f1 || this.headTm > f1 || this.headYp < f || this.headYm > f)) {
	...
	public void snapSet(float f, float f1) {
		...
		if (camera != null) {
			...
			if (actor != null) {
				...
				if (Azimut > _Azimut)
					for (; Math.abs(Azimut - _Azimut) > 180F; Azimut -= Circ)
						;
				...
				if (Tangage > _Tangage)
					for (; Math.abs(Tangage - _Tangage) > 180F; Tangage -= Circ)
						;
