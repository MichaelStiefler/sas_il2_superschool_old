...
public class FlightModelMain extends FMMath {
	...
	public float getSpeedKMH() {
		return (float)(Vflow.x * 3.6D);
	}

    public float getSpeed2KMH()
    {
        return (float)(Vflow.x * 3.6D * (double)BOMBER_RND);
    }
	...
	public void load(String s) {
		...
		if (Mission.isNet()) {
			...
			if (Config.cur.ini.get("Mods", "ShowOnlineCompatibleFlightModelChanges", 0) == 1) {
				if (CT.bHasArrestorControl != bOnlineArrestor || CT.bHasWingControl != bOnlineWingFold
						|| CT.bHasCockpitDoorControl != bOnlineCockpitDoor || CT.bHasBayDoorControl != bOnlineBombBay) {
					System.out.println("******************************************");
	...
	public FlightModelMain(String s) {
        ...
        BOMBER_RND = tmp_rnd.nextFloat(0.98F, 1.02F);
		...
		try {
			...
			if (obj != null) {
				...
				if (inputstream == null) {
					...
					if (s.endsWith(".emd"))
						System.out
								.println("*** This should have been the reason of the 'Explosion in the Air' when mission started ***");
					else
						System.out.println("*** This was the Reason of your 60% or 70% CTD ***");
				}
			}
			inputstream.mark(0);
			sectfile = new SectFile(new InputStreamReader(new KryptoInputFilter(inputstream, getSwTbl(Finger.Int(s1 + "ogh9"),
					inputstream.available())), "Cp1252"));
			inputstream.reset();
			if (obj == null)
