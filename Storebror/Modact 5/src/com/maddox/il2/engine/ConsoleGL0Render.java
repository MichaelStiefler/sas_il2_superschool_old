...
class ConsoleGL0Render extends Render {
	...
	private void setAIFlyablesCaption() {
		if (this.theAIFlyablesClassChecked) return;
		if (!getAIFlyablesActive()) return;
		...
		if (this.theAIFlyablesClass != null) {
			...
			if (versionAIFlyables.length() > 0) versionAIFlyables = " " + versionAIFlyables;
		}
			...
			if (sstep != null) {
				...
				if ("ru".equalsIgnoreCase(RTSConf.cur.locale.getLanguage())) {
					...
					TTFont.font[2].output(i, (float) RendersMain.getViewPortWidth() * 0.083F, ((float) RendersMain.getViewPortHeight() * 0.12F + (float) TTFont.font[2].height()) - (float) TTFont.font[2].descender(), 0.0F,
							GAME_VERSION + " c SAS ModA\u043a\u0442\u0438\u0432\u0430\u0442\u043e\u0440\u043e\u043c " + MODACT_VERSION + AI_FLYABLES);
				} else {
	...
	private static final String GAME_VERSION = "V 4.12.2m";
	private static final String MODACT_VERSION = "5.30";
