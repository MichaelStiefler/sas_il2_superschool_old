...
public class HUD {
    ...
    public static boolean isAdminMode() {
        ...
        if (NetMissionTrack.isPlaying()) return true;
        return adminMode;
    }
    ...
    static class MsgWaiting
    {
        ...
        MsgWaiting(String s, int i)
        {
            msg = s;
            last = i;
            endTime = Time.current() + last;
            textWidth = 0F;
            StrTokenizer st = new HUD.StrTokenizer(s, "\n");
            while (st.hasMoreTokens()) textWidth = Math.max(textWidth, TTFont.font[2].width(st.nextToken()));
        }
    }
    
    /**
     *  This is a replacement for the standard StringTokenizer which differs
     *  in that if there are multiple delimiters encountered in succession,
     *  nextToken() returns a blank string ("") for each delimiter pair
     *  encountered,  rather than gouping the delimiters together as one,
     *  which is what StringTokenizer does.  Thus the string "A,B,,,E"
     *  breaks into tokens "A", "B", "", "", "E", rather than "A", "B", "E".
     */
    public static class StrTokenizer {
        ...
        public StrTokenizer(String str, String delimStr) {
            this.str = str;
    ...
    private void renderLog() {
        ...
        try {
            if(msgWaitingList.size() > 0) {
                ...
                Iterator it = msgWaitingList.iterator();
                while (it.hasNext()) {
                    MsgWaiting msgWaiting = (MsgWaiting)it.next();
                    if (logTriggerTime != 0L && l > msgWaiting.getEndTime()) {
                        it.remove();
                    }
                }
                if (msgWaitingList.isEmpty()) {
                    ...
                    int lineNumber = 0;
                    it = msgWaitingList.iterator();
                    while (it.hasNext()) {
                        MsgWaiting msgWaiting = (MsgWaiting)it.next();
                        float logPosX = ((float)viewDX - msgWaiting.getTextWidth()) / 2.0F;
                        StrTokenizer st = new StrTokenizer(msgWaiting.getMsg(), "\n");
                        while (st.hasMoreTokens()) {
                            try {
                                ttfont1.output(j1, logPosX , logPosY - (fontheight * lineNumber++), 0.0F, st.nextToken());
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        if (it.hasNext()) lineNumber++;
                    }
                }
            } else {
                logTriggerTime = 0L;
...
//    }
    ...
    {
//        System.out.println("addMsgToWaitingList (" + s + ", " + f + ")");
