...
public class CockpitP_38M_Radar extends CockpitPilot {
    ...
    public void doToggleAim(boolean flag) {
        ...
        if (this.radarShortRange == flag) {
            ...
            this.distanceMax = flag ? 10000F : 25000F;
    ...
    public CockpitP_38M_Radar() {
        super("3DO/Cockpit/P-61-Radar/hier.him", "He111");
        this.distanceMin = 100F;
        this.distanceMax = 25000F;
        this.heightMin = 200F;
    ...
    public void draw() {
        if (!Config.isUSE_RENDER() || !Actor.isValid(this.aircraft()) || !Actor.isAlive(this.aircraft())) {
            return;
        }
        int i = (int) ((Time.current() % this.beamTurnTime) / (this.beamTurnTime / 360F));
        Point3d point3d = this.aircraft().pos.getAbsPoint();
        if (this.oldBeamAngleInt > i) {
            Orient orient = this.aircraft().pos.getAbsOrient();
            ...
            for (int i1 = 0; i1 < this.radarPlane.size(); i1++) {
                if (l <= this.nTgts) {
                    ...
                    float f9 = (float) (Math.sqrt((f3 * f3) + (f5 * f5) + (f7 * f7)) / Math.sqrt((f3 * f3) + (f5 * f5))) / this.distanceMax / CockpitP_38M_Radar.screenPlotFactor;
                    float f11 = f3 * f9;
                    float f12 = f5 * f9;
                    l++;
    ...
    static {
        Property.set(CockpitP_38M_Radar.class, "astatePilotIndx", 1);
    }
}
