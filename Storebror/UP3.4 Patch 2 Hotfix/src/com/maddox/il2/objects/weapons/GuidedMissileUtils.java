        ...
        try {
            ...
            for (int i1 = 0; i1 < k; i1++) {
                ...
                if (theTarget instanceof Aircraft || theTarget instanceof MissileInterceptable) {
                    double maxTargetDistance = maxDistance * visibleFactor;
                    double maxTargetAngle = maxFOVfrom * visibleFactor;
                    if (Math.abs(actor.pos.getAbsPoint().x - theTarget.pos.getAbsPoint().x) > maxTargetDistance) continue;
                    if (Math.abs(actor.pos.getAbsPoint().y - theTarget.pos.getAbsPoint().y) > maxTargetDistance) continue;
                    
                    targetDistance = distanceBetween(actor, theTarget);
                    if (targetDistance > maxTargetDistance) continue;
                    targetAngle = angleBetween(actor, theTarget);
                    if (targetAngle > maxTargetAngle) continue;
...
//        if (distanceToTarget > maxDist || angleToTarget > maxAngleToTarget || angleFromTarget > maxAngleFromTargetAft) return 0F;
        ...
        if (Math.abs(this.missileOwner.pos.getAbsPoint().x - this.trgtInSight.pos.getAbsPoint().x) > maxDist) return 0F;
        if (Math.abs(this.missileOwner.pos.getAbsPoint().y - this.trgtInSight.pos.getAbsPoint().y) > maxDist) return 0F;
        float distanceToTarget = (float) distanceBetween(this.missileOwner, this.trgtInSight);
        if (distanceToTarget > maxDist) return 0F;
        float angleFromTarget = angleBetween(this.missileOwner, this.trgtInSight);
        if (angleFromTarget > maxAngleFromTargetAft) return 0F;
        float angleToTarget = 180.0F - angleBetween(this.trgtInSight, this.missileOwner);
        if (angleToTarget > maxAngleToTarget) return 0F;
