...
//        System.out.println("checkTask() isActive=" + isActive + ", isPlaying=" + Mission.isPlaying());
...
//        this.ticker.post();
        ...
        while (triggersProcessed < triggerSize && this.hpt.getSecondsElapsed() < 0.001D) {
            for (int triggerIndex = this.nextTrigger; triggerIndex < triggerSize; triggerIndex++) {
                ...
                // +++ TODO: Added by SAS~Storebror: The only Trigger being dealt with on Client Side in Online Missions is the "Player only" Message Trigger!
                if (Mission.isNet() && !Mission.isServer()) {
                    System.out.println("Trigger " + trigger.getTriggerName() + " Class=" + trigger.getTriggerClass() + ", TriggeredBy=" + trigger.getTriggeredBy());
                    if (trigger.getTriggerClass() != Trigger.TYPE_MESSAGE || trigger.getTriggeredBy() != ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY) continue;
                }
//                else if (Mission.isNet() && Mission.isServer()) {
//                    if (trigger.getTriggerClass() == Trigger.TYPE_MESSAGE && trigger.getTriggeredBy() == ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY) continue;
//                }
                // ---
