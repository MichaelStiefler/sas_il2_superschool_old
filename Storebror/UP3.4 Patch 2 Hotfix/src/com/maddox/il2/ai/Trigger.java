...
//        System.out.println("Trigger create(" + s + ")");
        ...
        int triggeredBy = numbertokenizer.next(0, 0, ActorTrigger.TRIGGERED_BY_SIZE - 1);
        boolean triggerOnExit = numbertokenizer.next(0) == 1;
        int noObjectsMin = numbertokenizer.next(1, -1, 1000);
        if (noObjectsMin == 0) noObjectsMin = 1;
        boolean triggerMultiple = triggeredBy == ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY && noObjectsMin == -1;
        ...
        switch (triggerType) {
            ...
            case TYPE_MESSAGE:
                new TriggerMessage(name, triggeredByArmy, intTimeout, posX, posY, radius, altitudeMin, altitudeMax, triggeredBy, triggerOnExit, noObjectsMin, probability, linkActorName, displayMessage, displayTime, triggerMultiple);
                break;
        }
    }
...
//                HUD.clearWaitingList();
    ...
    public void setTriggerMultiple(boolean triggerMultiple) {
...
//        test.printStackTrace();
        ...
        try {
            ...
            // +++ TODO: Added by SAS~Storebror: Skip replicating "Player only" Message Trigger in Online Missions!
//            if (Mission.isNet()) {
//                if (trigger.getTriggerClass() == Trigger.TYPE_MESSAGE || trigger.getTriggeredBy() == ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY) return;
//            }
            // ---
...
//                        System.out.println("getReplicatedTrigger class=" + triggerClass + ", alt=" + trigger.getMessageAltitude() + ", time=" + trigger.getDisplayTime());
                        if (netmsginput.available() > 1) {
                            NetObj netObj = netmsginput.readNetObj();
                            NetUser netUser = (NetUser)netObj;
//                            System.out.println("getReplicatedTrigger netUser=" + netUser.shortName());
                            if (World.getPlayerAircraft() instanceof NetAircraft) {
                                NetAircraft netPlayerAircraft = (NetAircraft)World.getPlayerAircraft();
                                if (netPlayerAircraft.netUser() == netUser) {
//                                    System.out.println("is Player Aircraft!");
                                    trigger.doExecute();
                                }
                            }
                            return;
...
//                System.out.println("Trigger getReplicatedTrigger() " + triggerName);
              trigger.doExecute();
//              if (Mission.isNet()) {
//                  if (trigger.getTriggerClass() == Trigger.TYPE_MESSAGE && trigger.getTriggeredBy() == ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY) return;
//              }
...
//        // +++ TODO: Added by SAS~Storebror: Skip replicating "Player only" Message Trigger in Online Missions!
//        if (Mission.isNet()) {
//            if (this.getTriggerClass() == Trigger.TYPE_MESSAGE || this.getTriggeredBy() == ActorTrigger.TRIGGERED_BY_CURRENT_PLAYER_ONLY) return;
//        }
//        // ---
...
//            System.out.println("Trigger replicateTrigger() " + this.getTriggerName());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
//    prepareTextKeyWords(this.prepareTextArmy(World.getPlayerArmy()), point2d, this.getMessageAltitude()), this.getDisplayTime() * 1000
    ...
    private boolean         triggerMultiple;
}
