...
public class CockpitN1K1JA extends CockpitPilot {
	...
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			if (fm != null) {
                ...
                float f = fm.CT.getGearL() + fm.CT.getGearR() + fm.CT.getGearC();
                if(setOld.gearLever != 0.0F && gears == f || !fm.Gears.bIsHydroOperable)	 	 
                    setNew.gearLever = 0.8F * setOld.gearLever;	 	 
                else	 	 
                if(gears < f)	 	 
                {	 	 
                    gears = f;	 	 
                    setNew.gearLever = 0.8F * setOld.gearLever + 6F;	 	 
                } else	 	 
                if(gears > f)	 	 
                {	 	 
                    gears = f;	 	 
                ...
                N1K n1k = (N1K)aircraft();	 	 
                if(n1k.emergencyPressure == 0.0F || fm.CT.GearControl == 1.0F)	 	 
                {	 	 
                    setNew.emPressLever = 0.9F * setOld.emPressLever;	 	 
                } else	 	 
                {	 	 
                    if(n1k.emergencyPressure > oldEmPressure)	 	 
                        setNew.emPressLever = 0.9F * setOld.emPressLever + 0.1F;	 	 
                    else	 	 
                        setNew.emPressLever = 0.9F * setOld.emPressLever;	 	 
                    oldEmPressure = n1k.emergencyPressure;	 	 
                }	 	 
			}
			return true;
		}
	...
	protected float waypointAzimuth() {
		return super.waypointAzimuthInvertMinus(3F);
	}

	public CockpitN1K1JA() {
		super("3DO/Cockpit/N1K2-Ja/hierNIK1.him", "bf109");
        ...
        gears = 0.0F;
        oldEmPressure = 0.0F;	 	 
		cockpitNightMats = (new String[] { "Gauges_01", "Gauges_02", "Gauges_03", "Gauges_04", "Gauges_05", "Gauges_06", "Gauges_07", "DGauges_01", "DGauges_02", "DGauges_03", "DGauges_04", "DGauges_05", "DGauges_06" });
		setNightMats(false);
		cockpitDimControl = !cockpitDimControl;
		interpPut(new Interpolater(), null, Time.current(), null);
		limits6DoF = (new float[] { 0.7F, 0.055F, -0.07F, 0.11F, 0.13F, -0.1F, 0.025F, -0.035F });
        light1 = new LightPointActor(new LightPoint(), new Point3d(-0.32500000000000001D, -0.32000000000000001D, 0.53129999999999999D));	 
        light2 = new LightPointActor(new LightPoint(), new Point3d(-0.57499999999999996D, 0.32500000000000001D, 0.496D));	 	 
        light3 = new LightPointActor(new LightPoint(), new Point3d(-0.33000000000000002D, 0.20499999999999999D, 0.65300000000000002D));	 	 
	...
	public void reflectWorldToInstruments(float f) {
		...
		mesh.chunkSetLocate("Canopy_glass", Cockpit.xyz, Cockpit.ypr);
		resetYPRmodifier();
        ...
        mesh.chunkSetAngles("GearHandle", setNew.gearLever, 0.0F, 0.0F);
        mesh.chunkSetAngles("GearHandleMan", cvt(setNew.emPressLever, 0.0F, 1.0F, 0.0F, -40F), 0.0F, 0.0F);	 	 
		...
		mesh.chunkSetAngles("ChargerLvr", cvt(fm.EI.engines[0].getControlCompressor(), 0.0F, 1.0F, 0.0F, 49F), 0.0F, 0.0F);
		mesh.chunkSetAngles("ChargerAutoLvr", cvt(fm.EI.engines[0].getControlCompressor(), 0.0F, 1.0F, 0.0F, 49F), 0.0F, 0.0F);
		...
		mesh.chunkSetAngles("Pedal_L", 35F * fm.CT.getBrakeL(), 0.0F, 0.0F);
		mesh.chunkSetAngles("Pedal_R", 35F * fm.CT.getBrakeR(), 0.0F, 0.0F);
		...
		mesh.chunkSetAngles("NeedOilPress", cvt(1.0F + 0.05F * fm.EI.engines[0].tOilIn, 0.0F, 10F, 0.0F, 180F), 0.0F, 0.0F);
		...
		if ((fm.AS.astateCockpitState & 0x10) == 0)
			mesh.chunkSetAngles("NeedDF", cvt(setNew.waypointAzimuth.getDeg(f * 0.2F), -90F, 90F, -33F, 33F), 0.0F, 0.0F);
	...
	public void toggleLight() {
		...
		if(cockpitLightControl) { 
            light1.light.setEmit(0.9F, 0.35F);	 	 
            light2.light.setEmit(0.9F, 0.15F);	 	 
            light3.light.setEmit(0.9F, 0.15F);	 	 
            setNightMats(true);
        } else {	 	 
            light1.light.setEmit(0.0F, 0.0F);	 	 
            light2.light.setEmit(0.0F, 0.0F);	 	 
	...
	static {
		Property.set(CLASS.THIS(), "normZNs", new float[] { 0.77F, 0.77F, 0.79F, 0.77F });
	}
