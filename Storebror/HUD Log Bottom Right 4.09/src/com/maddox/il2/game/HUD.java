...
public class HUD {
	...
	private void renderOrder() {
		...
		for (int k1 = 0; k1 < order.length; k1++) {
			if (orderStr[k1] != null) {
				if (s != null) drawOrder(s, i, l, j1 != 0 ? j1 : -1, i1);
				j1 = k1;
				s = orderStr[k1];
				l = k;
				i1 = order[k1].attrib();
		...
		if (s != null) drawOrder(s, i, l, 0, i1);
	}

	private void drawOrder(String s, int i, int j, int k, int l) {
		int i1 = 0xff0000ff;
	...
	private void renderLog() {
		...
		for (; logLen > 0; logLen--) {
			if (l < logTime[logPtr] + logTimeLife) break;
	...
	public void renderNetStat() {
		...
		for (int i2 = pageSizeNetStat * pageNetStat; i2 < pageSizeNetStat * (pageNetStat + 1) && i2 < statUsers.size(); i2++) {
			...
			int k2 = (int) ttfont.width(statuser.sNum);
			if (k < k2) k = k2;
			k2 = (int) ttfont1.width(statuser.sPing);
			if (l < k2) l = k2;
			k2 = (int) ttfont.width(statuser.user.uniqueName());
			if (i1 < k2) i1 = k2;
			k2 = (int) ttfont.width(statuser.sScore);
			if (j1 < k2) j1 = k2;
			k2 = (int) ttfont.width(statuser.sArmy);
			if (k1 < k2) k1 = k2;
			k2 = (int) ttfont.width(statuser.sAircraft);
			if (l1 < k2) l1 = k2;
		}

		int j2 = x1024(40F) + k;
		int l2 = j2 + l + x1024(16F);
		int i3 = l2 + i1 + x1024(16F);
		...
		for (int j4 = pageSizeNetStat * pageNetStat; j4 < pageSizeNetStat * (pageNetStat + 1) && j4 < statUsers.size(); j4++) {
			...
			ttfont1.output(-1, (float) l2 - ttfont1.width(statuser1.sPing) - (float) x1024(4F), i4, 0.0F, statuser1.sPing);
			ttfont.output(k4, l2, i4, 0.0F, statuser1.user.uniqueName());
			if (!Mission.isCoop()) ttfont.output(k4, i3, i4, 0.0F, statuser1.sScore);
	...
	private void renderDashBoard() {
		...
		float f5 = fntLcd.width(s);
		fntLcd.output(-1, 208F * f2 - f5, 70F * f3, 0.0F, s);
		...
		f5 = fntLcd.width(s);
		fntLcd.output(-1, 104F * f2 - f5, 135F * f3, 0.0F, s);
		...
		s = "" + (int) ((double) f4 + 0.5D);
		f5 = fntLcd.width(s);
		fntLcd.output(-1, 960F * f2 - f5, 216F * f3, 0.0F, s);
