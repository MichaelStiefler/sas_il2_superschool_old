...
public class PlMisAir extends Plugin
{
    ...
    private void makeRegimentList(int i)
    {
        ...
        for(int l = 0; l < j; l++)
        {
            ...
            String s = I18N.regimentShort(regiment1.shortInfo());
    ...
    public void load(SectFile sectfile)
    {
        ...
        for(int k = 0; k < j; k++)
        {
            ...
            for(int i5 = 0; i5 < i1; i5++)
            {
                ...
                pair = new PAir(pathair, ((PPoint) (pair)), point3d, byte0, d, d1);
                if(s7 != null)
                {
                    pair.iTarget = j5;
                    pair.sTarget = s7;
            ...
            if(l4 > 0)
            {
                PAir pair2 = (PAir)pathair.point(0);
                pair2.time = (double)l4 * 60D;
    ...
    public boolean save(SectFile sectfile)
    {
      ...
      for (int j = 0; j < aobj.length; j++)
      {
        ...
        if ((actor instanceof PathAir))
        {
          ...
          String str1 = pathair.sRegiment + pathair.iSquadron + pathair.iWing;
          
          String str2 = str1 + "_Way";
          sectfile.lineAdd(i, str1, "");
          int m = sectfile.sectionAdd(str1);
          sectfile.lineAdd(m, "Planes", "" + pathair.planes);
          if (pathair.bOnlyAI) {
            sectfile.lineAdd(m, "OnlyAI", "1");
          }
          if (!pathair.bParachute) {
            sectfile.lineAdd(m, "Parachute", "0");
          }
          if (pathair.skill != -1) {
            sectfile.lineAdd(m, "Skill", "" + pathair.skill);
          }
          int n;
          for (n = 0; n < 4; n++) {
            if (pathair.skill != pathair.skills[n]) {
              sectfile.lineAdd(m, "Skill" + n, "" + pathair.skills[n]);
            }
          }
          for (n = 0; n < 4; n++) {
            if (pathair.skins[n] != null) {
              sectfile.lineAdd(m, "skin" + n, pathair.skins[n]);
            }
          }
          for (n = 0; n < 4; n++) {
            if (pathair.noseart[n] != null) {
              sectfile.lineAdd(m, "noseart" + n, pathair.noseart[n]);
            }
          }
          for (n = 0; n < 4; n++) {
            if (pathair.pilots[n] != null) {
              sectfile.lineAdd(m, "pilot" + n, pathair.pilots[n]);
            }
          }
          for (n = 0; n < 4; n++) {
            if (!pathair.bNumberOn[n]) {
              sectfile.lineAdd(m, "numberOn" + n, "0");
            }
          }
          sectfile.lineAdd(m, "Class", ObjIO.classGetName(this.type[pathair._iType].item[pathair._iItem].clazz));
          sectfile.lineAdd(m, "Fuel", "" + pathair.fuel);
          if (pathair.weapons != null) {
            sectfile.lineAdd(m, "weapons", "" + pathair.weapons);
          } else {
            sectfile.lineAdd(m, "weapons", "none");
          }
          PAir pair = (PAir)pathair.point(0);
          if (pair.time > 0.0D) {
            sectfile.lineAdd(m, "StartTime", "" + (int)Math.round(pair.time / 60.0D));
          }
          int i1 = sectfile.sectionAdd(str2);
          for (int i2 = 0; i2 < k; i2++)
          {
            PAir pair1 = (PAir)pathair.point(i2);
            Point3d point3d = pair1.pos.getAbsPoint();
            switch (pair1.type())
            {
            case 0: 
              sectfile.lineAdd(i1, "NORMFLY", fmt(point3d.x) + " " + fmt(point3d.y) + " " + fmt(pair1.height) + " " + fmt(pair1.speed) + saveTarget(pair1) + saveRadioSilence(pair1));
              break;
            case 3: 
              sectfile.lineAdd(i1, "GATTACK", fmt(point3d.x) + " " + fmt(point3d.y) + " " + fmt(pair1.height) + " " + fmt(pair1.speed) + saveTarget(pair1) + saveRadioSilence(pair1));
              break;
            case 1: 
              sectfile.lineAdd(i1, "TAKEOFF", fmt(point3d.x) + " " + fmt(point3d.y) + " 0 0" + saveTarget(pair1) + saveRadioSilence(pair1));
              break;
            case 2: 
              sectfile.lineAdd(i1, "LANDING", fmt(point3d.x) + " " + fmt(point3d.y) + " 0 0" + saveTarget(pair1) + saveRadioSilence(pair1));
            }
          }
        }
      }
    ...
    public void insert(Loc loc, boolean flag)
    {
      ...
      try
      {
        ...
        {
          obj = (PAir)builder.selectedPoint();
          if (((PAir)obj).type() == 2) {
            return;
          }
          pAir = new PAir(builder.selectedPath(), builder.selectedPoint(), point3d, 0, this.defaultHeight, this.defaultSpeed);
        }
        else
        {
          ...
          pAir = new PAir(pathAir, null, point3d, 0, this.defaultHeight, this.defaultSpeed);
        }
