...
public class Jetman extends HE_LERCHE3
    implements TypeStormovik
{
    ...
    public void doSetSootState(int i, int j)
    {
        ...
        case 2:
            this.FM.AS.astateSootEffects[i][0] = Eff3DActor.New(this, findHook("_Engine" + (i + 1) + "EF_01"), null, 1.0F, "3DO/Effects/Aircraft/TurboJetman.eff", -1F);
            break;
    ...
    {
        exhaustFlames = new Actor[2];
        com.maddox.il2.engine.Hook theHook = null;
        for(int i = 0; i < 2; i++)
        {
            theHook = findHook("_Engine" + (i + 1) + "ES_01");
            exhaustFlames[i] = new ActorSimpleMesh("3DO/Effects/Aircraft/TurboJetmanFlame.sim");
            if(exhaustFlames[i] != null)
            {
                ((ActorSimpleMesh)exhaustFlames[i]).mesh().setScale(1.0F);
                exhaustFlames[i].pos.setBase(this, theHook, false);
                exhaustFlames[i].pos.changeHookToRel();
                exhaustFlames[i].pos.resetAsBase();
            }
        }
    ...
    private void endFlames()
    {
        if(exhaustFlames != null)
        {
            for(int i = 0; i < 2; i++)
                if(exhaustFlames[i] != null)
                    ObjState.destroy(exhaustFlames[i]);
    ...
    private void updateFlameSize()
    {
        if(exhaustFlames != null)
        {
            for(int engineNum = 0; engineNum < 2; engineNum++)
                if(exhaustFlames[engineNum] != null)
                {
                    ...
                    flameLenFactor[engineNum] = flameLenFactor[engineNum] * 0.95F + newFlameLenFactor * 0.05F;
                    ((ActorSimpleMesh)exhaustFlames[engineNum]).mesh().setScale(World.Rnd().nextFloat(0.5F * flameLenFactor[engineNum], 1.0F * flameLenFactor[engineNum]));
                }
    ...
    {
        hierMesh().chunkSetAngles("vVatorL_D0", 0.0F, 45F * f, 0.0F);
        hierMesh().chunkSetAngles("vVatorR_D0", 0.0F, -45F * f, 0.0F);
    }

    protected void moveAileron(float f)
    {
        hierMesh().chunkSetAngles("vAroneL_D0", 0.0F, -25F * f, 0.0F);
        hierMesh().chunkSetAngles("vAroneR_D0", 0.0F, -25F * f, 0.0F);
    }
    ...
    {
        hierMesh().chunkSetAngles("Flap01_D0", 0.0F, 65F * f, 0.0F);
        hierMesh().chunkSetAngles("Flap02_D0", 0.0F, -65F * f, 0.0F);
        hierMesh().chunkSetAngles("Flap03_D0", 0.0F, -65F * f, 0.0F);
        hierMesh().chunkSetAngles("Flap04_D0", 0.0F, 65F * f, 0.0F);
    }
        ...
        if(this.FM.AS.isMaster() && Config.isUSE_RENDER())
        {
            for(int engineNum = 0; engineNum < 2; engineNum++)
                if(this.FM.EI.engines[engineNum].getPowerOutput() > 0.8F && this.FM.EI.engines[engineNum].getStage() == 6)
                {
                    ...
                    if(this.FM.EI.engines[engineNum].getPowerOutput() > 0.9F)
    ...
    static 
    {
        ...
        Aircraft.weaponHooksRegister(class1, new String[] {
            "_CANNON01", "_CANNON02", "_ExternalRock01", "_ExternalBomb01"
        });
    }
}
