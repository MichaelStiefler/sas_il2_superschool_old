    ...
    protected void hitBone(String s, Shot shot, Point3d point3d)
    {
        if(s.startsWith("xx"))
        {
            if(s.startsWith("xxarmor"))
            {
                ...
                if(s.endsWith("t1"))
                {
                    getEnergyPastArmor((double)World.Rnd().nextFloat(21F, 42F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                    FM.AS.setCockpitState(shot.initiator, FM.AS.astateCockpitState | 2);
                    if(shot.power <= 0.0F)
                        doRicochetBack(shot);
                } else
                if(s.endsWith("t2"))
                    getEnergyPastArmor((double)World.Rnd().nextFloat(12.2F, 12.9F) / (Math.abs(v1.x) + 9.9999997473787516E-005D), shot);
                else
                if(s.endsWith("t3"))
            ...
            {
                if((s.endsWith("s1") || s.endsWith("s2")) && getEnergyPastArmor(4.8F, shot) > 0.0F && World.Rnd().nextFloat() < 0.25F)
                {
                ...
                if((s.endsWith("s5") || s.endsWith("s6") || s.endsWith("s7") || s.endsWith("s8")) && World.Rnd().nextFloat() < 0.5F && getEnergyPastArmor(0.35F, shot) > 0.0F)
                {
                    FM.AS.setControlsDamage(shot.initiator, 0);
            ...
            if(s.startsWith("xxspar"))
            {
                ...
                if(s.startsWith("xxspart") && chunkDamageVisible("Tail1") > 2 && getEnergyPastArmor(16.5F * World.Rnd().nextFloat(1.0F, 2.0F), shot) > 0.0F)
                {
