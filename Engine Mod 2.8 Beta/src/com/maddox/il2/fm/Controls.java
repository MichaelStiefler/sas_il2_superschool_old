...
public class Controls {
	...
	public void setLanded() {
		if (bHasGearControl) GearControl = Gear = gearC = gearR = gearL = 1.0F;
		else Gear = gearC = gearR = gearL = 1.0F;
	...
	public void setGearAirborne() {
		if (bHasGearControl) GearControl = Gear = gearC = gearL = gearR = 0.0F;
	}
	...
	public void forceGear(float f) {
		if (bHasGearControl) Gear = GearControl = gearC = gearR = gearL = f;
		else setFixedGear(true);
	}
	...
	public void setPowerControl(float f) {
		...
		// --------------------------------------------------------
		for (int i = 0; i < 8; i++)
			if (i < FM.EI.getNum() && FM.EI.bCurControl[i]) PowerControlArr[i] = f;
		// --------------------------------------------------------
	...
	public void setStepControl(float f) {
		if (!bUseElectricProp) {
			...
			// --------------------------------------------------------
			for (int i = 0; i < 8; i++)
				if (i < FM.EI.getNum() && FM.EI.bCurControl[i]) StepControlArr[i] = f;
			// --------------------------------------------------------
			HUD.log(AircraftHotKeys.hudLogPowerId, "PropPitch", new Object[] { new Integer(Math.round(getStepControl() * 100F)) });
		}
	}
	...
	public void setStepControl(float f, int i) {
		if (!bUseElectricProp) {
			...
			if (f < 0.0F) f = 0.0F;
			StepControlArr[i] = f;
			if (!getStepControlAuto(i)) HUD.log(AircraftHotKeys.hudLogPowerId, "PropPitch", new Object[] { new Integer(Math.round(getStepControl(i) * 100F)) });
		}
	}
	...
	public boolean getStepControlAuto(int i) {
		if (i < FM.EI.getNum()) return !FM.EI.engines[i].isHasControlProp() || FM.EI.engines[i].getControlPropAuto();
		else return true;
	}
	...
	public void setElectricPropUp(boolean flag) {
		if (bUseElectricProp) if (flag) electricPropUp = 1;
		else electricPropUp = 0;
	}

	public void setElectricPropDn(boolean flag) {
		if (bUseElectricProp) if (flag) electricPropDn = 1;
		else electricPropDn = 0;
	}
	...
	public void setRadiatorControlAuto(boolean flag, EnginesInterface enginesinterface) {
		...
		if (enginesinterface.getFirstSelected() != null) radiator = enginesinterface.getFirstSelected().getControlRadiator();
	}
	...
	public void setCompressorControl(int i) {
		if (i < 0) i = 0;
		if (i > FM.EI.engines[0].compressorMaxStep) i = FM.EI.engines[0].compressorMaxStep;
		CompressorControl = i;
	}
	...
	private float getGears(int i, float f) {
		...
		if (f1 == 0.0F || f1 == -1F || f1 == 1.0F) return f;
		if (FM.AS.isMaster() && f1 < 0.0F && FM.AS.gearDamRecoveryStates[i] >= 3 && Gear > Math.abs(f1)) FM.AS.hitGear(FM.actor, i, 11);
		return f;
	}
	...
	private float getDamMovSpd(int i, int j) {
		...
		case 4: // '\004'
			if (j == 0) return dvGear * 0.4F;
			else return dvGear * 0.6F;
	...
	public void update(float f, float f1, EnginesInterface enginesinterface, boolean flag, boolean flag1) {
		if (bHasBayDoors) bHasBayDoorControl = true;
		float f2 = 1.0F;
		float f3 = 1.0F;
		float f4 = 1.0F;
		float f5 = f1 * f1;
		if (f1 > AilThr) f2 = Math.max(0.2F, AilThr3 / (f5 * f1));
		if (f5 > RudThr2) f3 = Math.max(0.2F, RudThr2 / f5);
		if (f5 > ElevThr2) f4 = Math.max(0.4F, ElevThr2 / f5);
		f2 *= Sensitivity;
		f3 *= Sensitivity;
		f4 *= Sensitivity;
		if (!flag1) if (FM instanceof RealFlightModel) {
			float f6 = 0.0F;
			for (int j1 = 0; j1 < enginesinterface.getNum(); j1++) {
				...
				if (PowerControlArr[j1] > f6) f6 = PowerControlArr[j1];
			}
			...
			if (flag) Power = PowerControl;
			else Power = filter(f, PowerControl, Power, 5F, 0.01F * f);
			enginesinterface.setThrottle(Power);
		}
		if (!flag1) enginesinterface.setAfterburnerControl(afterburnerControl);
		if (!flag1) {
			...
			if (bUseElectricProp && (FM instanceof RealFlightModel)) {
				...
				if (i < 0) HUD.log(AircraftHotKeys.hudLogPowerId, "elPropDn");
				else if (i > 0) HUD.log(AircraftHotKeys.hudLogPowerId, "elPropUp");
				enginesinterface.setPropDelta(i);
			}
			if (enginesinterface.getFirstSelected() != null) if (bStepControlAuto) {
		...
		if (!flag1) enginesinterface.setMagnetos(MagnetoControl);
		if (!flag1 && flag && World.cur().diffCur.ComplexEManagement) enginesinterface.setCompressorStep(CompressorControl);
		if (!flag1) enginesinterface.setMix(MixControl);
		if (bHasGearControl || flag1) {
			...
			if (f7 > 0.0F) gearL = filter(f, f7, gearL, 999.9F, getDamMovSpd(FM.AS.gearDamType[0], 0));
			else if (f7 < 0.0F) gearL = filter(f, Math.min(GearControl, Math.abs(f7)), gearL, 999.9F, getDamMovSpd(FM.AS.gearDamType[0], 0));
			else gearL = filter(f, GearControl, gearL, 999.9F, dvGearL);
			if (f9 > 0.0F) gearR = filter(f, f9, gearR, 999.9F, getDamMovSpd(FM.AS.gearDamType[1], 1));
			else if (f9 < 0.0F) gearR = filter(f, Math.min(GearControl, Math.abs(f9)), gearR, 999.9F, getDamMovSpd(FM.AS.gearDamType[1], 1));
			else gearR = filter(f, GearControl, gearR, 999.9F, dvGearR);
			if (f10 > 0.0F) gearC = filter(f, f10, gearC, 999.9F, getDamMovSpd(FM.AS.gearDamType[2], 2));
			else if (f10 < 0.0F) gearC = filter(f, Math.min(GearControl, Math.abs(f10)), gearC, 999.9F, getDamMovSpd(FM.AS.gearDamType[2], 2));
			else gearC = filter(f, GearControl, gearC, 999.9F, dvGearC);
		}
		if (bHasAirBrakeControl || flag1) airBrake = filter(f, AirBrakeControl, airBrake, 999.9F, dvAirbrake);
		// TODO: For drag chute, refuelling, variable geometry wings and blown flaps
		// --------------------------------------------------------
		if (bHasDragChuteControl || flag1) dragChute = filter(f, DragChuteControl, dragChute, 999.9F, dvDragchute);
		if (bHasRefuelControl || flag1) refuel = filter(f, RefuelControl, refuel, 999.9F, dvRefuel);
		...
		if (bHasWingControl) {
			...
			if (wing > 0.01F && wing < 0.99F) FM.doRequestFMSFX(1, (int) (100F * wing));
		}
		if (bHasCockpitDoorControl) cockpitDoor = filter(f, cockpitDoorControl, cockpitDoor, 999.9F, dvCockpitDoor);
		if ((bHasArrestorControl || flag1) && (arrestorControl == 0.0F || arrestorControl == 1.0F)) arrestor = filter(f, arrestorControl, arrestor, 999.9F, 0.2F);
		if (bHasFlapsControl || flag1) {
			FlapsControl = clamp01(FlapsControl);
			if (Flaps > FlapsControl) Flaps = filter(f, FlapsControl, Flaps, 999F, Aircraft.cvt(FM.getSpeedKMH(), 150F, 280F, 0.15F, 0.25F));
			else Flaps = filter(f, FlapsControl, Flaps, 999F, Aircraft.cvt(FM.getSpeedKMH(), 150F, 280F, 0.15F, 0.02F));
		}
...
//			System.out.println(" getW().z=" + FM.getW().z);
		...
		if (bHasElevatorControl || flag1) {
			...
			if (FM.actor instanceof SU_26M2) Elevators = clamp11(Ev);
			else Elevators = clamp11(Ev - 0.25F * (1.0F - f4));
		}
		...
		if (bHasBrakeControl || flag1) {
			if (BrakeControl > Brake) Brake = Brake + 0.3F * f;
			else Brake = BrakeControl;
		} else {
			Brake = 0.0F;
		}
		...
		// --------------------------------------------------------
		if (bHasBrakeControl || flag1) {
			if (BrakeRightControl > BrakeRight) BrakeRight = BrakeRight + 0.3F * f;
			else BrakeRight = BrakeRightControl;
		} else {
			BrakeRight = 0.0F;
		}
		if (bHasBrakeControl || flag1) {
			if (BrakeLeftControl > BrakeLeft) BrakeLeft = BrakeLeft + 0.3F * f;
			else BrakeLeft = BrakeLeftControl;
		} else {
			BrakeLeft = 0.0F;
		}
		// --------------------------------------------------------
		if (tick == Time.tickCounter()) return;
		tick = Time.tickCounter();
		CTL = (byte) ((GearControl <= 0.5F ? 0 : 1) | (FlapsControl >= 0.25F ? ((byte) (FlapsControl >= 0.66F ? 6 : 4)) : FlapsControl <= 0.1F ? 0 : 2) | (BrakeControl <= 0.2F ? 0 : 8) | (RadiatorControl <= 0.5F ? 0 : 0x10)
				| (BayDoorControl <= 0.5F ? 0 : 0x20) | (AirBrakeControl <= 0.5F ? 0 : 0x40));
		WCT &= 0xfc;
		TWCT &= 0xfc;
		int k = 0;
		...
		for (int weaponTrigger = 0; weaponTrigger < Weapons.length; weaponTrigger++) {
			if (Weapons[weaponTrigger] != null) {
				...
				case 3: // default for bombs, not valid for missiles with GuidedMissileUtils support
...
//						HUD.log("Control " + weaponTrigger + " triggered!");
						...
						if ((System.currentTimeMillis() > (lWeaponTime + delay))) {
							...
							//
							// int dropBullet = -1;
							// for (int evenBullet = 0; evenBullet < Weapons[weaponTrigger].length; evenBullet += 2) {
							// if (!(Weapons[weaponTrigger][evenBullet] instanceof FuelTankGun) && Weapons[weaponTrigger][evenBullet].haveBullets()) {
							// if (Weapons[weaponTrigger][evenBullet].getHookName().startsWith("_BombSpawn") && ((Aircraft) FM.actor).needsOpenBombBay()) {
							...
							// } else {
							// if (Weapons[weaponTrigger][evenBullet].getHookName().startsWith("_InternalRock") && BayDoorControl == 0.0F) break;
							// else if (weaponFireMode == 2 || (Weapons[weaponTrigger][evenBullet] instanceof RocketGun4andHalfInch) || ((Weapons[weaponTrigger][evenBullet] instanceof RocketGun) && (iToggleRocketSide == LEFT))
							// || ((Weapons[weaponTrigger][evenBullet] instanceof RocketBombGun) && (iToggleRocketSide == LEFT)) || ((Weapons[weaponTrigger][evenBullet] instanceof BombGun) && (iToggleBombSide == LEFT))) {
							// Weapons[weaponTrigger][evenBullet].shots(1);
							// }
							// if (Weapons[weaponTrigger][evenBullet].getHookName().startsWith("_BombSpawn") && ((Aircraft) FM.actor).needsOpenBombBay()) if (bHasBayDoorControl && isPlayers((Aircraft) FM.actor)) {
							// if (BayDoorControl < 0.9F) break;
							// } else {
							// BayDoorControl = 1.0F;
							// }
							// }
							// if (((Weapons[weaponTrigger][evenBullet] instanceof BombGun) && weaponFireMode == 0) || ((Weapons[weaponTrigger][evenBullet] instanceof RocketGun) && !(Weapons[weaponTrigger][evenBullet] instanceof MissileGun) &&
							// weaponFireMode == 0)) {
							// Weapons[weaponTrigger][evenBullet].shots(1);
							// } else if (((Weapons[weaponTrigger][evenBullet] instanceof BombGun) && !((BombGun) Weapons[weaponTrigger][evenBullet]).isCassette() && weaponFireMode != 0)
							// || ((Weapons[weaponTrigger][evenBullet] instanceof RocketGun) && !((RocketGun) Weapons[weaponTrigger][evenBullet]).isCassette() && weaponFireMode != 0)) {
							...
							// }
							// }
							// for (int oddBullet = 1; oddBullet < Weapons[weaponTrigger].length; oddBullet += 2) {
							// if (!(Weapons[weaponTrigger][oddBullet] instanceof FuelTankGun) && Weapons[weaponTrigger][oddBullet].haveBullets()) {
							// if (Weapons[weaponTrigger][oddBullet].getHookName().startsWith("_BombSpawn") && ((Aircraft) FM.actor).needsOpenBombBay()) {
							// if (bHasBayDoorControl && isPlayers((Aircraft) FM.actor)) {
							// if (BayDoorControl < 0.9F) break;
							// } else {
							// BayDoorControl = 1.0F;
							...
							// } else {
							// if (Weapons[weaponTrigger][oddBullet].getHookName().startsWith("_InternalRock") && BayDoorControl == 0.0F) break;
							// else if (weaponFireMode == 2 || (Weapons[weaponTrigger][oddBullet] instanceof RocketGun4andHalfInch) || ((Weapons[weaponTrigger][oddBullet] instanceof RocketGun) && (iToggleRocketSide == RIGHT))
							// || ((Weapons[weaponTrigger][oddBullet] instanceof RocketBombGun) && (iToggleRocketSide == LEFT)) || ((Weapons[weaponTrigger][oddBullet] instanceof BombGun) && (iToggleBombSide == RIGHT))) {
							// Weapons[weaponTrigger][oddBullet].shots(1);
							// }
							// }
							// if (((Weapons[weaponTrigger][oddBullet] instanceof BombGun) && weaponFireMode == 0) || ((Weapons[weaponTrigger][oddBullet] instanceof RocketGun) && !(Weapons[weaponTrigger][oddBullet] instanceof MissileGun) && weaponFireMode
							// == 0)) {
							// Weapons[weaponTrigger][oddBullet].shots(1);
							// } else if (((Weapons[weaponTrigger][oddBullet] instanceof BombGun) && !((BombGun) Weapons[weaponTrigger][oddBullet]).isCassette() && weaponFireMode != 0)
							// || ((Weapons[weaponTrigger][oddBullet] instanceof RocketGun) && !((RocketGun) Weapons[weaponTrigger][oddBullet]).isCassette() && weaponFireMode != 0)
							// || ((Weapons[weaponTrigger][oddBullet] instanceof RocketBombGun) && !((RocketBombGun) Weapons[weaponTrigger][oddBullet]).isCassette() && weaponFireMode != 0)) {
							...
							// }
							// if (dropBullet != -1) {
							// if (Weapons[weaponTrigger][dropBullet] instanceof BombGun) {
							// if (iToggleBombSide == LEFT) iToggleBombSide = RIGHT;
							// else iToggleBombSide = LEFT;
							// } else if (!(Weapons[weaponTrigger][dropBullet] instanceof RocketGun4andHalfInch)) {
							// if (iToggleRocketSide == LEFT) iToggleRocketSide = RIGHT;
							// else iToggleRocketSide = LEFT;
							// }
							// }
							// if (weaponFireMode != 0) WeaponControl[weaponTrigger] = false;
						}
					}
					break;
				}
				default:
					...
					if (WeaponControl[weaponTrigger] && !flag2 && FM.isPlayers()) com.maddox.il2.objects.effects.ForceFeedback.fxTriggerShake(weaponTrigger, false);
					break;
				}
			}
		}
	...
	private boolean fireWeapon(int triggerNum, int hookNum, boolean dropFollowingNulls) {
		...
		if (theEmitter instanceof FuelTankGun) return false;
		if (!theEmitter.haveBullets()) return false;
		if (theEmitter.getHookName().startsWith("_BombSpawn") && ((Aircraft) FM.actor).needsOpenBombBay()) {
			if (bHasBayDoorControl && isPlayers((Aircraft) FM.actor)) {
				if (BayDoorControl < 0.9F) return true;
			} else {
				BayDoorControl = 1.0F;
			}
		}
		if (theEmitter.getHookName().startsWith("_InternalRock") && BayDoorControl == 0.0F) return true;
		if (!(theEmitter instanceof BombGun) && !(theEmitter instanceof RocketGun) && !(theEmitter instanceof RocketBombGun)) return false;
		...
		if (dropFollowingNulls) {
			for (int nullHooks = hookNum + 1; nullHooks < Weapons[triggerNum].length; nullHooks++) {
				...
				if (theEmitter == null) continue;
				if (theEmitter instanceof FuelTankGun) continue;
				if (!(theEmitter instanceof BombGunNull) && !(theEmitter instanceof RocketGunNull)) break;
				if (theEmitter.haveBullets()) theEmitter.shots(theEmitter.countBullets());
			}
		}
		return true;
	}
	...
	private boolean isPlayers(Aircraft aircraft) {
		if (aircraft.FM.isPlayers() && (aircraft.FM instanceof RealFlightModel) && ((RealFlightModel) aircraft.FM).isRealMode()) return true;
	...
	public float getWeaponMass() {
		...
		float f = 0.0F;
		for (int k = 0; k < i; k++) {
			if (Weapons[k] == null) continue;
			int l = Weapons[k].length;
			for (int j = 0; j < l; j++) {
				BulletEmitter bulletemitter = Weapons[k][j];
				if (bulletemitter == null || (bulletemitter instanceof FuelTankGun)) continue;
				int i1 = bulletemitter.countBullets();
				if (i1 < 0) {
					i1 = 1;
					if ((bulletemitter instanceof BombGun) && ((BombGun) bulletemitter).isCassette()) i1 = 10;
				}
	...
	public int getWeaponCount(int i) {
		if (i >= Weapons.length || Weapons[i] == null) return 0;
		int l = Weapons[i].length;
		int k;
		int j = k = 0;
		for (; k < l; k++) {
			BulletEmitter bulletemitter = Weapons[i][k];
			if (bulletemitter != null && !(bulletemitter instanceof FuelTankGun)) j += bulletemitter.countBullets();
		}
	...
	public FuelTank[] getFuelTanks() {
		...
		for (int i = 0; i < Weapons.length; i++) {
			...
			for (int k = 0; k < Weapons[i].length; k++)
				if (Weapons[i][k] instanceof FuelTankGun) i1++;
		...
		for (int j = j1 = 0; j < Weapons.length; j++) {
			...
			for (int l = 0; l < Weapons[j].length; l++)
				if (Weapons[j][l] instanceof FuelTankGun) afueltank[j1++] = ((FuelTankGun) Weapons[j][l]).getFuelTank();
	...
	public boolean isShooting() {
		for (int i = 0; i < WeaponControl.length; i++)
			if (i != 3 && WeaponControl[i]) return true;
	...
	public boolean toggleRocketHook() {
		if (rocketHookSelected == 2) {
			if (Weapons[4] != null) return doSetRocketHook(4);
			if (Weapons[5] != null) return doSetRocketHook(5);
			if (Weapons[6] != null) return doSetRocketHook(6);
		} else if (rocketHookSelected == 4) {
			if (Weapons[5] != null) return doSetRocketHook(5);
			if (Weapons[6] != null) return doSetRocketHook(6);
			if (Weapons[2] != null) return doSetRocketHook(2);
	...
	public void toggleWeaponFireMode(int hudLogWeaponId) {
		...
		if (!canToggleWeaponFireMode) return;
	...
	public void toggleWeaponReleaseDelay(int hudLogWeaponId) {
		if (Weapons[2] == null && Weapons[3] == null) return;
		if (weaponReleaseDelay == 33L) this.doChangeWeaponReleaseDelay(92L);
		else if (weaponReleaseDelay >= 125L) {
			if (weaponReleaseDelay >= 250L) this.doChangeWeaponReleaseDelay(250L);
			else this.doChangeWeaponReleaseDelay(125L);
		}
		if (weaponReleaseDelay > 1000L) this.doSetWeaponReleaseDelay(33L);
