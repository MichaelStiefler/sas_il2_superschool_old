...
public class AircraftTools {
	 ...
	 * It provides the implementation of the missing method which is called by lines like this:<br>&nbsp;<br>
	 * <code>weaponsRegister(class1, "default", new String[] { "MGunBrowning50k 250", "MGunBrowning50k 250" });</code><br>&nbsp;<br>
	 * The method "weaponsRegister" was part of former IL-2 versions (Pacific Fighters or earlier) but was removed
	 * and replaced by an empty method in IL-2 1946, therefore these lines in aircraft classes currently
	 * serve no purpose and weapon slots have to be either created manually by a lot of Java code lines
	 * or by providing the weapon slot declaration in a proprietary encrypted "cod" format.<br>
	 * This method brings back the old functionality so you can create weapon slots with a single line of code
	 * again.<br>&nbsp;<br>
	 * 
	 ...
	 * If the string consists of 3 parameters, then the first parameter defines the trigger index for this
	 * weapon, e.g. "0" for guns, "1" for cannons, "2" for rockets, "3" for bombs etc.<br>
	 * If the string consists of 2 parameters, then the trigger index for this weapon will be taken
	 * from the triggers defined in a previous call of the "weaponTriggersRegister" method.<br>
	 * The next parameter defines the weapon's class short name.<br>
	 * The last parameter defines the number of bullets for this weapon.<br>&nbsp;<br>
	 *
	 * @param airClass
	...
	private static void doWeaponsRegister(Class aircraftClass, String slotName, String theWeapons[]) {
		if (theWeapons == null) return;
		if (theWeapons.length < 1) return;
		...
		_WeaponSlot a_lweaponslot[] = new _WeaponSlot[theWeapons.length];
		for (int index = 0; index < theWeapons.length; index++) {
			if (theWeapons[index] == null) continue;
			...
			try {
				a_lweaponslot[index] = new _WeaponSlot(weaponHookNumber, numbertokenizer.next(null), numbertokenizer.next(-12345));
			} catch (Exception e) {
				System.out.println("Error registering Weapon Slot '" + slotName + "' for Aircraft '" + aircraftClass.getName() + "', StrackTrace follows:");
				e.printStackTrace();
			}
		}
	...
	private static void initMethods() {
		if (aircraftWeaponsListProperty == null) {
			aircraftWeaponsListProperty = Reflection.getMethod(Aircraft.class, "weaponsListProperty", new Object[] { Class.class });
		}
		if (aircraftWeaponsMapProperty == null) {
			aircraftWeaponsMapProperty = Reflection.getMethod(Aircraft.class, "weaponsMapProperty", new Object[] { Class.class });
		}
		if (aircraftGetWeaponTriggersRegistered == null) {
			aircraftGetWeaponTriggersRegistered = Reflection.getMethod(Aircraft.class, "getWeaponTriggersRegistered", new Object[] { Class.class });
		}
	}

	private static ArrayList weaponsListProperty(Class class1) {
		initMethods();
		if (aircraftWeaponsListProperty == null) return null;
	...
	private static HashMapInt weaponsMapProperty(Class class1) {
		initMethods();
		if (aircraftWeaponsMapProperty == null) return null;
	...
	private static int[] getWeaponTriggersRegistered(Class class1) {
		initMethods();
		if (aircraftGetWeaponTriggersRegistered == null) return null;
