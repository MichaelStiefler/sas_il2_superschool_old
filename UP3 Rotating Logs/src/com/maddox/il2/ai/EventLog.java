...
public class EventLog
{
	...
	public static String historyLogFolder = null;
	public static long maxLogLen = 0L;
	public static int maxNumHistoryFiles = 0;
	// TODO: -- Instant, persistent and rotating logs Mod --
	...
	private static void checkInited()
	{
		if (!bInited)
		{
			...
			// TODO: ++ Instant, persistent and rotating logs Mod ++
			checkIPRLModSettings();
			if (maxNumHistoryFiles != 0)
				logKeep = true;
			// TODO: -- Instant, persistent and rotating logs Mod --
			bInited = true;
		}
	...
	// TODO: ++ Instant, persistent and rotating logs Mod ++
	public static void checkIPRLModSettings() {
		historyLogFolder = Config.cur.ini.get("Mods", "IPRL_historyLogFolder", "logs");
		String maxLogLenStr = Config.cur.ini.get("Mods", "IPRL_maxLogLen", "1M");
		String numString = null;
		char dimChar = ' ';
		boolean hasDim = false;
		if (Character.isDigit(maxLogLenStr.charAt(maxLogLenStr.length() - 1)))
			numString = maxLogLenStr;
		else {
			numString = maxLogLenStr.substring(0, maxLogLenStr.length() - 1);
			dimChar = maxLogLenStr.toUpperCase().charAt(maxLogLenStr.length() - 1);
			hasDim = true;
		}
		try {
			maxLogLen = Long.parseLong(numString);
			if (hasDim) {
				long dimMult = 1L;
				switch (dimChar) {
				case 'G':
		...
		maxNumHistoryFiles = Config.cur.ini.get("Mods", "IPRL_maxNumHistoryFiles", 10);
		if (maxNumHistoryFiles < 0)
			maxNumHistoryFiles = Integer.MAX_VALUE;
	}

	public static void flushAndCheckIPRLLogFileSize() {
		if (maxNumHistoryFiles < 1)
			return;
		if (file == null)
			return;
		file.flush();
		...
		if (eventLogFile.length() <= maxLogLen)
			return;
		...
		new File(HomePath.toFileSystemName(historyLogFolder, 0)).mkdirs();
		eventLogFile.renameTo(new File(HomePath.toFileSystemName(historyLogFolder, 0) + "\\" + historyFileName));
	}
	// TODO: -- Instant, persistent and rotating logs Mod --
