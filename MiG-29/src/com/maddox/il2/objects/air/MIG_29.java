...
public class MIG_29 extends Scheme2 implements TypeSupersonic, TypeFighter, TypeFighterAceMaker, TypeGSuit, TypeFastJet {
	...
	public void checkHydraulicStatus() {
		if (this.FM.EI.engines[0].getStage() < 6 && this.FM.EI.engines[1].getStage() < 6 && this.FM.Gears.nOfGearsOnGr > 0) {
			gearTargetAngle = 90F;
	...
	public void rareAction(float f, boolean flag) {
		...
		if (this.FM.AS.isMaster() && Config.isUSE_RENDER()) {
			...
			float f1 = mn;
			World.cur().getClass();
			mig_29.mn = f1 / Atmosphere.sonicSpeed((float) ((Tuple3d) (this.FM.Loc)).z);
		...
		if ((!super.FM.isPlayers() || !(super.FM instanceof RealFlightModel) || !((RealFlightModel) super.FM).isRealMode()) && (super.FM instanceof Maneuver))
			if (this.FM.AP.way.isLanding() && super.FM.getSpeed() > this.FM.VmaxFLAPS && super.FM.getSpeed() > this.FM.AP.way.curr().getV() * 1.4F) {
				if (this.FM.CT.AirBrakeControl != 1.0F)
					this.FM.CT.AirBrakeControl = 1.0F;
			} else if (((Maneuver) super.FM).get_maneuver() == 25 && this.FM.AP.way.isLanding() && super.FM.getSpeed() < this.FM.VmaxFLAPS * 1.2F) {
				if (super.FM.getSpeed() > this.FM.VminFLAPS * 0.5F && this.FM.Gears.nearGround()) {
					if (this.FM.Gears.onGround()) {
						if (this.FM.CT.AirBrakeControl != 1.0F)
							this.FM.CT.AirBrakeControl = 1.0F;
						this.FM.CT.DragChuteControl = 1.0F;
					} else if (this.FM.CT.AirBrakeControl != 1.0F)
						this.FM.CT.AirBrakeControl = 1.0F;
				...
				if (this.FM.CT.AirBrakeControl != 1.0F)
					this.FM.CT.AirBrakeControl = 1.0F;
			} else if (hasHydraulicPressure && this.FM.CT.AirBrakeControl != 0.0F)
				this.FM.CT.AirBrakeControl = 0.0F;
		if (this.FM.AP.way.curr().Action == 3)
			this.FM.CT.dropFuelTanks();
		if (this.FM.AP.way.curr().Action == 3 && !((Maneuver) super.FM).hasBombs()) {
			this.FM.AP.way.next();
	...
	private final void updateControlsVisuals() {
		hierMesh().chunkSetAngles("VatorL_D0", 12F * this.FM.CT.getElevator(), -17F * this.FM.CT.getElevator() + 10F * this.FM.CT.getAileron(), 0.0F);
		hierMesh().chunkSetAngles("VatorR_D0", -12F * this.FM.CT.getElevator(), -17F * this.FM.CT.getElevator() - 10F * this.FM.CT.getAileron(), 0.0F);
	}
	...
	private final void updateFlaperon() {
		hierMesh().chunkSetAngles("AroneL_D0", 0.0F, -30F * this.FM.CT.getAileron() - 5F * this.FM.CT.getFlap(), 0.0F);
		hierMesh().chunkSetAngles("AroneR_D0", 0.0F, -30F * this.FM.CT.getAileron() - 5F * this.FM.CT.getFlap(), 0.0F);
	}
	...
	protected boolean cutFM(int i, int j, Actor actor) {
		...
		case 13: // '\r'
			...
			if (f < 0.1F) {
				...
				this.FM.EI.engines[1].setEngineDies(actor);
	...
	public void update(float f) {
		if (super.FM.getSpeedKMH() > 700F && this.FM.CT.bHasFlapsControl) {
		...
		if (this.FM.AS.isMaster() && Config.isUSE_RENDER()) {
			if (this.FM.EI.engines[0].getThrustOutput() > 0.5F && this.FM.EI.engines[0].getStage() == 6) {
				...
				else if (this.FM.EI.engines[0].getThrustOutput() > 0.92F && this.FM.EI.engines[0].getThrustOutput() < 1.001F)
			...
			if (this.FM.EI.engines[1].getThrustOutput() > 0.25F && this.FM.EI.engines[1].getStage() == 6) {
				if (this.FM.EI.engines[1].getThrustOutput() > 1.001F)
					this.FM.AS.setSootState(this, 1, 5);
				else if (this.FM.EI.engines[1].getThrustOutput() > 0.92F && this.FM.EI.engines[1].getThrustOutput() < 1.001F)
	...
	public void doSetSootState(int i, int j) {
		...
		case 3: // '\003'
			this.FM.AS.astateSootEffects[i][0] = Eff3DActor.New(this, findHook("_Engine" + (i + 1) + "EF_01"), null, 4F, "3DO/Effects/Aircraft/TurboJRD1100F.eff", -1F);
		...
		case 5: // '\005'
			this.FM.AS.astateSootEffects[i][0] = Eff3DActor.New(this, findHook("_Engine" + (i + 1) + "EF_01"), null, 3F, "3DO/Effects/Aircraft/AfterBurnerF100D.eff", -1F);
			this.FM.AS.astateSootEffects[i][1] = Eff3DActor.New(this, findHook("_Engine" + (i + 1) + "ES_01"), null, 1.5F, "3DO/Effects/Aircraft/BlackMediumTSPD.eff", -1F);
			break;
	...
	private final void umn() {
		...
		float f = mn;
		World.cur().getClass();
		mig_29.mn = f / Atmosphere.sonicSpeed((float) ((Tuple3d) (this.FM.Loc)).z);
		if (mn >= lteb)
			ts = true;
		else
			ts = false;
	...
	static {
		Class class1 = com.maddox.il2.objects.air.MIG_29.class;
