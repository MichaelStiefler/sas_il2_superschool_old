...
//TODO: For Countermeasures:
		if (this.iDetectorMode == Missile.DETECTOR_TYPE_INFRARED) {
			...
			int m = list1.size();
			for (int t = 0; t < m; t++) {
				Actor flare = (Actor) list1.get(t);
				targetDistance = distanceBetween(actor, flare);
				targetAngle = angleBetween(actor, flare);
				if(flare instanceof com.maddox.il2.objects.weapons.RocketFlare && targetDistance < maxDistance && targetAngle < maxFOVfrom)
				{
					this.trgtMissile = flare;
				}
			}
		}
