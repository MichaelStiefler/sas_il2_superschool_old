...
public class HUD {
	...
	private final void renderSpeed() {
		...
		if (iDrawSpeed != lastDrawSpeed) try {
	...
	private void drawWeaponsInfo() {
		...
		TTFont ttfont = TTFont.font[1];
		int j = 0xff0000ff;
		int k = (int) ((double) viewDX * 0.59999999999999998D);
		int l = ttfont.height();
	...
	private void drawOrder(String s, int i, int j, int k, int l, boolean flag) {
		...
		if (k >= 0) ttfont.output(i1, i, j, 0.0F, "" + k + ". " + s);
	...
	public void resetMsgSizes() {
		...
		if (msgDY > (int) ((double) viewDY * 0.90000000000000002D)) msgDY = (int) ((double) viewDY * 0.90000000000000002D);
	...
	public void renderNetStat() {
		...
		for (int j4 = pageSizeNetStat * pageNetStat; j4 < pageSizeNetStat * (pageNetStat + 1) && j4 < statUsers.size(); j4++) {
			StatUser statuser1 = (StatUser) statUsers.get(j4);
			i4 -= i;
			int k4 = Army.color(statuser1.iArmy);
			if (!Main.cur().netServerParams.netStat_ShowPilotArmy) k4 = -1;
			if (Main.cur().netServerParams.netStat_ShowPilotNumber) ttfont.output(k4, (float) j2 - ttfont.width(statuser1.sNum), i4, 0.0F, statuser1.sNum);
			...
			if (Main.cur().netServerParams.netStat_ShowPilotName) ttfont.output(k4, k2, i4, 0.0F, statuser1.user.uniqueName());
			if (!Mission.isCoop() && Main.cur().netServerParams.netStat_ShowPilotScore) ttfont.output(k4, i3, i4, 0.0F, statuser1.sScore);
			if (Main.cur().netServerParams.netStat_ShowPilotArmy) ttfont.output(k4, j3, i4, 0.0F, statuser1.sArmy);
			if (Main.cur().netServerParams.netStat_ShowPilotACDesignation) ttfont.output(k4, k3, i4, 0.0F, statuser1.sAircraft);
			if (Main.cur().netServerParams.netStat_ShowPilotACType) ttfont.output(k4, l3, i4, 0.0F, statuser1.sAircraftType);
		}
	...
	private void renderDashBoard() {
		...
		float f2 = f / 1024F;
		float f3 = f1 / 768F;
		Render.drawTile(0.0F, 0.0F, 256F * f2, 256F * f3, 0.0F, spriteLeft, -1, 0.0F, 1.0F, 1.0F, -1F);
		Render.drawTile(768F * f2, 0.0F, 256F * f2, 256F * f3, 0.0F, spriteRight, -1, 0.0F, 1.0F, 1.0F, -1F);
		Point3d point3d = World.getPlayerAircraft().pos.getAbsPoint();
		Orient orient = World.getPlayerAircraft().pos.getAbsOrient();
		float f4 = (float) (point3d.z - World.land().HQ(point3d.x, point3d.y));
		_p.x = 172F * f2;
		_p.y = 84F * f3;
		...
		meshNeedle1.render();
		String s = "" + (int) ((double) f4 + 0.5D);
		float f6 = fntLcd.width(s);
		fntLcd.output(-1, 208F * f2 - f6, 70F * f3, 0.0F, s);
		if (f4 > 90F) meshNeedle5.setScale(90F * f2);
		else meshNeedle5.setScaleXYZ(90F * f2, 90F * f2, cvt(f4, 0.0F, 90F, 13F, 90F) * f2);
		f4 = (float) World.getPlayerAircraft().getSpeed(null);
		f4 *= 3.6F;
		...
		float f5 = (float) i / 10F;
		String s1 = "" + f5;
		float f7 = fntLcd.width(s1);
		if (World.getPlayerFM().getLoadDiff() < World.getPlayerFM().getLimitLoad() * 0.25F) fntLcd.output(0xff0000ff, 215F * f2 - f7, 182F * f3, 0.0F, s1);
		else if (i < 0) fntLcd.output(0xff000000, 215F * f2 - f7, 182F * f3, 0.0F, s1);
		else fntLcd.output(-1, 215F * f2 - f7, 182F * f3, 0.0F, s1);
	}
	...
	private void renderRec() {
		if (!NetMissionTrack.isRecording() || !bRec) return;
		float f = viewDX;
		float f1 = viewDY;
		float f2 = f / 1024F;
		float f3 = f1 / 768F;
		if (NetMissionTrack.isPlaying()) {
			Render.drawTile(828F * f2, 2.0F * f3, 16F * f2, 16F * f3, 0.0F, spriteRec, -1, 0.0F, 1.0F, 1.0F, -1F);
		} else {
			Render.drawTile(896F * f2, 2.0F * f3, 16F * f2, 16F * f3, 0.0F, spriteRec, -1, 0.0F, 1.0F, 1.0F, -1F);
		}
	}
