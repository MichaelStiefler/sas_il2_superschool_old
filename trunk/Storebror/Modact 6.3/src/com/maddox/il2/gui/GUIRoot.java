...
public class GUIRoot extends GWindowRoot {
    ...
    public void setBackCountry(String s, String s1, String s2) {
        if ((s == null) || (s1 == null)) {
            ...
            if (obj != null) {
                this.backgroundCountry = GTexture.New(s5);
            }
        }
    }
    ...
    public void render() {
        if (RendersMain.getRenderFocus() == (Render) Actor.getByName("renderGUI")) {
            ...
            if (this.backgroundCountry != null) {
                gtexture = this.backgroundCountry;
            } else if (this.background != null) {
                gtexture = this.background;
            }
            if (gtexture != null) {
                if (((this.win.dx / this.win.dy) < 1.0F) && (gtexture.size.dx >= gtexture.size.dy)) {
                    float f = this.win.dy * (gtexture.size.dx / gtexture.size.dy);
                    float f1 = (gtexture.size.dx / 2.0F) - (f / 2.0F);
                    this.draw(f1, 0.0F, f, this.win.dy, gtexture);
                } else {
                    this.draw(0.0F, 0.0F, this.win.dx, this.win.dy, gtexture);
                }
            }
        }
    }
    ...
    private void drawGUIBackgroundFromJpg() {
        ...
        if (createSplashResult == 0) {
            ...
            this.background = GTexture.New(matImageFileName);
        }
    }
