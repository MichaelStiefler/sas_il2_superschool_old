...
public class CockpitBEAUFORT_Bombardier extends CockpitBombardier {
	class Interpolater extends InterpolateRef {

		public boolean tick() {
			float f = ((Beaufort) aircraft()).fSightCurForwardAngle;
			float f1 = ((Beaufort) aircraft()).fSightCurSideslip;
			CockpitBEAUFORT_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
			mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitBEAUFORT_Bombardier.calibrAngle + f);
			if (bEntered) {
				HookPilot hookpilot = HookPilot.current;
				hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitBEAUFORT_Bombardier.calibrAngle + f, 0.0F);
			}
			return true;
		}
...
//			aircraft().hierMesh().chunkVisible("Head2_D0", true);
	...
	public CockpitBEAUFORT_Bombardier() {
		super("3DO/Cockpit/Blenheim-Bombardier/BombardierBeaufort.him", "he111");
		w = new Vector3f();
		bEntered = false;
		try {
			Loc loc = new Loc();
		...
		cockpitNightMats = (new String[] { "4_gauges", "norden_crosshair" });
		setNightMats(false);
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (bEntered) {
			mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((Beaufort) aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
