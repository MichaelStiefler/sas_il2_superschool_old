...
public class GUINetServerMisSelect extends GameState
{
    public class DialogClient extends GUIDialogClient
    {
        ...
        public void render()
        {
            ...
            draw(x1024(104F), y1024(592F), x1024(192F), y1024(48F), 0, !USGS.isUsed() && Main.cur().netGameSpy == null ? i18n("netsms.MainMenu") : i18n("main.Quit"));
...
;
        ...
        fillDirs();
	    	try		//By PAL set Last Mission
		    	{   //wCountry.setSelected(0, true, true);
					...
					if(Main.cur().netServerParams.isCoop())
					{	    		
			    		LastDir = inifile.get("LastNetCoop", "LastDir", 0);
			    		LastMission = inifile.get("LastNetCoop", "LastMission", 0);
					} else  
					if(Main.cur().netServerParams.isDogfight())
					{
			    		LastDir = inifile.get("LastNetDogfight", "LastDir", 0);
			    		LastMission = inifile.get("LastNetDogfight", "LastMission", 0);
					}	
    ...
    public void _leave()
    {
    	try //By PAL, save last settings
    	{
			...
			if(Main.cur().netServerParams.isCoop())
			{
				inifile.set("LastNetCoop", "LastDir", wDirs.getSelected());
				inifile.set("LastNetCoop", "LastMission", wTable.selectRow);
			} else  
			if(Main.cur().netServerParams.isDogfight())
			{
				inifile.set("LastNetDogfight", "LastDir", wDirs.getSelected());
				inifile.set("LastNetDogfight", "LastMission", wTable.selectRow);
			}			
			inifile.saveFile();
    	}
    	catch(Exception exception)
