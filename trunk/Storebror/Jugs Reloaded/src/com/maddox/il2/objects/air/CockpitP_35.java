...
public class CockpitP_35 extends CockpitPilot {
		...
		public boolean tick() {
			if (CockpitP_35.this.fm != null) {
				...
				if (BaseGameVersion.is412orLater()) {
					CockpitP_35.this.setNew.waypointAzimuth = (10F * CockpitP_35.this.setOld.waypointAzimuth + (CockpitP_35.this.waypointAzimuth() - CockpitP_35.this.setOld.azimuth.getDeg(1.0F)) + World.rnd().nextFloat(-30F, 30F)) / 11F;
				} else {
				...
				byte byte0;
				float f1;
				if (CockpitP_35.this.fuelPerct > fuelTankLevels[0]) {
					if (CockpitP_35.this.fuelPerct > fuelTankLevels[1]) {
						byte0 = 2;
					} else {
						byte0 = 1;
					}
					f1 = CockpitP_35.this.fuelPerct - fuelTankLevels[byte0 - 1];
				} else {
					byte0 = 0;
				...
				if (CockpitP_35.this.fuelSwitchAngle > fuelSwitchAngles[byte0]) {
	...
	public void reflectWorldToInstruments(float f) {
		...
		xyz[1] = this.cvt(this.fm.CT.getCockpitDoor(), 0.1F, 0.9F, 0.0F, 0.7F);
		this.mesh.chunkSetLocate("Z_canopy", xyz, ypr);
		...
		this.mesh.chunkSetAngles("Z_canopy_open_rod", xyz[1] * 2570F, 0.0F, 0.0F);
		...
		this.resetYPRmodifier();
		xyz[1] = this.interp(this.setNew.prop, this.setOld.prop, f);
		this.mesh.chunkSetAngles("Z_Prop_Pitch", 0.0F, 65F * xyz[1], 0.0F);
		xyz[1] = xyz[1] * 0.02F - 0.02F;
		this.mesh.chunkSetLocate("Z_Prop_Pitch_strut", xyz, ypr);
		xyz[1] = this.cvt(this.fm.EI.engines[0].getControlMix(), 0.0F, 1.2F, 0.0F, 1.0F);
		this.mesh.chunkSetAngles("Z_Mixture", 0.0F, 68F * xyz[1], 0.0F);
		xyz[1] = xyz[1] * 0.02F - 0.02F;
		this.mesh.chunkSetLocate("Z_Mixture_strut", xyz, ypr);
		if (this.reversedThrottle) {
			xyz[1] = 1.0F - this.interp(this.setNew.throttle, this.setOld.throttle, f);
		} else {
			xyz[1] = this.interp(this.setNew.throttle, this.setOld.throttle, f);
		}
		this.mesh.chunkSetAngles("Z_Throttle", 0.0F, this.cvt(xyz[1], 0.0F, 1.1F, -3F, 60F), 0.0F);
		xyz[1] = xyz[1] * 0.02F - 0.021F;
		this.mesh.chunkSetLocate("Z_Throttle_strut", xyz, ypr);
		...
		if (BaseGameVersion.is412orLater()) {
			if (this.fm.Gears.lgear) {
				this.mesh.chunkSetAngles("ZGearL", 0.0F, this.cvt(this.fm.CT.getGearL(), 0.0F, 1.0F, 0.0F, 76F), 0.0F);
			}
			if (this.fm.Gears.rgear) {
				this.mesh.chunkSetAngles("ZGearR", 0.0F, this.cvt(this.fm.CT.getGearR(), 0.0F, 1.0F, 0.0F, -76F), 0.0F);
			}
			if (this.fm.Gears.cgear) {
				this.mesh.chunkSetAngles("ZGearC", 0.0F, this.cvt(this.fm.CT.getGearC(), 0.0F, 1.0F, 0.0F, 90F), 0.0F);
			}
		} else {
			if (this.fm.Gears.lgear) {
	...
	protected void reflectPlaneMats() {
		...
		if (this.aircraft().thisWeaponsName.startsWith("P35A")) {
