...
public class CockpitPB4Y_2_Bombardier extends CockpitPilot
{
    ...
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            ...
            setOld = setNew;
            setNew = setTmp;
            float f = ((PB4Y_2)aircraft()).fSightCurForwardAngle;
            float f1 = ((PB4Y_2)aircraft()).fSightCurSideslip;
            CockpitPB4Y_2_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
            mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitPB4Y_2_Bombardier.calibrAngle + f);
            if(bEntered)
            {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitPB4Y_2_Bombardier.calibrAngle + f, 0.0F);
            }
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        if(bEntered)
        {
            this.mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((PB4Y_2)aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
            boolean flag = ((PB4Y_2)aircraft()).fSightCurReadyness > 0.93F;
