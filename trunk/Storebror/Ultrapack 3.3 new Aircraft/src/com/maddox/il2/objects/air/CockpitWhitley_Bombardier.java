...
public class CockpitWhitley_Bombardier extends CockpitPilot
{
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            float f = ((WhitleyMkV)aircraft()).fSightCurForwardAngle;
            float f1 = ((WhitleyMkV)aircraft()).fSightCurSideslip;
            CockpitWhitley_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
            mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitWhitley_Bombardier.calibrAngle + f);
            if(bEntered)
            {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitWhitley_Bombardier.calibrAngle + f, 0.0F);
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        if(bEntered)
        {
            mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((WhitleyMkV)aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
            boolean flag = ((WhitleyMkV)aircraft()).fSightCurReadyness > 0.93F;
