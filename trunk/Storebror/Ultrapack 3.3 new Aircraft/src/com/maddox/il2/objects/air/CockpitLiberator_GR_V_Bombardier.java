...
public class CockpitLiberator_GR_V_Bombardier extends CockpitPilot
{
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            ...
            setOld = setNew;
            setNew = setTmp;
            float f = ((Liberator_GR_V)aircraft()).fSightCurForwardAngle;
            float f1 = ((Liberator_GR_V)aircraft()).fSightCurSideslip;
            CockpitLiberator_GR_V_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
            mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitLiberator_GR_V_Bombardier.calibrAngle + f);
            if(bEntered)
            {
                HookPilot hookpilot = HookPilot.current;
                hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitLiberator_GR_V_Bombardier.calibrAngle + f, 0.0F);
            }
    ...
    public CockpitLiberator_GR_V_Bombardier()
    {
        ...
        enteringAim = false;
        bEntered = false;
        setOld = new Variables((Variables)null);
        setNew = new Variables((Variables)null);
        try
        {
            Loc loc = new Loc();
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        this.mesh.chunkSetAngles("zCompass2", 0.0F, setNew.waypointAzimuth.getDeg(0.1F), 0.0F);
        if(enteringAim)
        {
            HookPilot hookpilot = HookPilot.current;
        ...
        if(bEntered)
        {
            this.mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((Liberator_GR_V)aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
            boolean flag = ((Liberator_GR_V)aircraft()).fSightCurReadyness > 0.93F;
