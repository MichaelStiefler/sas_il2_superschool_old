...
public class CockpitJU_88A1 extends CockpitPilot
{
    class Interpolater extends InterpolateRef
    {
        ...
        public boolean tick()
        {
            ...
            if(fm != null)
            {
                ...
                if(buzzerFX != null)
                    if(fm.Loc.z < (double)((JU_88A1)aircraft()).fDiveRecoveryAlt && ((JU_88A1)((Interpolate) (fm)).actor).diveMechStage == 1)
                        buzzerFX.play();
                    else
                    if(buzzerFX.isPlaying())
                        buzzerFX.stop();
                f1 = ((JU_88A1)aircraft()).fDiveRecoveryAlt;
                float f2 = -(((JU_88A1)aircraft()).fDiveVelocity * 0.27777F * (float)Math.sin(Math.toRadians(((JU_88A1)aircraft()).fDiveAngle))) * 0.1019F;
                f2 += (float)Math.sqrt(f2 * f2 + 2.0F * f1 * 0.1019F);
                float f3 = ((JU_88A1)aircraft()).fDiveVelocity * 0.27777F * (float)Math.cos(Math.toRadians(((JU_88A1)aircraft()).fDiveAngle));
                float f4 = (f3 * f2 + 10F) - 10F;
                setNew.alpha = 90F - ((JU_88A1)aircraft()).fDiveAngle - (float)Math.toDegrees(Math.atan(f4 / f1));
            }
            return true;
        }
    ...
    public void reflectWorldToInstruments(float f)
    {
        ...
        this.mesh.chunkSetAngles("z_Object20", cvt(((JU_88A1)aircraft()).fSightCurSpeed, 400F, 800F, 87F, -63.5F), 0.0F, 0.0F);
        ...
        this.mesh.chunkSetAngles("zAlt3", (((JU_88A1)aircraft()).fDiveRecoveryAlt * 360F) / 6000F, 0.0F, 0.0F);
