...
//		System.out.println("MissileGun 1 shots(" + numMissilesToShoot + ") class=" + this.getClass().getName() + ", hash=" + this.hashCode());
...
//		e.printStackTrace();
		...
		try {
			if (Actor.isValid(this.actor) && (this.actor instanceof Aircraft) && (this.actor instanceof TypeGuidedMissileCarrier)) {
				if ((Aircraft)this.actor == World.getPlayerAircraft() && ((RealFlightModel)((Aircraft) this.actor).FM).isRealMode() && ((TypeGuidedMissileCarrier) this.actor).getGuidedMissileUtils().hasMissiles()) {
					...
					if (((Aircraft)actor).FM.CT.getRocketFireMode() == Controls.fullSalvo) shotFrequency = -100L;
					if (Time.current() < ((TypeGuidedMissileCarrier) this.actor).getGuidedMissileUtils().getStartLastMissile() + shotFrequency) {
