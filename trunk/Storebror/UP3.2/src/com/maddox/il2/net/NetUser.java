...
public class NetUser extends NetHost implements NetFileClient, NetUpdate {
    ...
    public void setPatchLevel(String patchLevel) {
        if (this.patchLevel != patchLevel) {
            System.out.println("Player " + this.uniqueName() + " uses Patch Level " + patchLevel + "!");
        }
    ...
    public void setUltrapackVersion(String ultrapackVersion) {
        if (this.ultrapackVersion != ultrapackVersion) {
            System.out.println("Player " + this.uniqueName() + " uses Ultrapack Version " + ultrapackVersion + "!");
        }
...
// replicateSelectorVersion(null);
        ...
        for (int j = 0; j < list.size(); j++) {
            NetChannel netchannel = (NetChannel) list.get(j);
            if (netchannel.isMirrored(this) || netchannel == this.masterChannel())
                this.replicateUltrapackVersion(netchannel, theNetUser);
        }
...
// System.out.println("Replicating Selector Version " + theNetUser.getSelectorVersion() + " for user " + theNetUser.uniqueName() + " to " + (netchannel == null ? "all" : "Channel " + netchannel.id()));
    ...
    private void checkUpdateChatMessages() {
        ...
        for (int j = 0; j < list.size(); j++) {
            NetUser netuser = (NetUser) list.get(j);
            if (netuser.getUltrapackVersion().equalsIgnoreCase(UP_VERSION) && netuser.getPatchLevel().equalsIgnoreCase(PATCH_LEVEL))
                continue;
