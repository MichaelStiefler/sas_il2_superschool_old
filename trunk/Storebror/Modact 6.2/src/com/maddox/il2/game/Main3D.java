...
public class Main3D extends Main {
	...
	class FarActorFilter implements ActorFilter {
		...
		private void drawPointer(int i, double d, int j, int k) {
			...
			d3 += j;
			d4 += k;
			HUD.addPointer((float)d3, (float)d4, Army.color(i),
					(float)((1.0D - d / iconDrawMax) * 0.80000000000000004D + 0.20000000000000001D), (float)d2);
		}
		...
		public boolean isUse(Actor actor, double d) {
			...
			int i2 = k1 | (Main3D.iconFarColor & 0xff) * i1 + (l & 0xff) * j1 >>> 8
					| ((Main3D.iconFarColor >>> 8 & 0xff) * i1 + (l >>> 8 & 0xff) * j1 >>> 8) << 8
					| ((Main3D.iconFarColor >>> 16 & 0xff) * i1 + (l >>> 16 & 0xff) * j1 >>> 8) << 16;
			int j2 = l1 | (Main3D.iconFarColor >>> 1 & 0x3f) * i1 + (l >>> 0 & 0xff) * j1 >>> 8
					| ((Main3D.iconFarColor >>> 9 & 0x3f) * i1 + (l >>> 8 & 0xff) * j1 >>> 8) << 8
				...
				if (d <= dotrange.id()) {
					...
					if (World.cur().isArcade() && !Mission.isNet()) {
						...
						if (aircraft.FM instanceof Maneuver) {
							...
							float f2 = (float)j3 / 10F;
							s1 = s1 + " T:" + i3 + "% G:" + f2 + " " + ManString.tname(maneuver.get_task()) + ""
									+ ManString.name(maneuver.get_maneuver());
						}
					}
				}
	...
	class ShadowPairsFilter implements ActorFilter {
		...
		public boolean isUse(Actor actor, double d) {
			if (actor != shadowPairsCur1 && (actor instanceof ActorHMesh) && d <= Main3D.shadowPairsR2
		...
		public void preRender() {
			if (Engine.land().ObjectsReflections_Begin(0) == 1) {
				...
				if (Landscape.isExistMeshs()) {
					drwSolid.clear();
					drwTransp.clear();
					Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxVisualDistance * 0.5F, 1, drwSolidL, drwTranspL,
							null, flag);
				...
				Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxVisualDistance * 0.5F, 14, drwSolid, drwTransp, null,
						flag);
	...
	public boolean saveRecordedMission(String s) {
		...
		try {
			...
			long l = Finger.incLong(mission.finger(), 137);
	...
	private void doPreRender3D(Render render) {
		...
		Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxVisualDistance, 4, drwarray.drwSolid, drwarray.drwTransp,
				drwarray.drwShadow, true);
		Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxLongVisualDistance, 8, drwarray.drwSolid, drwarray.drwTransp,
		...
		if (!bRenderMirror || viewMirror > 1) {
			Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxStaticVisualDistance, 2, drwarray.drwSolid,
					drwarray.drwTransp, drwarray.drwShadow, false);
			Engine.drawEnv().preRender(__p.x, __p.y, __p.z, World.MaxPlateVisualDistance, 1, drwarray.drwSolidPlate,
					drwarray.drwTranspPlate, drwarray.drwShadowPlate, true);
		}
	...
	private void clipFarActorItems() {
		...
		for (int j = 0; j < iconFarListLen[iRenderIndx]; j++) {
			...
			for (int k = j + 1; k < iconFarListLen[iRenderIndx]; k++) {
				...
				if (faractoritem1.x + faractoritem1.dx >= faractoritem.x && faractoritem1.x <= faractoritem.x + faractoritem.dx
						&& faractoritem1.y + i >= faractoritem.y && faractoritem1.y <= faractoritem.y + i) {
	...
	protected void drawFarActors() {
		...
		if (iconFarPadlockItem.str != null) {
			...
			Render.drawLines(line3XYZ, 3, 1.0F, iconFarPadlockItem.color, Mat.TESTZ | Mat.MODULATE | Mat.NOTEXTURE | Mat.BLEND,
					5);
			Render.drawEnd();
		}
		clearFarActorItems();
	...
	private void drawRandomSplashScreen() {
        ...
        if(splashScreenSectionIndex >= 0)
        {
            ...
            for(int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVars; splashScreenFileNameIndex++)
            {
                ...
                if (splashScreenFileName.length() < 4) continue;
                if (!splashScreenFileName.endsWith(".jpg") && !splashScreenFileName.endsWith(".jpeg")) continue;
                splashScreenFileNameArray.add(splashScreenFileName);
            }
        }
        
        if (splashScreenFileNameArray == null) return;
	...
	private void drawSplashScreenTest() {
		...
		for (int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVarsWide; splashScreenFileNameIndex++) {
			String splashScreenFileName = sectFileSplashScreens.line(splashScreenSectionIndexWide, splashScreenFileNameIndex)
					.trim();
		...
		for (int splashScreenFileNameIndex = 0; splashScreenFileNameIndex < numVarsNormal; splashScreenFileNameIndex++) {
			String splashScreenFileName = sectFileSplashScreens.line(splashScreenSectionIndexNormal, splashScreenFileNameIndex)
					.trim();
			if (splashScreenFileName.length() < 4)
				continue;
			if (!splashScreenFileName.endsWith(".jpg") && !splashScreenFileName.endsWith(".jpeg"))
	...
	private void debugSplashScreen(String theLine) {
		if (DEBUG_SPLASH) System.out.println(theLine);
	}
	...
	public static final String gameHotKeyCmdEnvs[] = { "Console", "hotkeys", "HookView", "PanView", "HeadMove", "SnapView",
			"pilot", "move", "gunner", "misc", "orders", "aircraftView", "timeCompression", "gui" };
