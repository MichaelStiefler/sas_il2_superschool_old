...
public class Supersonic {
    ...
    public float getDragFactorForMach(float theMachValue) {
        ...
        for (i = 0; i < this.fDragFactorX.length; i++)
            if (this.fDragFactorX[i] > theMachValue) break;
        if (i < 1) return this.fDragFactorY[0];
        if (i >= this.fDragFactorX.length) i = this.fDragFactorX.length - 1;
        if (i >= this.fDragFactorY.length) i = this.fDragFactorY.length - 1;
