...
public class Missile extends Rocket 
  // TODO: reactivate for interceptable missiles
    ...
    if (Time.current() > tStart + (2 * this.lTrackDelay)) {
      if (Actor.isValid(victim)) {
        ...
        if (/*((victim instanceof Aircraft) || (victim instanceof MissileInterceptable)) && */f2 > prevd && prevd != 1000F) {
          if (f2 < 30F) {
//            EventLog.type("stepTargetHoming f2 < 30F");
            this.collide(false);
            this.doExplosionAir();
            this.postDestroy();
            this.drawing(false);
...
//      EventLog.type("stepTargetHoming Actor invalid");
      this.doExplosionAir();
      this.postDestroy();
      this.collide(false);
      this.drawing(false);
    ...
    if (Time.current() > this.tStart + (2 * this.lTrackDelay)) {
      if (Actor.isValid(this.victim)) {
        ...
        if (((victim instanceof Aircraft) || (victim instanceof MissileInterceptable)) && (f2 > this.prevd) && (this.prevd != 1000.0F)) {
          if (f2 < 30.0F) {
//            EventLog.type("stepBeamRider f2 < 30F");
            this.doExplosionAir();
            this.postDestroy();
            this.collide(false);
            this.drawing(false);
...
//    System.out.println("MissileInit() flags=" + this.flags);
    if (this instanceof MissileInterceptable) {
//      this.flags = 0x1000;
//      this.flags &= 0xffffffcf;
//      this.flags |= 0xe0; // impact OK
      this.flags |= 0xd0; // collision OK
      this.collide(true);
//      com.maddox.il2.engine.MissilePos.collideChange(this, true);
      this.drawing(true);
//      this.dreamFire(true);
...
//    super.start(-1F, 0);
    ...
    this.setArmy(this.getOwner().getArmy());
...
// TODO: Implement working interceptable missile code
...
//  {
//    EventLog.type("msgShot 1");
//    Exception localException = new Exception();
//    localException.printStackTrace();
//    paramShot.bodyMaterial = Shot.BODY_METALL;
//
...
//    
//    
//    if ((paramShot.chunkName == null) || (paramShot.chunkName.equals("")))
//    {
//      return;
//    }
//    if(paramShot.chunkName.startsWith("x")) {
...
////    EventLog.type("msgShot 2");
//      this.explosionLoc = new Loc(this.pos.getAbs());
//      Explosions.HydrogenBalloonExplosion(this.explosionLoc, null);
//      
//      this.collide(false);
//      this.doExplosionAir();
//      this.postDestroy();
...
//  }
//
//  public void msgExplosion(com.maddox.il2.ai.Explosion paramExplosion)
//  {
//    EventLog.type(this.getClass().getName() + " msgExplosion 1 from " + paramExplosion.initiator.getClass().getName());
//    Exception localException = new Exception();
//    localException.printStackTrace();
...
//    if (this.isDestroyed) return;
//    this.isDestroyed = true;
////    EventLog.type(this.getClass().getName() + " msgExplosion 2");
//      this.explosionLoc = new Loc(this.pos.getAbs());
//      Explosions.HydrogenBalloonExplosion(this.explosionLoc, null);
//      this.collide(false);
//      this.doExplosionAir();
...
//  {
//    EventLog.type(this.getClass().getName() + " msgCollision 1 from " + paramActor.getClass().getName() + " s1=" + paramString1 + " s2=" + paramString2);
//    Exception localException = new Exception();
//    localException.printStackTrace();
//    if (!Actor.isValid(paramActor)) {
//      EventLog.type(this.getClass().getName() + " msgCollision 1 paramActor invalid!");
//      return;
//    }
//    if ((paramActor.net != null) && (paramActor.net.isMirror())) {
//      EventLog.type(this.getClass().getName() + " msgCollision 1 paramActor.net!");
//      return;
//    }
//    if (this.isDestroyed) return;
//    this.isDestroyed = true;
...
//      Explosions.HydrogenBalloonExplosion(this.explosionLoc, null);
//      this.collide(false);
//      this.doExplosion(paramActor, paramString2);
//      this.postDestroy();
//      this.drawing(false);
//      this.destroy();
//  }
...
//    {
//    EventLog.type(this.getClass().getName() + " msgCollisionRequest 1 from " + actor.getClass().getName());
...
////                    if(l >= Time.tickNext())
////                        l = Time.tickNext() - 1L;
////                    com.maddox.il2.engine.MsgCollision.post2(l, this, actor, "<edge>", "Body");
//        
//    }
//
