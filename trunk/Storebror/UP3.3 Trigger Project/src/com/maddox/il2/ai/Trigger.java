...
public class Trigger extends Actor
{
    ...
    protected boolean checkPeriodic()
    {
        ...
        if(sLink != "" && sLink != null && Actor.getByName(sLink) == null)
            return false;
        ResultTrigger result = Engine.collideEnv().getEnemiesInCyl(posTigger, rayon, altiMin, altiMax, army, iaHumans, avionMin);
    ...
    protected Trigger(String zname, int i, int j, int posx, int posy, int r, int zmin, 
            int zmax, int ziaHumans, boolean bSortie, int zAvionMin, int zProba, String zslink, String sTextDisplay, 
            int zTextDuree)
    {
        declanche = false;
        bIsEnter = false;
    ...
    protected String prepareTextArmy(int i)
    {
        ...
        if(l >= 0)
            do
            {
                ...
                if(textDisplay.startsWith(s1, l))
                {
                    l += s1.length();
                    subString(stringbuffer, textDisplay, l, i1);
                }
    ...
    protected static String getGrid(Point2d pos)
    {
        ...
        double d1 = (Main3D.cur3D().land2D.worldOfsX() + ((Tuple2d) (pos)).x) / 10000D;
        double d2 = (Main3D.cur3D().land2D.worldOfsX() + ((Tuple2d) (pos)).y) / 10000D;
