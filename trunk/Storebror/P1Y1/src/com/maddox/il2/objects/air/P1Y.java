...
public abstract class P1Y extends Scheme2 {
    ...
    public void update(float f) {
        ...
        if (P1Y.getGameVersion() >= 4.12F) {
            float bayDoor = this.FM.CT.getBayDoor();
            if (Math.abs(this.fGingaBayDoor1 - bayDoor) > 0.003F) {
                this.fGingaBayDoor1 += 0.005F * (bayDoor <= this.fGingaBayDoor1 ? -1F : 1F);
                this.moveGingaBayDoor1(this.fGingaBayDoor1);
            } else if ((Math.abs(this.fGingaBayDoor1 - bayDoor) > 0.000001F) && ((Math.abs(bayDoor) - 1F) < 0.000001F)) {
                this.fGingaBayDoor1 = bayDoor;
                this.moveGingaBayDoor1(this.fGingaBayDoor1);
            }
    
            if (Math.abs(this.fGingaBayDoor2 - bayDoor) > 0.019F) {
                this.fGingaBayDoor2 += 0.025F * (bayDoor <= this.fGingaBayDoor2 ? -1F : 1.5F);
                this.moveGingaBayDoor2(this.fGingaBayDoor2);
            } else if ((Math.abs(this.fGingaBayDoor2 - bayDoor) > 0.000001F) && ((Math.abs(bayDoor) - 1F) < 0.000001F)) {
                this.fGingaBayDoor2 = bayDoor;
                this.moveGingaBayDoor2(this.fGingaBayDoor2);
            }
        }
...
//        HUD.training("W:" + df.format(this.FM.EI.engines[0].tWaterOut) + " (" + df.format(this.FM.EI.engines[0].tWaterCritMax) + "), O:" + df.format(this.FM.EI.engines[0].tOilOut) + " (" + df.format(this.FM.EI.engines[0].tOilCritMax) + ")");
        super.update(f);
    }
    ...
    {
        if (P1Y.getGameVersion() >= 4.12F) return;
    ...
    private float   fGingaBayDoor1        = 0.0F;
    private float   fGingaBayDoor2        = 0.0F;
    private static float fGameVersion = -99.999F;
    
    public static float getGameVersion() {
        if (fGameVersion < 0.0F) GetGameVersion();
        return fGameVersion;
    }

    private static void GetGameVersion() {
        if (fGameVersion > 0.0F) return;
        ...
        versionToParse += 'F';
        fGameVersion = Float.parseFloat(versionToParse);
    }
