...
class OrderChange_Formation_To_7 extends Order {

    public OrderChange_Formation_To_7() {
        super("C_F_LINEASTERN");
    }
            ...
            if (Actor.isAlive(aircraft) && (aircraft.FM instanceof Pilot) && Actor.isAlive(aircraft.FM.actor)) {
                ...
                if (pilot.Group != null) {
                    if (this.isEnableVoice() && this.CommandSet()[i] != this.Player() && (this.CommandSet()[i].getWing() == this.Player().getWing() || this.CommandSet()[i].aircIndex() == 0))
                        Voice.speakLineAstern(this.CommandSet()[i]);
                    pilot.Group.setFormationAndScale((byte) 5, pilot.formationScale, true);
                }
            }
        }
