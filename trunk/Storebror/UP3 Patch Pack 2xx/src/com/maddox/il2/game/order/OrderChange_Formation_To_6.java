...
class OrderChange_Formation_To_6 extends Order {

    public OrderChange_Formation_To_6() {
        super("C_F_LINEABREAST");
    }
            ...
            if (Actor.isAlive(aircraft) && (aircraft.FM instanceof Pilot) && Actor.isAlive(aircraft.FM.actor)) {
                ...
                if (pilot.Group != null) {
                    if (this.isEnableVoice() && this.CommandSet()[i] != this.Player() && (this.CommandSet()[i].getWing() == this.Player().getWing() || this.CommandSet()[i].aircIndex() == 0))
                        Voice.speakLineAbreast(this.CommandSet()[i]);
                    pilot.Group.setFormationAndScale((byte) 4, pilot.formationScale, true);
                }
            }
        }
