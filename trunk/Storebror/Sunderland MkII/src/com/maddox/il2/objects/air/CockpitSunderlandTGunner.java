...
public class CockpitSunderlandTGunner extends CockpitGunner {
    ...
    protected void interpTick() {
        if (!this.isRealMode())
            return;
        if (this.emitter == null || !this.emitter.haveBullets() || !this.aiTurret().bIsOperable || this.suppressGunFire)
            this.bGunFire = false;
    ...
    public void moveGun(Orient orient) {
        ...
        /*
         * Add rotation offset when turret is elevated.
         * This is just an estimated value, it's necessary because plane's gun hooks are not aligned orthogonally
    ...
    public void clipAnglesGun(Orient orient) {
        ...
        /*
         * limit turret movement to FN turret physical limits and
         * avoid shooting own plane
         */
        this.suppressGunFire = false;
        if (f >= 3F && f < 11F) {
            if (f1 < 27F)
                this.suppressGunFire = true; // Don't shoot at own plane!
        } else if (f >= 11F && f < 38F) {
            if (f1 < -6F)
                this.suppressGunFire = true; // Don't shoot at own plane!
    ...
    protected boolean doFocusEnter() {
        if (super.doFocusEnter()) {
            /*
             * When entering turret position, make sure to hide external 3D model of the turret,
             * and hide the gunner,
             * they shall not be seen from inside the turret!
             */
