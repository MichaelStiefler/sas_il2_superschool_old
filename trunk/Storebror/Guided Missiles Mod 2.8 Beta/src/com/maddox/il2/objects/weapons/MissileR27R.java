...
public class MissileR27R extends Missile {
	...
	static {
		...
		Property.set(class1, "timeFire", 6.0F); // Rocket Engine Burn time in Seconds
		Property.set(class1, "force", 72000F); // Rocket Engine Power (in Newton)
		...
		Property.set(class1, "power", 3.90F); // RL Data: 39kg HE warhead, for realism reduced to 1/10th of it's RL weight
		Property.set(class1, "radius", 24.5F); // Radius of Explosion Effect
		Property.set(class1, "kalibr", 0.23F); // Diameter of Missile Body
		Property.set(class1, "massa", 253.0F); // Missile Weight at Launch time
		Property.set(class1, "massaEnd", 186F); // Missile Weight when Booster burned out
		...
		Property.set(class1, "minPkForAI", 35.0F); // min. Kill Probability for AI to launch a missile
		Property.set(class1, "timeForNextLaunchAI", 10000L); // time between two Missile launches for AI
		Property.set(class1, "engineDelayTime", 500L); // Rocket Engine Start Delay Time (in milliseconds), negative Value means that Engine is started before Missile leaves it's rail
		...
		Property.set(class1, "maxLockGForce", 8.0F); // max. G-Force for lockon
		...
		Property.set(class1, "PkMaxFOVto", 180.0F); // max Angle offset from target aft POV for Pk calculation
		Property.set(class1, "PkDistMin", 3000.0F); // min Distance for Pk calculation
		Property.set(class1, "PkDistOpt", 15000.0F); // optimum Distance for Pk calculation
		...
		Property.set(class1, "stepsForFullTurn", 12); // No. of ticks (1 tick = 30ms) for full control surface turn, higher value means slower reaction and smoother flight, lower value means higher agility
		...
		Property.set(class1, "friendlyName", "R-27"); // Display Name of this missile
