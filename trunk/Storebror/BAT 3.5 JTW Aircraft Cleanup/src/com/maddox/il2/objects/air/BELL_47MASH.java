...
public class BELL_47MASH extends Scheme1
    implements TypeScout, TypeTransport, TypeStormovik
{
    ...
    protected void moveFan(float f)
    {
        ...
        if(oldProp[j] < 2)
        {
            ...
            {
                hierMesh().chunkVisible(Aircraft.Props[0][oldProp[j]], false);
                oldProp[j] = i;
        ...
        hierMesh().chunkSetAngles(Aircraft.Props[j][0], 0.0F, -this.propPos[j], 0.0F);
        if(oldProp[k] < 2)
        {
            i = Math.abs((int)(this.FM.EI.engines[0].getw() * 0.1F));
            if(i >= 1)
                i = 1;
            if(i != oldProp[k] && hierMesh().isChunkVisible(Aircraft.Props[k][oldProp[k]]))
            {
                hierMesh().chunkVisible(Aircraft.Props[k][oldProp[k]], false);
                oldProp[k] = i;
                hierMesh().chunkVisible(Aircraft.Props[k][i], true);
            }
        }
        if(i == 0)
        {
        ...
        hierMesh().chunkSetAngles(Aircraft.Props[k][0], 0.0F, -this.propPos[k], 0.0F);
    }
