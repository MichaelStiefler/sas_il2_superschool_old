        ...
        if(bHasDeployedDragChute && this.FM.CT.bHasDragChuteControl)
            if(this.FM.CT.DragChuteControl == 1.0F && this.FM.getSpeedKMH() > 600F || this.FM.CT.DragChuteControl < 1.0F)
            {
                if(chute != null)
                {
                    ...
                    ((Actor) (chute)).pos.setRel(new Point3d(-10D, 0.0D, 1.0D), new Orient(0.0F, 30F + (this.FM.getSpeedKMH() / 600F) * 50F, 0.0F));
                }
...
;
    ...
    private float chuteElev;
    private float chuteIElev;
    private float chuteAz;
    private float chuteIAz;
