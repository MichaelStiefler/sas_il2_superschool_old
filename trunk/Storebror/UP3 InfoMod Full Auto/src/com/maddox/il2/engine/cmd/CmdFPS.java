...
import com.maddox.il2.objects.weapons.MGunNullGeneric;
...
public class CmdFPS extends Cmd implements MsgTimeOutListener {
    ...
    private String fpsInfo() {
        return "" + (int) Math.floor(fpsCur) + " avg:" + (int) Math.floor((1000D * (double) (framePrev - frameStart)) / (double) (timePrev - timeStart)) + " max:" + (int) Math.floor(fpsMax) + " min:" + (int) Math.floor(fpsMin) + " #fr:"
                + (framePrev - frameStart);
    }
...
//	    for (int iAirClassIterator = DumpStart; iAirClassIterator < 10; iAirClassIterator++) {
            ...
            pw.flush();
            pw.close();
            //Reflection.invokeMethod(consoleLogFile, "close");
            try {
...
//      EnginesInterface.inFmDump = true;
        ...
        try {
            ...
            if (aircraftFM != null) {
                ...
                Reflection.invokeMethod(theAircraft, "weaponsLoad", new Object[] { "default" });
...
//              System.out.println(i18Name + " Masses: massEmpty=" + theFlightModel.M.massEmpty + ", maxFuel=" + theFlightModel.M.maxFuel + ", maxNitro=" + theFlightModel.M.maxNitro + ", parasiteMass=" + theFlightModel.M.parasiteMass + ", mass="
//                      + theFlightModel.M.mass);
...
//                Wing.maxP[k] = EI.forcePropAOA(k, 1000F, 1.1F, true);
    ...
    public static void DataToFile(Motor m, PrintWriter printwriter) {
        ...
        if (m.propDirection == 0)
            printwriter.println("left");
        else if (1 == m.propDirection)
            printwriter.println("right");
        if (m.type == 0 || m.type == 1 || m.type == 7) {
            printwriter.println("Horse powers: " + m.horsePowers);
            printwriter.println("Engine mass: " + m.engineMass);
        } else {
            printwriter.println("Thrust: " + m.thrustMax);
        }
        printwriter.println("Boost factor: " + m.engineBoostFactor);
        printwriter.println("WEP boost factor: " + m.engineAfterburnerBoostFactor);
        printwriter.println("Throttle control: " + (m.bHasThrottleControl ? "+" : "-"));
        printwriter.println("Afterburner control: " + (m.bHasAfterburnerControl ? "+" : "-"));
        printwriter.println("Propeller control: " + (m.bHasPropControl ? "+" : "-"));
        printwriter.println("Mixture control: " + (m.bHasMixControl ? "+" : "-"));
        printwriter.println("Magneto control: " + (m.bHasMagnetoControl ? "+" : "-"));
        printwriter.println("Compressor control: " + (m.bHasCompressorControl ? "+" : "-"));
        printwriter.println("Feather control: " + (m.bHasFeatherControl ? "+" : "-"));
        printwriter.println("Radiator control: " + (m.bHasRadiatorControl ? "+" : "-"));
        printwriter.println("Extinguishers control: " + (m.bHasExtinguisherControl ? "+" : "-"));
...
//            System.out.println("Aircraft " + theAircraft.getClass().getName() + " has " + weaponSlotNames.size() + " Weapon Slots and " + weaponTriggers.length + " Triggers.");
            for (int i = 0; i < weaponSlotNames.size(); i++) {
                ...
                weaponSlotClearName = weaponSlotClearName.replace('=', '·');
                weaponSlotClearName = weaponSlotClearName.replace(';', '·');
//                System.out.println("Weapon Slot " + weaponSlotName + " for Aircraft " + aircraftName + " has clear name " + weaponSlotClearName);
//                System.out.println("Parsing Weapon Slot '" + weaponSlotClearName + "'");
                ...
                for (int j = 0; j < weaponSlot.length; j++) {
                    ...
                    // System.out.println("Slot index " + j + " = " +
                    // stripLeading(weaponClass.getName(),
                    // "com.maddox.il2.objects.weapons.") + ", Trigger = " +
                    // weaponTriggers[j] + ", Type = " + type);
                    if (weaponClass.equals(BombGunNull.class))
                        continue;
                    if (weaponClass.equals(GunNull.class))
                        continue;
                    if (weaponClass.equals(MGunNull.class))
                        continue;
                    if (weaponClass.equals(MGunNullGeneric.class))
                        continue;
                    if (weaponClass.equals(RocketGunNull.class))
                        continue;
                    int num = 1;
                    ...
                    if (BombGun.class.isAssignableFrom(weaponClass) || RocketGun.class.isAssignableFrom(weaponClass) || RocketBombGun.class.isAssignableFrom(weaponClass) || FuelTankGun.class.isAssignableFrom(weaponClass)) {
                        ...
                        num = bullets;
//                        System.out.println("bulletClass = " + stripLeading(weaponClass.getName(), "com.maddox.il2.objects.weapons."));
                    } else if (type == WeaponSlot.TYPE_TANK) {
//                        System.out.println("Trigger is 9, but no bomb, rocket, rocketbomb or fuel tank... skip!");
                        continue;
                    }
                    if (weaponClass.equals(RocketNull.class))
                        continue;
...
//                    System.out.println("addWeaponSlot(" + weaponName + ", " + num + ", " + bullets + ", " + type + ", <list>);");
    ...
    public static String       DumpDesc   = "UP3 RC4";
