	...
	public Element createTopLevelXml() throws FlickrException, SAXException, IOException {
    	...
    	for (Collection collection : collectionsInterface.getTree(null, null)) {
    		...
    		allCollections.addContent(new Element("collection")
    			...
    			.addContent(XmlUtils.downloadMediaAndCreateElement("iconLarge", largeFile, iconsDir.getName() + File.separator + largeFile.getName(), iconLarge, this.configuration.downloadCollectionIcons, this.configuration))
    			.addContent(XmlUtils.downloadMediaAndCreateElement("iconSmall", smallFile, iconsDir.getName() + File.separator + smallFile.getName(), iconSmall, this.configuration.downloadCollectionIcons, this.configuration))
    			.addContent(setsEle));
    	}
