    ...
    implements TypeFighter, TypeBNZFighter, TypeFighterAceMaker, TypeSupersonic
{
    ...
    public Banshee_X()
    {
        oldthrl = new float[] { -1.0F, -1.0F };
        curthrl = new float[] { -1.0F, -1.0F };
        engineSurgeDamage = new float[] { 0.0F, 0.0F };
        arrestor2 = 0.0F;
    ...
    protected boolean cutFM(int i, int j, Actor actor)
    {
        ...
        {
        case 19:
            for (int engineIndex=0; engineIndex<2; engineIndex++) this.FM.EI.engines[engineIndex].setEngineDies(actor);
            break;
        case 33:
    ...
    public void engineSurge(float f)
    {
        ...
        for (byte b = 0; b < 2; b++) {
            if(curthrl[b] == -1F)
            {
                curthrl[b] = oldthrl[b] = this.FM.EI.engines[b].getControlThrottle();
            } else
            {
                curthrl[b] = this.FM.EI.engines[b].getControlThrottle();
                if(curthrl[b] < 1.05F)
                {
                    if((curthrl[b] - oldthrl[b]) / f > 20F && this.FM.EI.engines[b].getRPM() < 3200F && this.FM.EI.engines[b].getStage() == 6 && World.Rnd().nextFloat() < 0.4F)
                    {
                        ...
                        engineSurgeDamage[b] += 0.01D * (double)(this.FM.EI.engines[b].getRPM() / 1000F);
                        this.FM.EI.engines[b].doSetReadyness(this.FM.EI.engines[b].getReadyness() - engineSurgeDamage[b]);
                        if(World.Rnd().nextFloat() < 0.05F && (this.FM instanceof RealFlightModel) && ((RealFlightModel)this.FM).isRealMode())
                            this.FM.AS.hitEngine(this, b, 100);
                        if(World.Rnd().nextFloat() < 0.05F && (this.FM instanceof RealFlightModel) && ((RealFlightModel)this.FM).isRealMode())
                            this.FM.EI.engines[b].setEngineDies(this);
                    }
                    if((curthrl[b] - oldthrl[b]) / f < -20F && (curthrl[b] - oldthrl[b]) / f > -100F && this.FM.EI.engines[b].getRPM() < 3200F && this.FM.EI.engines[b].getStage() == 6)
                    {
                        this.playSound("weapon.MGunMk108s", true);
                        engineSurgeDamage[b] += 0.001D * (double)(this.FM.EI.engines[b].getRPM() / 1000F);
                        this.FM.EI.engines[b].doSetReadyness(this.FM.EI.engines[b].getReadyness() - engineSurgeDamage[b]);
                        if(World.Rnd().nextFloat() < 0.4F && (this.FM instanceof RealFlightModel) && ((RealFlightModel)this.FM).isRealMode())
                        {
                            ...
                            this.FM.EI.engines[b].setEngineStops(this);
                        } else
                        if(this.FM.actor == World.getPlayerAircraft())
                            HUD.log(AircraftHotKeys.hudLogWeaponId, "Compressor Stall!");
                    }
    ...
    public void update(float f)
    {
        ...
        if(this.FM.AS.isMaster() && Config.isUSE_RENDER()) {
            for (byte b = 0; b < 2; b++) {
                if(this.FM.EI.engines[b].getPowerOutput() > 0.5F && this.FM.EI.engines[b].getStage() == 6)
                {
                    if(this.FM.EI.engines[b].getPowerOutput() > 0.75F)
                        this.FM.AS.setSootState(this, b, 3);
                    else
                        this.FM.AS.setSootState(this, b, 2);
                } else
                {
                    this.FM.AS.setSootState(this, b, 0);
                }
            }
        }
