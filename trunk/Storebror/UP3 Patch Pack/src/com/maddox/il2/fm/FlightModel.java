            ...
            case Turret.TU_MO_SLEEP:
                ...
                if (Time.current() > theTurret.timeNext) {
                    ...
                    if (theTurret.target == null) {
                        ...
                        else if (isDawn && this.isTargetExposed(theTurret.target)) {
                        ...
                        if (isDawn) {
                            ...
                            if (this.isNightTargetVisible(theTurret.target, gunnerSkillFloat) || this.isTargetExposed(theTurret.target)) {
                                theTurret.tMode = Turret.TU_MO_TRACKING;
                                theTurret.timeNext = 0L;
                            }
                        } else {
...
//                turret1.tuLim[1] = tu[1] * World.Rnd().nextFloat(0.91F + 0.03F * f2, 1.09F - 0.03F * f2) + World.Rnd().nextFloat(-5F + 1.666F * f2, 5F - 1.666F * f2);
        ...
        if (this.shoot)
            this.shoot = !this.isComingFromTheSun(theTurret.target);
    ...
    private boolean isNightTargetVisible(Actor actor, float f) {
        ...
        float f3 = 500F;
        if (this.am.pos.getAbs().getZ() < actor.pos.getAbs().getZ()) {
            if (actor.pos.getAbs().getZ() < f2 + f3)
                if (i > 2)
                    f1 *= 1.2F;
                else if (i > 3)
                    f1 *= 1.3F;
            f1 *= 1.1F;
        } else if (actor.pos.getAbs().getZ() > f2 + f3)
            if (i > 2)
                f1 *= 1.2F;
            else if (i > 3)
                f1 *= 1.3F;
        v3.sub(this.am.pos.getAbsPoint(), actor.pos.getAbsPoint());
        float f4 = FlightModel.cvt(f, 0.0F, 3F, 0.75F, 1.2F);
        float f5 = FlightModel.cvt((float) v3.length(), 0.0F, 800F, 1.0F, 0.1F);
        float f6 = FlightModel.cvt(f1, 0.095F, 1.0F, 1E-005F, this.tAcc1) * f5 * f4;
    ...
    private boolean isComingFromTheSun(Actor actor) {
        if ((actor instanceof Aircraft) && World.Sun().ToSun.z > 0.0F) {
            if (Mission.curCloudsType() > 3 && actor.pos.getAbs().getZ() < Mission.curCloudsHeight() + 200F)
                return false;
            v3.set(World.Sun().ToSun.x, World.Sun().ToSun.y, World.Sun().ToSun.z);
            v2.sub(this.am.pos.getAbsPoint(), actor.pos.getAbsPoint());
