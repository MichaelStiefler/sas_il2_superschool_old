...
public class Supersonic {
    ...
    public final void load(SectFile theSectFile) {
        ...
        this.allParamsSet = theSectFile.exist(strSection, "VminMach") && theSectFile.exist(strSection, "VsoundMach1") && theSectFile.exist(strSection, "CfsoundMach1") && theSectFile.exist(strSection, "VsoundMach2")
                && theSectFile.exist(strSection, "CfsoundMach2") && theSectFile.exist(strSection, "VmaxMach") && theSectFile.exist(strSection, "CfmaxMach");
    }

    public float getDragFactorForMach(float theMachValue) {
        if (theMachValue <= this.VminMach)
            return 1.0F;
        if (theMachValue >= this.VmaxMach)
            return this.CfmaxMach;

        int i = 0;
        for (i = 0; i < this.fDragFactorX.length; i++) {
            if (this.fDragFactorX[i] > theMachValue)
                break;
        }
        if (i < 1)
            return this.fDragFactorY[0];
        if (i >= this.fDragFactorX.length)
            i = this.fDragFactorX.length - 1;
        if (i >= this.fDragFactorY.length)
            i = this.fDragFactorY.length - 1;
        float baseFactor = this.fDragFactorY[i - 1];
        float spanFactor = this.fDragFactorY[i] - baseFactor;
        float baseMach = this.fDragFactorX[i - 1];
        float spanMach = this.fDragFactorX[i] - baseMach;
