...
public class HUD {
	...
	private final void renderSpeed() {
		...
		if(!bMixedMessage)
		{
			...
			k2 = (int)((((SndAircraft) ((Aircraft)main3d.viewActor())).FM).Or.getYaw() + 0.5F);
			k2 = k2 > 90 ? 450 - k2 : 90 - k2;
		} else
		if(main3d.viewActor() instanceof Aircraft)
		{
			...
			k2 = (int)(((FlightModelMain) (((SndAircraft) ((Aircraft)main3d.viewActor())).FM)).Or.getYaw() + 0.5F);
			k2 = k2 > 90 ? 450 - k2 : 90 - k2;
		} else
		{
			fa = 0.0F;
		...
		default:
			...
			i1 = iIAS = (int)(3.6F * Pitot.Indicator((float)d, fs) * speedbarMultiplierKMH * (float)speedbarSpdKMH + 0.5F);
		...
		case 2: // '\002'
			...
			i1 = iIAS = (int)(1.943845F * Pitot.Indicator((float)d, fs) * speedbarMultiplierKnots * (float)speedbarSpdKnots + 0.5F);
		...
		case 3: // '\003'
			l = (int)(3.28084F * fa * speedbarMultiplierFeet * (float)speedbarAltFeet + 0.5F);
			i1 = iIAS = (int)(2.236936F * Pitot.Indicator((float)d, fs) * speedbarMultiplierMPH * (float)speedbarSpdMPH + 0.5F);
		...
		case 4: // '\004'
			l = (int)(fa * speedbarMultiplierMeters * (float)speedbarAltMeters + 0.5F);
			i1 = iTAS = (int)(3.6F * fs * speedbarMultiplierKMH * (float)speedbarSpdKMH + 0.5F);
		...
		case 5: // '\005'
			l = (int)(3.28084F * fa * speedbarMultiplierFeet * (float)speedbarAltFeet + 0.5F);
			i1 = iTAS = (int)(1.943845F * fs * speedbarMultiplierKnots * (float)speedbarSpdKnots + 0.5F);
		...
		case 6: // '\006'
			l = (int)(3.28084F * fa * speedbarMultiplierFeet * (float)speedbarAltFeet + 0.5F);
			i1 = iTAS = (int)(2.236936F * fs * speedbarMultiplierMPH * (float)speedbarSpdMPH + 0.5F);
		...
		if(!bMixedMessage)
		{
			if(bUseColor)
				if(World.getPlayerAircraft() != null)
					j = main3d.viewActor().getArmy() != 1 ? 0xc0ff0000 : 0xc00000ff;
			ttfont.output(j, 5F, 5F, 0.0F, renderSpeedSubstrings[0][0] + " " + k2 + " " + renderSpeedSubstrings[0][1]);
						...
						if(!bSA)
							try
							{
								...
								if(sbAIManeuver >= 1.0F)
									ttfont.output(j, 5F, 5 + (3 + (int)sbAIManeuver) * i, 0.0F, renderSpeedSubstrings[17][0] + " " + maneuver + " , sub:" + submaneuver + " , speedMode:" + speedMode + " , spd:" + smConstSpeed + "m/s , pwr:" + smConstPower + " " + ((TaxiMode)?", TaxiMode":" ") + renderSpeedSubstrings[17][1]);
								if(sbThrottle >= 1.0F)
									ttfont.output(j, 5F, 5 + (3 + (int)sbThrottle) * i, 0.0F, "Rudder:" + rudder + ", Elevator:" + elevator + ", Aileron:" + aileron + ", " + renderSpeedSubstrings[18][0] + throttle + renderSpeedSubstrings[18][1]);
								if(sbOrient >= 1.0F)
									ttfont.output(j, 5F, 5 + (3 + (int)sbOrient) * i, 0.0F, "Yaw:" +  ((yaw > 90)? 450 - yaw : 90 - yaw) + ", Pitch:" + ((pitch > 180)? pitch - 360 : pitch) + ", Roll:" + (360 - roll));
								if(sbHPSet >= 1.0F)
