...
public class Gear
{
    ...
    public Gear()
    {
      ...
      //TODO: +++ CTO Mod 4.12 +++
		...
		catapultPowerJets = Config.cur.ini.get("Mods", "CatapultPowerJets", 25F);
		if (Mission.curYear() > 1958) {
			catapultPower += 3F;
			catapultPowerJets += 10F;
		}
		if (Mission.curYear() < 1955) {
			catapultPower -= 3F;
			catapultPowerJets -= 3F;
		}
...
//            if((Mission.isCoop() || Mission.isDogfight()) && !Mission.isServer() && Actor.isAlive(FM.brakeShoeLastCarrier) && Time.current() < 60000L)
    ...
    private boolean testGearCollision(int i)
    {
        ...
        case 1: // '\001'
            ...
            if(i == 0)
                ...
                f = FM.CT.getGearR();
            if(bIsMaster && (NormalVPrj < 0.0D || f < 0.99F))
            {
        ...
        case 2: // '\002'
            ...
            float f1 = FM.CT.getGearC();
            if(!bIsMaster || NormalVPrj >= 0.0D && f1 >= 0.99F)
                break;
            double d21 = (ForwardVPrj * ForwardVPrj + RightVPrj * RightVPrj) * 0.0001D;
            if((double)f1 < 0.94999999999999996D)
                d21 = 4D;
            else
            if((double)f1 < 0.98999999999999999D)
                d21 *= 2D;
            if(FM.CT.bHasArrestorControl)
  ...
  //TODO: +++ CTO Mod 4.12 +++
	public boolean setCatapultOffset(BigshipGeneric bigshipgeneric) {
		...
		dCatapultYaw2 = 0.0D;
		bCatapultAI = false;
		String shipName = bigshipgeneric.getClass().getName();
		if (shipName.endsWith("USSKitkunBayCVE71") || shipName.endsWith("USSCasablancaCVE55") || shipName.endsWith("USSShamrockBayCVE84")) {
			...
			if (!bCatapultAllowAI || !bCatapultAI_CVE)
				...
				bCatapultAI = true;
		} else if (shipName.endsWith("USSEssexCV9") || shipName.endsWith("USSIntrepidCV11")) {
			dCatapultOffsetX = 9D;
			...
			if (!bCatapultAllowAI || !bCatapultAI_EssexClass)
				...
				bCatapultAI = true;
		} else if (shipName.endsWith("HMSIllustriousCV")) {
			dCatapultOffsetX = 7D;
		...
		if (!flag2) {
			if (shipName.indexOf("ShipPack.Carrier0") != -1) {
				dCatapultOffsetX = 5.8D;
				dCatapultOffsetY = -113.5D;
				dCatapultOffsetX2 = 20.1D;
		...
		if (!flag2) {
			if (shipName.indexOf("ShipPack.Carrier1") != -1) {
				dCatapultOffsetX = 4.2D;
				dCatapultOffsetY = -64D;
				flag2 = true;
				if (!bCatapultAllowAI || !bCatapultAI_CVE)
		...
		if (!flag2) {
			if (shipName.endsWith("USSSanJacintoCVL30") || shipName.endsWith("USSBelleauWoodCVL24") || shipName.endsWith("USSPrincetonCVL23")) {
				if (bStandardDeckCVL) {
					dCatapultOffsetX = 6D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSValleyForgeCV45") || shipName.endsWith("USSBoxerCV21")) {
				dCatapultOffsetX = 9D;
				dCatapultOffsetY = -130D;
				dCatapultOffsetX2 = 27.6D;
				dCatapultOffsetY2 = -137.5D;
		...
		if (!flag2) {
			if (shipName.endsWith("GrafZeppelin") || shipName.endsWith("PeterStrasser") || shipName.endsWith("Aquila")) {
				dCatapultOffsetX = 5.8D;
				dCatapultOffsetY = -113.5D;
				dCatapultOffsetX2 = 20.1D;
				dCatapultOffsetY2 = -113.5D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSShangriLaCV38_1955")) {
		...
		if (!flag2) {
			if (shipName.endsWith("USSOriskanyCV34_1965")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSKearsargeCV33_1965")) {
				dCatapultOffsetX = 9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = 0.5D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSHancockCV19_1957") || shipName.endsWith("USSHancockCV19_1968")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSRandolphCV15_1957") || shipName.endsWith("USSRandolphCV15_1962")) {
		...
		if (!flag2) {
			if (shipName.endsWith("USSHornetCV12_1970")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
				dCatapultOffsetX2 = 24.1D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSIntrepidCV11_1970") || shipName.endsWith("USSIntrepidCV11_1957")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSYorktownCV10_1956") || shipName.endsWith("USSEssexCV9_1957") || shipName.endsWith("USSHornetCV12_1957")) {
		...
		if (!flag2) {
			if (shipName.endsWith("USSEssexCV9_1960")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
				dCatapultOffsetX2 = 24.5D;
		...
		if (!flag2) {
			if (shipName.endsWith("USSEssexCV_ead_SCB125")) {
				dCatapultOffsetX = 6.9D;
				dCatapultOffsetY = -41D;
				dCatapultYaw = -1.6D;
				dCatapultOffsetX2 = 24.5D;
		...
		if (!flag2) {
			if (shipName.endsWith("FSClemenceauR98CV_1962")) {
				dCatapultOffsetX = 9.8D;
				dCatapultOffsetY = -46D;
				dCatapultYaw = 0.3D;
				dCatapultOffsetX2 = -0.7D;
				dCatapultOffsetY2 = 47D;
				dCatapultYaw2 = 5D;
				flag2 = true;
