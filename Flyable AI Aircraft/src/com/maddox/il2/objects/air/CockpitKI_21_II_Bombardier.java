...
public class CockpitKI_21_II_Bombardier extends CockpitPilot {
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			float f = ((KI_21_II) aircraft()).fSightCurForwardAngle;
			float f1 = ((KI_21_II) aircraft()).fSightCurSideslip;
			CockpitKI_21_II_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
			mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitKI_21_II_Bombardier.calibrAngle + f);
			if (bEntered) {
				HookPilot hookpilot = HookPilot.current;
				hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitKI_21_II_Bombardier.calibrAngle + f, 0.0F);
			}
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (bEntered) {
			mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((KI_21_II) aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
			boolean flag = ((KI_21_II) aircraft()).fSightCurReadyness > 0.93F;
