...
public class CockpitMOSQUITO4_Bombardier extends CockpitPilot {
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			if (bEntered) {
				...
				mesh.chunkSetAngles("BlackBox", -10F * f1, 0.0F, CockpitMOSQUITO4_Bombardier.calibrAngle + f);
				HookPilot hookpilot = HookPilot.current;
				hookpilot.setInstantOrient(aAim + 10F * f1, tAim + CockpitMOSQUITO4_Bombardier.calibrAngle + f, 0.0F);
			}
			return true;
		}
	...
	public void reflectWorldToInstruments(float f) {
		...
		if (bEntered) {
			mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((MOSQUITO4) aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
	...
	static {
		Property.set(CockpitMOSQUITO4_Bombardier.class, "astatePilotIndx", 0);
	}
}
