...
public abstract class P_47X extends Scheme1 implements TypeFighter, TypeBNZFighter, TypeStormovik {
	...
	protected void hitBone(String s, Shot shot, Point3d point3d) {
		if (s.startsWith("xx")) {
			if (s.startsWith("xxarmor")) {
				...
				if (s.endsWith("p1")) {
					getEnergyPastArmor((double) World.Rnd().nextFloat(20F, 40F) / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
					FM.AS.setCockpitState(shot.initiator, FM.AS.astateCockpitState | 2);
					if (shot.power <= 0.0F)
						doRicochetBack(shot);
				} else if (s.endsWith("p2"))
					getEnergyPastArmor(12.520000457763672D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
				else if (s.endsWith("p3"))
					getEnergyPastArmor(12.560000419616699D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
				else if (s.endsWith("p4"))
					getEnergyPastArmor(12.560000419616699D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
				else if (s.endsWith("f1")) {
					getEnergyPastArmor(2.0999999046325684D / (Math.abs(Aircraft.v1.x) + 9.9999997473787516E-005D), shot);
					if (shot.powerType == 3)
						shot.powerType = 2;
				}
				return;
		...
		if (s.startsWith("xcf") || s.startsWith("xcockpit")) {
			...
			if (s.startsWith("xcockpit")) {
				if (point3d.z > 0.7160000205039978D && World.Rnd().nextFloat() < 0.25F)
	...
	public void update(float f) {
		float f1 = Aircraft.cvt(FM.EI.engines[0].getControlRadiator(), 0.0F, 1.0F, 0.0F, -5F);
		for (int i = 1; i < 13; i++)
			hierMesh().chunkSetAngles("Cowflap" + i + "_D0", 0.0F, f1, 0.0F);

		f1 = Aircraft.cvt(FM.EI.engines[0].getControlRadiator(), 0.0F, 1.0F, 25F, 0.0F);
	...
	public void sfxFlaps(boolean flag) {
		if (flag) {
			bFlaps = true;
			bFlapsEnd = false;
			P_47X.printDebugMessage("*** Flaps sound starts");
		}
		if (bFlaps && !flag) {
			bFlaps = false;
			bFlapsEnd = true;
			P_47X.printDebugMessage("*** Flaps sound ends");
		}
	}
	...
	public void sfxWheels() {
		if (FM.getSpeedKMH() > 0.0F) {
			...
			P_47X.printDebugMessage("*** Wheels sound used");
		}
	}
	...
	protected static void printDebugMessage(String theMessage) {
		if (_DEBUG)
			P_47X.printDebugMessage(theMessage);
	}
	...
	static {
		Class class1 = com.maddox.il2.objects.air.P_47X.class;
