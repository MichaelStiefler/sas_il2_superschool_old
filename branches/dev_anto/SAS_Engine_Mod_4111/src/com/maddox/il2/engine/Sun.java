...
public class Sun {
	...
	public void activate() {
		if (Config.isUSE_RENDER())
			setNative(ToLight.x, ToLight.y, ToLight.z, Ambient, Diffuze,
	...
	public void setLight(float f, float f1, float f2, float f3, float f4,
			float f5) {
		Ambient = f;
		Diffuze = f1;
		Specular = f2;
	...
	public void setAstronomic(int i, int j, int k, float f, float f1) {
		...
		Specular = f22; // cvt(sunMultiplier, darkness, 1.0F, 0.5F, 1.0F);
		f23 += f22 * ((f18 / 16F) * 0.4F + 0.3F);
		f22 -= f23;
		//Diffuze = f22 * sunMultiplier;
		//Ambient = f23 * sunMultiplier;
		// --------------------------------------------------------------------------
		 Diffuze = f22 * 1.4285F;
		 Ambient = f23 * 1.4285F;
		if (Ambient < 0.001F)
	...
	private static native void setNative(float f, float f1, float f2, float f3,
