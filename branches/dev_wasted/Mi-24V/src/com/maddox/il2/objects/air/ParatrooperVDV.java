...
public class ParatrooperVDV extends ActorMesh
    implements MsgCollisionRequestListener, MsgCollisionListener, MsgExplosionListener, MsgShotListener
{
    ...
    class Master extends ParaNet
    {
        ...
        public Master(Actor actor)
        {
            ...
            float f = (float)ParatrooperVDV.p.z - Engine.land().HQ((float)ParatrooperVDV.p.x, (float)ParatrooperVDV.p.y);
            if(f <= 500F)
...
//
//
//			insert(spawn, new Loc (pos.getAbsPoint().x, pos.getAbsPoint().y, 0.0F, ii, 0.0F, 0.0F), actor3.getArmy());
//
...
//    		pos.getAbs(point3d);
            ...
            case 3: // '\003'
                pos.getAbs(ParatrooperVDV.p);
                Engine.land();
                float f = Landscape.HQ((float)ParatrooperVDV.p.x, (float)ParatrooperVDV.p.y);
                if(st == 0)
                    if(l >= 2500L)
                    {
                        pos.setAbs(faceOrient);
                        if(dying == 0 && (float)ParatrooperVDV.p.z - f <= turn_para_on_height && speed.z < -5D)
                        {
                            st = 1;
                            animStartTime = Time.current();
                    ...
                    } else
                    {
                        ...
                        if(f1 >= 1.0F)
                            f1 = 1.0F;
                        ParatrooperVDV.o.interpolate(startOrient, faceOrient, f1);
                ...
                ParatrooperVDV.p.scaleAdd(Time.tickLenFs(), speed, ParatrooperVDV.p);
                speed.z -= Time.tickLenFs() * World.g();
                if(st == 2)
                {
                    if(speed.x != 0.0D)
                        speed.x -= (Math.abs(speed.x) / speed.x) * 0.0099999997764825821D * (speed.x * speed.x) * (double)Time.tickLenFs();
                    if(speed.y != 0.0D)
                        speed.y -= (Math.abs(speed.y) / speed.y) * 0.0099999997764825821D * (speed.y * speed.y) * (double)Time.tickLenFs();
                    EventLog.type("320");
                } else
                {
                    if(speed.x != 0.0D)
                        speed.x -= (Math.abs(speed.x) / speed.x) * 0.0010000000474974513D * (speed.x * speed.x) * (double)Time.tickLenFs();
                    if(speed.y != 0.0D)
                        speed.y -= (Math.abs(speed.y) / speed.y) * 0.0010000000474974513D * (speed.y * speed.y) * (double)Time.tickLenFs();
                    EventLog.type("327");
                }
                ...
                if(ParatrooperVDV.p.z <= (double)f)
                {
                    ...
                    } else
                    {
                        ...
                        animStartTime = Time.current();
                        EventLog.type("369");
                        if(name().equals("_paraplayer_") && Mission.isNet() && World.getPlayerFM() != null && Actor.isValid(World.getPlayerAircraft()) && World.getPlayerAircraft().isNetPlayer())
                        {
            ...
            case 4: // '\004'
                pos.getAbs(ParatrooperVDV.p);
                ParatrooperVDV.p.scaleAdd(Time.tickLenFs(), speed, ParatrooperVDV.p);
                ParatrooperVDV.p.z = Engine.land().HQ(ParatrooperVDV.p.x, ParatrooperVDV.p.y);
                pos.setAbs(ParatrooperVDV.p);
                
                if(World.land().isWater(ParatrooperVDV.p.x, ParatrooperVDV.p.y))
                {
                    if(swimMeshCode < 0)
                    {
                        st = 5;
...
//                    st = 5;
                    ...
                    insert(spawn, new Loc(pos.getAbsPoint().x, pos.getAbsPoint().y, ((Tuple3d) (pos.getAbsPoint())).z, (float)rndOr, 0.0F, 0.0F), army);
    ...
    {
        return "3do/humans/Paratroopers/" + (i != 2 ? "Russia" : "Russia") + "/mono.sim";
    }
    ...
    public ParatrooperVDV(Actor actor, int i, int j, Loc loc, Vector3d vector3d, NetMsgInput netmsginput, int k)
    {
        super(GetMeshName(i));
