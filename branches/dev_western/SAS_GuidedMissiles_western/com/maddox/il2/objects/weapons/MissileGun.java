...
// Author:           Storebror
// Edit:			western0221 on 13th/Nov/2017
...
//		testException.printStackTrace();
		...
		try {
			...
			if (Actor.isValid(this.actor) && (this.actor instanceof Aircraft) && (this.actor instanceof TypeGuidedMissileCarrier) && (Aircraft) this.actor == World.getPlayerAircraft()
					&& ((RealFlightModel) ((SndAircraft) ((Aircraft) this.actor)).FM).isRealMode() && ((TypeGuidedMissileCarrier) this.actor).getGuidedMissileUtils().hasMissiles()
					&& (Time.current() < ((TypeGuidedMissileCarrier) this.actor).getGuidedMissileUtils().getStartLastMissile() + this.shotFrequency)) {
		...
		if (!(NetMissionTrack.isPlaying() || this.actor.isNetMirror() /*Mission.isNet()*/)) {
			if ((paramInt > 0) && Actor.isValid(super.actor) && (this.actor instanceof TypeGuidedMissileCarrier) && (World.cur().diffCur.Limited_Ammo || (this.actor != World.getPlayerAircraft()))) {
				((TypeGuidedMissileCarrier) this.actor).getGuidedMissileUtils().shotMissile();
			}
		}
	}

	// moving missiles like F/A-18 wingtip AIM-9 launchers in folding / extending, F-102 missile bay suspentions up / down.
