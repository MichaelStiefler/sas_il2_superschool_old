    ...
    public boolean interpolateStep()
    {
        ...
        if(Time.current() > tStart + 350L)
            if(!locked)
            {
                ...
                if(victim != null){
                    Actor flare = NearestTargets.getEnemy(0, -1, p, 4200D, getOwner().getArmy());
                    if(flare instanceof MissileInterceptable)
                    	victim = flare;
                }
                if(victim instanceof MissileInterceptable)
                	HUD.log(AircraftHotKeys.hudLogWeaponId, "Locked On Flare");
                if(victim != null)
                {
