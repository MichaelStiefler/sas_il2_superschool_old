...
public class MissileSAM extends Missile {
	...
	public MissileSAM(Actor actor, NetChannel netchannel, int i, Point3d point3d, Orient orient, float f, Actor radartarget) {
		this.MissileSAMInit(actor, netchannel, i, point3d, orient, f, radartarget);
	}
	...
	private boolean computeFuzeState() {
		...
		//			Time.setPause(true); 
		//			HUD.log(AircraftHotKeys.hudLogWeaponId, "266");
		//			this.doExplosionAir();
		//			this.postDestroy();
		//			this.collide(false);
		//			this.drawing(false);
	...
	private void doStart(float f, Actor radartarget) {
		...
		this.setMissileVictim(radartarget);
	...
	private void getMissileProperties() {
		...
		this.maxFOVto = Property.floatValue(localClass, "maxFOVto", 99.9F);
	...
	private int engineHeatSpreadType(Actor theActor)
	{
		...
		for (int i = 0; i < checkEI.getNum(); i++) {
			...
			if (iMotorType == Motor._E_TYPE_INLINE || iMotorType == Motor._E_TYPE_RADIAL || iMotorType == Motor._E_TYPE_HELO_INLINE) {
	...
	private void setMissileVictim(Actor radartarget) {
		if (radartarget != null) {
			...
			this.pos.setAbs(this.missilePoint3d, this.missileOrient);
			victim = com.maddox.il2.ai.air.NearestTargetsAngle.getEnemyAngle(9, -1, this.missilePoint3d, this.attackMaxDistance, getArmy(), this.missileOrient, maxFOVfrom);
		}
	}
	...
	//		
	//		if (this.detectorType == DETECTOR_TYPE_MANUAL) this.victim = null;
	//		try {
	...
	//					targetDistance = GuidedMissileUtils.distanceBetween(this, theTarget1);
	//					if (targetDistance > attackMaxDistance) {
	//						continue;
	//					}
	//					targetAngle = GuidedMissileUtils.angleBetween(this, theTarget1);
	//					if (targetAngle > maxFOVfrom) {
	//						continue;
	//					}
	//					targetAngleAft = 180.0F - GuidedMissileUtils.angleBetween(theTarget1, this);
	//					if (targetAngleAft > maxFOVto) {
	//						if (this.detectorType == DETECTOR_TYPE_INFRARED) {
	...
	//							victim = theTarget1;
	//							if (theTarget1 instanceof Aircraft) {
	//								this.victimOffsetPoint3d = theEI.engines[maxEngineForceEngineNo].getEnginePos();
	//								this.safeVictimOffset.set(this.victimOffsetPoint3d);
	//							}
	//						}
	//						break;
	//					}
	//					case DETECTOR_TYPE_RADAR_BEAMRIDING:{
	...
	//						break;
	//					}
	//					case DETECTOR_TYPE_RADAR_HOMING:{
	//						Object[] fireDeviceArr = (Object[])Reflection.getValue(getOwner(), "arms");
	//						if (fireDeviceArr.length > 0) {
	//						  Aim aim = (Aim)Reflection.getValue(fireDeviceArr[0], "aim");
	//						  victim = aim.getEnemy();
	//						  this.victimOffsetPoint3d.set(0, 0, 0);
	//						  this.safeVictimOffset.set(this.victimOffsetPoint3d);
	//						}
	//						break;
	//					}
	//					case DETECTOR_TYPE_RADAR_TRACK_VIA_MISSILE: 
	//					default:
	//						break;
	//					}
	//
	...
	public boolean stepTargetHoming() {
		...
		if (Time.current() < this.startTime + this.trackDelay) {
			...
			if (theVictimSpeed > 10D) {
				...
				if (detectorType == DETECTOR_TYPE_INFRARED) victimAdvance -= 5.0D; // impact point 10m aft of engine (track exhaust).
	...
	private void checkChaffFlareLock() {
		...
		for (int counterMeasureIndex = 0; counterMeasureIndex < counterMeasureSize; counterMeasureIndex++) {
			...
			if(detectorType == DETECTOR_TYPE_INFRARED && flarechaff instanceof RocketFlare && flareLockTime < lockTime && flareDistance < 200D && (double) (GuidedMissileUtils.angleActorBetween(victim, this)) > 30)
			{
	...
	private void doEvade() {
		...
		for (int TargetsIndex = 0; TargetsIndex < TargetsSize; TargetsIndex++) {
			...
			if (prey != World.getPlayerAircraft() && prey instanceof Aircraft && prey.getArmy() != getArmy()) {
				double rnd = TrueRandom.nextDouble(0D, 80D);
				double rndDist = TrueRandom.nextDouble(0D, 1000D);
				double missileDistance = GuidedMissileUtils.distanceBetween(this, prey);
				int victimSkillDist = ((Pilot)((Aircraft)prey).FM).Skill * 1000;
				int victimSkill = ((Pilot)((Aircraft)prey).FM).Skill * 10;
				if (missileDistance < (1000 + victimSkillDist + rndDist) && victimSkill > rnd) {
					if (prey instanceof TypeFighter || prey instanceof TypeStormovik || prey instanceof TypeStormovikArmored) {
						((Pilot)((Aircraft)prey).FM).set_maneuver(100);
					}
					if (((Pilot)((Aircraft)prey).FM).CT.Weapons[7] != null && ((Pilot)((Aircraft)prey).FM).CT.Weapons[7][((Pilot)((Aircraft)prey).FM).CT.Weapons[7].length - 1] != null && ((Pilot)((Aircraft)prey).FM).CT.Weapons[7][((Pilot)((Aircraft)prey).FM).CT.Weapons[7].length - 1].haveBullets()) { 
						((Pilot)((Aircraft)prey).FM).CT.WeaponControl[7] = true;
					} else {
						((Pilot)((Aircraft)prey).FM).CT.WeaponControl[7] = false;
					}
					if (((Pilot)((Aircraft)prey).FM).CT.Weapons[8] != null && ((Pilot)((Aircraft)prey).FM).CT.Weapons[8][((Pilot)((Aircraft)prey).FM).CT.Weapons[8].length - 1] != null && ((Pilot)((Aircraft)prey).FM).CT.Weapons[8][((Pilot)((Aircraft)prey).FM).CT.Weapons[8].length - 1].haveBullets()) { 
						((Pilot)((Aircraft)prey).FM).CT.WeaponControl[8] = true;
					} else {
						((Pilot)((Aircraft)prey).FM).CT.WeaponControl[8] = false;
					}
				}
			}
		}
