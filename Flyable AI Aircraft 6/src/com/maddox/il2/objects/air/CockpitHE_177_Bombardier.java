...
public class CockpitHE_177_Bombardier extends CockpitPilot {
	...
	class Interpolater extends InterpolateRef {
		...
		public boolean tick() {
			...
			setOld = setNew;
			setNew = setTmp;
			float f = ((HE_177A3) aircraft()).fSightCurForwardAngle;
			float f1 = ((HE_177A3) aircraft()).fSightCurSideslip;
			CockpitHE_177_Bombardier.calibrAngle = 360F - fm.Or.getPitch();
...
//		}
		...
		hookpilot.doAim(false);
			if (hasToKG) {
				hookpilot.setAim(new Point3d(5.75D, -0.146D, -0.18D));
				Point3d point3d = new Point3d();
				point3d.set(0.35D, 0.0D, 0.0D);
				hookpilot.setTubeSight(point3d);
			} else {
				Point3d point3d = new Point3d();
				point3d.set(0.18D, 0.056D, -0.12D);
				hookpilot.setTubeSight(point3d);
			}
	...
	private void enter411andLater() {
		...
		HookPilot hookpilot = HookPilot.current;
		if (hasToKG) {
			CmdEnv.top().exec("fov 15.0");
			hookpilot.setInstantOrient(0.0F, -3F, 0.0F);
			hookpilot.pCamera().add(ToKG_HeadOffset);
	...
	private void leave411andLater() {
		HookPilot hookpilot = HookPilot.current;
		if (hasToKG)
			hookpilot.pCamera().sub(ToKG_HeadOffset);
		if (enteringAim) {
			hookpilot.doAim(false);
			if (hasToKG) {
				hookpilot.setInstantOrient(0.0F, -3F, 0.0F);
	...
	public CockpitHE_177_Bombardier() {
		super("3DO/Cockpit/He-177A-3/Bombardier.him", "he111");
		enteringAim = false;
		doToKGCheck = true;
		hasToKG = false;
		bEntered = false;
		...
		prevFuel = 0.0F;
		this.ToKG_HeadOffset = new Point3d(ToKG_HeadOffsetParams);
		try {
			Loc loc = new Loc();
	...
	public void reflectWorldToInstruments(float f) {
		...
		boolean bShowBombsight = bEntered;
			bShowBombsight &= !hasToKG;
		if (bShowBombsight) {
			mesh.chunkSetAngles("zAngleMark", -floatindex(cvt(((HE_177A3) aircraft()).fSightCurForwardAngle, 7F, 140F, 0.7F, 14F), angleScale), 0.0F, 0.0F);
			boolean flag = ((HE_177A3) aircraft()).fSightCurReadyness > 0.93F;
	...
	private static final double ToKG_HeadOffsetParams[] = { 2.8D, 0.0D, 0.013D };
	private Point3d ToKG_HeadOffset;

	static {
		Class class1 = CockpitHE_177_Bombardier.class;
