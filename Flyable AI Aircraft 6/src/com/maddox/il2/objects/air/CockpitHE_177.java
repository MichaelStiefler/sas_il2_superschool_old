...
public class CockpitHE_177 extends CockpitPilot {
	...
	protected boolean doFocusEnter() {
		...
		if (doToKGCheck) {
			doToKGCheck = false;
			if ((aircraft() instanceof HE_177A3) && ((HE_177A3)aircraft()).hasToKG) {
				hasToKG = true;
				showToKG();
			}
		}
	...
	public CockpitHE_177() {
		...
		cockpitNightMats = (new String[] { "Peil1", "Peil2", "Instrument1", "Instrument2", "Instrument4", "Instrument5",
				"Instrument6", "Instrument7", "Instrument8", "Instrument9", "Needles" });
		setNightMats(false);
	...
	public void reflectWorldToInstruments(float f) {
		...
		mesh.chunkSetAngles("Z_Alt1Set", 0.0F, (((HE_177A3)aircraft()).fSightCurAltitude * 360F) / 6000F, 0.0F);
		Pn.set(fm.Loc);
		Pn.z = (double)f1 - Engine.cur.land.HQ(Pn.x, Pn.y);
		mesh.chunkSetAngles("z_AFN101", 0.0F, cvt((float)Pn.z, 0.0F, 750F, 0.0F, 223F), 0.0F);
		mesh.chunkSetAngles("z_Speed", 0.0F,
				floatindex(cvt(Pitot.Indicator((float)fm.Loc.z, fm.getSpeedKMH()), 50F, 800F, 0.0F, 15F), speedometerScale),
				0.0F);
		...
		mesh.chunkSetAngles("z_RPM1", 0.0F, cvt(f2, 600F, 3600F, -172F, 156F), 0.0F);
		mesh.chunkSetAngles("z_RPM2", 0.0F, cvt(f3, 600F, 3600F, -172F, 156F), 0.0F);
		mesh.chunkSetAngles("z_ATA1", 0.0F,
				pictManf1 = 0.9F * pictManf1 + 0.1F * cvt(fm.EI.engines[0].getManifoldPressure(), 0.6F, 1.8F, -162.5F, 162.5F),
				0.0F);
		mesh.chunkSetAngles("z_ATA2", 0.0F,
				pictManf2 = 0.9F * pictManf2 + 0.1F * cvt(fm.EI.engines[1].getManifoldPressure(), 0.6F, 1.8F, -162.5F, 162.5F),
				0.0F);
		mesh.chunkSetAngles("Z_Hours", 0.0F, cvt(World.getTimeofDay(), 0.0F, 24F, 0.0F, 720F), 0.0F);
		mesh.chunkSetAngles("Z_Minutes", 0.0F, cvt(World.getTimeofDay() % 1.0F, 0.0F, 1.0F, 0.0F, 360F), 0.0F);
		...
		mesh.chunkSetAngles("z_ElevTrim", 0.0F, -cvt(interp(setNew.elevTrim, setOld.elevTrim, f), -0.5F, 0.5F, -750F, 750F),
				0.0F);
		mesh.chunkSetAngles("z_RudderTrim", 0.0F,
				cvt(interp(setNew.rudderTrim, setOld.rudderTrim, f), -0.5F, 0.5F, -750F, 750F), 0.0F);
		mesh.chunkSetAngles("z_AileronTrim", 0.0F, cvt(interp(setNew.ailTrim, setOld.ailTrim, f), -0.5F, 0.5F, -750F, 750F),
				0.0F);
		...
		mesh.chunkSetAngles("z_OilPres1", 0.0F,
				cvt(1.0F + 0.05F * fm.EI.engines[0].tOilOut * fm.EI.engines[0].getReadyness(), 0.0F, 15F, 0.0F, -135F), 0.0F);
		mesh.chunkSetAngles("z_OilPres2", 0.0F,
				cvt(1.0F + 0.05F * fm.EI.engines[1].tOilOut * fm.EI.engines[1].getReadyness(), 0.0F, 15F, 0.0F, -135F), 0.0F);
		mesh.chunkSetAngles("z_FuelPres1", 0.0F, cvt(fm.M.fuel <= 1.0F ? 0.0F : 0.78F, 0.0F, 3F, 0.0F, 135F), 0.0F);
		mesh.chunkSetAngles("z_FuelPres2", 0.0F, cvt(fm.M.fuel <= 1.0F ? 0.0F : 0.78F, 0.0F, 3F, 0.0F, 135F), 0.0F);
		...
		mesh.chunkSetAngles("z_OAT", 0.0F,
				floatindex(cvt(Atmosphere.temperature((float)fm.Loc.z) - 273.15F, -60F, 40F, 0.0F, 10F), OATscale), 0.0F);
		mesh.chunkSetAngles("z_TrimIndicator", 0.0F, cvt(interp(setNew.elevTrim, setOld.elevTrim, f), -0.5F, 0.5F, 40F, -40F),
				0.0F);
		...
		mesh.chunkVisible("RWL_Ein", fm.CT.getGearL() < 0.025F || !fm.Gears.lgear);
		mesh.chunkVisible("RWL_Aus", fm.CT.getGearL() > 0.999F && fm.Gears.lgear);
		mesh.chunkVisible("RWR_Ein", fm.CT.getGearR() < 0.001F || !fm.Gears.rgear);
		mesh.chunkVisible("RWR_Aus", fm.CT.getGearR() > 0.975F && fm.Gears.rgear);
		mesh.chunkVisible("RWC_Ein", fm.CT.getGearC() < 0.01F || !fm.Gears.cgear);
		mesh.chunkVisible("RWC_Aus", fm.CT.getGearC() > 0.999F && fm.Gears.cgear);
		...
		mesh.chunkVisible("RK_Ein", fm.CT.getFlap() < 0.1F);
		mesh.chunkVisible("RK_Start", fm.CT.getFlap() > 0.1F && fm.CT.getFlap() < 0.5F);
		mesh.chunkVisible("RK_Aus", fm.CT.getFlap() > 0.5F);
		...
		if (hasToKG) {
			mesh.chunkSetAngles("Ship", 0.0F, -((HE_177A3)aircraft()).fAOB, 0.0F);
			mesh.chunkSetAngles("speedDial", 0.0F, 6.4F * ((HE_177A3)aircraft()).fShipSpeed, 0.0F);
			mesh.chunkSetAngles("Z_Angle", 0.0F, ((HE_177A3)aircraft()).FM.AS.getGyroAngle(), 0.0F);
		}
	}
	...
	private static final float speedometerScale[] = { 0.0F, 17F, 41F, 66F, 93F, 119F, 139F, 164F, 188F, 214F, 239F, 266F, 292F,
			317F, 341F, 372F };
